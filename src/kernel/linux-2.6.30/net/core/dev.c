	...
	list_for_each_entry_rcu(ptype, &ptype_all, list) {
		...
		if ((ptype->dev == dev || !ptype->dev) &&
			...
			if (skb_network_header(skb2) < skb2->data ||
				...
				scrlog_printk(KERN_EXTRA_WARNING"[1][1]protocol %04x is buggy, dev %s\n",skb2->protocol,dev->name);
...
/*
 * TBS_TAG: by WuGuoxiang 2012-7-3
 * Desc: Added hook func for EBT AROUTE
 */
#ifdef CONFIG_BRIDGE_EBT_AROUTE
int (*after_route_hook) (struct sk_buff *skb) = NULL;
#endif
/*
 * TBS_END_TAG
 */
...
/*
 * TBS_TAG: by WuGuoxiang 2012-7-3
 * Desc: go to AROUTE hook
 */
#ifdef CONFIG_BRIDGE_EBT_AROUTE
    if(after_route_hook)
        after_route_hook(skb);
#endif
/*
 * TBS_END_TAG
 */
		...
		if (txq->xmit_lock_owner != cpu) {
			...
			if (net_ratelimit())
				printk(KERN_CRIT "Virtual device %s asks to "
				       "queue packet!\n", dev->name);
			    scrlog_printk(KERN_EXTRA_WARNING"[1][1]Virtual device %s asks toqueue packet!\n",dev->name);
				scrlog_printk(KERN_EXTRA_WARNING"[2][1]Virtual device %s asks toqueue packet!\n",dev->name);
				scrlog_printk(KERN_EXTRA_WARNING"[3][1]Virtual device %s asks toqueue packet!\n",dev->name);
			...
			if (net_ratelimit())
				printk(KERN_CRIT "Dead loop on virtual device "
				       "%s, fix it urgently!\n", dev->name);
			scrlog_printk(KERN_EXTRA_WARNING"[1][1]Dead loop on virtual device %s, fix it urgently!\n",dev->name);
		}
	}
...
/*
 * TBS_TAG: by WuGuoxiang 2012-7-3
 * Desc: Added hook func for PROUTE
 */
#ifdef CONFIG_BRIDGE_EBT_PROUTE
int (*pr_should_route_hook) (struct sk_buff  *skb) = NULL;
#endif
/*
 * TBS_END_TAG
 */
...
int netif_receive_skb(struct sk_buff *skb)
{
	...
	__be16 type;
		...
		if(skb->recvif[0] == '\0' && strcmp(skb->dev->name, "eth") != 0)
		{
			/*在skb->recvif设置成端口设备，端口设备在MTK通过vlan划分的，就是vlan设备*/
			strncpy(skb->recvif, skb->dev->name, sizeof(skb->recvif) - 1);
		}
#endif
...
/*
 * TBS_TAG: by WuGuoxiang 2012-7-4
 * Desc: Go to PROUTE HOOK
 */
#ifdef CONFIG_BRIDGE_EBT_PROUTE
    if (pr_should_route_hook &&
            skb->pkt_type != PACKET_LOOPBACK )
    {
        int kret;
        kret = pr_should_route_hook(skb);
        if (!kret)
        {
            kfree_skb(skb);
            ret = NET_RX_DROP;
