...
#if 1 //Add by Sudenghai 2014-2-24 for dnatnew target
 ...
 * __ip_conntrack_confirm and drop the packet. */
static void
get_unique_tuple_new(struct nf_conntrack_tuple *tuple,
		 const struct nf_conntrack_tuple *orig_tuple,
		 const struct nf_nat_multi_range_compat_new *mr,
		 struct nf_conn *ct,
		 enum nf_nat_manip_type maniptype)
{
	unsigned int orig_min, orig_max, modify_min, modify_max, orig_curr;
	__be16 *portptr;
	const struct nf_nat_range *range = &mr->range[1];
	u16 zone = nf_ct_zone(ct);
	...
	*tuple = *orig_tuple;
	...
	//if((mr->range[0].flags & IP_NAT_RANGE_MAP_FROM_PORT) \
		//&& (mr->range[1].flags & IP_NAT_RANGE_MAP_TO_DEST))
	//{
		portptr = &tuple->dst.u.all;

		orig_curr = ntohs(*portptr);
		orig_min = ntohs(mr->range[0].min.all);
		orig_max = ntohs(mr->range[0].max.all);
		modify_min = ntohs(mr->range[1].min.all);
		modify_max = ntohs(mr->range[1].max.all);

		if((orig_curr >= orig_min) && (orig_curr <= orig_max)) {
			
			*portptr = htons(orig_curr - orig_min + modify_min);
			
			scrlog_printk(KERN_EXTRA_WARNING"[2][3]LAN access from remote, proto:%s, %pI4:%u -> %pI4:%u\n",
                tuple->dst.protonum == IPPROTO_TCP ? "tcp" : "udp",
                &tuple->src.u3.ip,
...
unsigned int
nf_nat_setup_info_new(struct nf_conn *ct,
		  const struct nf_nat_multi_range_compat_new *mr,
		  enum nf_nat_manip_type maniptype)
{
	...
	NF_CT_ASSERT(maniptype == IP_NAT_MANIP_DST);
	BUG_ON(nf_nat_initialized(ct, maniptype));
	...
	nf_ct_invert_tuplepr(&curr_tuple,
			     &ct->tuplehash[IP_CT_DIR_REPLY].tuple);
	get_unique_tuple_new(&new_tuple, &curr_tuple, mr, ct, maniptype);
...
EXPORT_SYMBOL(nf_nat_setup_info_new);
