			...
			case WEBURL_ALL_PART:
				...
				for(prefix_index=0; test_prefixes[prefix_index] != NULL && test == 0; prefix_index++)
				{
					...
					if(!test && strcmp(path, "/") == 0)
					{
					 ...
					 MYDEBUG("test_url = \"%s\", test=%d\n", test_url, test); 
					 if(net_ratelimit())
			         {
			             ipaddr = (unsigned char *)(&(accessSrcIPaddr)); 
			             //if(1 == test)
			             if(info->logflag == WEBURL_LOG_ENABLE) 
					         scrlog_printk(KERN_EXTRA_WARNING"[2][2] Web site %s accessed from %d.%d.%d.%d.\n",test_url,ipaddr[0],ipaddr[1],ipaddr[2],ipaddr[3]);
					 }
				}
				if(!test && strstr(host, "www.") == host)
				{
