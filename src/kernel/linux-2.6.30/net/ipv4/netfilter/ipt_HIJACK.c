...
static char g_data[MAX_PAYLOAD_LEN]={0};
static char g_req_url[MAX_URI_LEN]={0};
static char g_line[MAX_LINE_LEN];
static char g_relative[MAX_URI_LEN];
...
static int skb_get_url(const struct iphdr *ih, struct tcphdr *th, char *url_path, size_t size)
{
		...
		memset(g_relative,0,sizeof(g_relative));
...
static void send_fin(struct sk_buff *oldskb, int hook, struct ipt_hijack_info *info)
{
	...
	char *data=g_data;
	char *req_url=g_req_url;

    memset(g_data,0,sizeof(g_data));
	memset(g_req_url,0,sizeof(g_req_url));
	...
	/* We need a linear, writeable skb.  We also need to expand
	   ...
	   outgoing interface */
    //printk("info->url=%s\n",info->url);
	if(skb_get_url(oiph, oth, req_url, sizeof(g_req_url)))
		snprintf(data, sizeof(g_data)-1, "HTTP/1.1 302 Found\r\nContent-Type: text/html\r\nLocation: http://%s&url=%s\r\n\r\n", info->url, req_url);
	else
		snprintf(data, sizeof(g_data)-1, "HTTP/1.1 302 Found\r\nContent-Type: text/html\r\nLocation: http://%s&url=%s\r\n\r\n", info->url, "notfound");
	
    //printk("HIJACK send_fin() data=%s\n",data);
...
#if 1 /* +++ Huanyao Kang, Attachthe FIN packet to connection track. */
...
#if 1 /* +++ Huanyao Kang, Attachthe RESET packet to connection track. */
	nf_ct_attach(nskb, oldskb);
#endif
	ip_local_out(nskb);
...
{
	const struct ipt_hijack_info *hijack = (const struct ipt_hijack_info *)(par->targinfo);
