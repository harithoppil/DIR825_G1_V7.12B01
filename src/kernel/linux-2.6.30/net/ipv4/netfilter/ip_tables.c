...
check_entry(struct ipt_entry *e, const char *name)
{
	...
	if (!ip_checkentry(&e->ip)) {
		...
		printk("check_entry()__LINE %d: check failed for  %p name=%s\n",__LINE__,e, name);
		return -EINVAL;
	}
	    ...
	    unsigned int *i)
{
	...
	if (ret < 0) {
		duprintf("ip_tables: check failed for `%s'.\n",
			 par.match->name);
		printk("check_match()__line %d: check failed for par.match->name=%s\n",__LINE__,par->match->name);
		return ret;
	}
	++*i;
	return 0;
...
static int check_target(struct ipt_entry *e, const char *name)
{
	...
	if (ret < 0) {
		duprintf("ip_tables: check failed for `%s'.\n",
			 t->u.kernel.target->name);
		printk("check_target()__LINE %d: check failed for t->u.kernel.target->name=%s\n",__LINE__,t->u.kernel.target->name);
		return ret;
	}
	return 0;
}
...
static int __init ip_tables_init(void)
{
	...
	rtl_ip_tables_init_hooks();
		...
		IgmpRxFilter_Hook = NULL;//ipt_do_table;
#endif
