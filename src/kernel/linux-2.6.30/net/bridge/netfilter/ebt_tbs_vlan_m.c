...
static bool ebt_filter_tbs_vlan(const struct sk_buff *skb,
...
static bool ebt_check_tbs_vlan(const struct xt_mtchk_param *par)
{
    struct ebt_tbs_vlan_info *info = (struct ebt_tbs_vlan_info *)par->matchinfo;
...
static struct xt_match filter_tbs_vlan = {
    .name       = EBT_VLAN_MATCH,
    .revision	= 0,
    .family		= NFPROTO_BRIDGE,
    .match      = ebt_filter_tbs_vlan,
    .checkentry = ebt_check_tbs_vlan,
...
{
    return xt_register_match(&filter_tbs_vlan);
}

static void __exit ebt_tbs_vlan_fini(void)
{
    xt_unregister_match(&filter_tbs_vlan);
}
