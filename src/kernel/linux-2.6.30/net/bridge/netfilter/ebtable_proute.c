...
{
	return ebt_do_table(NF_BR_PROUTING, skb, skb->dev, NULL, dev_net(skb->dev)->xt.proute_table);
}
...
static int __init ebtable_proute_init(void)
{
	...
	if (ret < 0)
		return ret;
	rcu_assign_pointer(pr_should_route_hook,ebt_proute);
...
{
	rcu_assign_pointer(pr_should_route_hook,NULL);
	synchronize_net();
