...
static int ebt_target_ftos(struct sk_buff *skb, const struct xt_match_param *par)
{
	//struct ebt_ftos_info *ftosinfo = (struct ebt_ftos_info *)data;
	struct ebt_ftos_info *ftosinfo = par->targinfo;
    ...
    if(NULL == ih)
    {
        //printk("In %s:%d, Unkonw packet, protocol:0x%04x\n", __FILE__, __LINE__, ntohs((*pskb)->protocol));
...
static int ebt_target_ftos_check(const struct xt_tgchk_param *par)
{
	...
	struct ebt_ftos_info *info = par->targinfo;
...
static struct xt_target ftos_target =
{
        .name           = EBT_FTOS_TARGET,
        .family         = NFPROTO_BRIDGE,
        .target         = ebt_target_ftos,
        .checkentry          = ebt_target_ftos_check,
        .targetsize	= XT_ALIGN(sizeof(struct ebt_ftos_info)),
        .me             = THIS_MODULE,
};
