					   ...
					   struct in6_addr *mcastgrp)
{
	...
	//2016-06-07 linsd add for mld v1 only
	//补充组播源的查找
	if(c == NULL) 
    {
        struct in6_addr origin_zero;
        
        memset(&origin_zero, 0, sizeof(origin_zero));
        line = MFC6_HASH(mcastgrp, &origin_zero);
        
      	for (c = net->ipv6.mfc6_cache_array[line]; c; c = c->next) {
      		if (ipv6_addr_equal(&c->mf6c_origin, &origin_zero) &&
      		    ipv6_addr_equal(&c->mf6c_mcastgrp, mcastgrp))
      			break;
      	}
    }
