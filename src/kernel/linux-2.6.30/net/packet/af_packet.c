...
/*
	...
	* Desc: Filter the non-dhcp packet for dhcp raw socket
	*/
	  if(sk->sk_dhcp)
	  {
			 if(!(skb->len > 23 &&										   //WAN Device
				 skb->data[0] == 0x45 && skb->data[9] == 0x11 &&		   //IPv4 UDP Packet
				 skb->data[20] == 0x00 && skb->data[21] == 0x43 &&		   //SRC Port 67
				 skb->data[22] == 0x00 && skb->data[23] == 0x44))		   //DST Port 68
			 {
					 goto drop;
			 }
	  }
			...
			unsigned long arg)
{
    ...
    /* TBS_TAG: by pengyao 2012-05-23 Desc:for dproxy/mld_proxy*/
    case SIOCGRECVIF:
        return copy_to_user((char __user *)arg, sk->recvdev, sizeof(sk->recvdev)) ? -EFAULT: 0;
        break;
#endif
