...
static bool mac_mt(const struct sk_buff *skb, const struct xt_match_param *par)
{
	...
	//unsigned char *ptrMac1;
	//unsigned char *ptrMac2;
    //printk("___________%s(%d)__________\n",__FUNCTION__,__LINE__);
	#if defined(CONFIG_RTL_MAC_FILTER_CARE_INPORT)
	if((info->flags&INPORT_FLAG) && !((1<<(skb->srcPhyPort)) & info->inPortMask)){
		return 0;
	}
	#endif
	//printk("___________%s(%d)_____info->flags=0x%x_____MAC_SRC=0x%x\n",__FUNCTION__,__LINE__,info->flags,MAC_SRC);
	//if (skb_mac_header(skb) >= skb->head
	//			&& (skb_mac_header(skb) + ETH_HLEN) <= skb->data)
    //
	//{
	//    printk("___________%s(%d)__________ptrMac1=%02x:%02x:%02x:%02x:%02x:%02x:\n",__FUNCTION__,__LINE__,*ptrMac1,*(ptrMac1+1),*(ptrMac1+2),*(ptrMac1+3),*(ptrMac1+4),*(ptrMac1+5));
	//    printk("___________%s(%d)__________ptrMac2=%02x:%02x:%02x:%02x:%02x:%02x:\n",__FUNCTION__,__LINE__,*ptrMac2,*(ptrMac2+1),*(ptrMac2+2),*(ptrMac2+3),*(ptrMac2+4),*(ptrMac2+5));
	//}
	if (info->flags & MAC_SRC) {
	    ...
	    if ((skb_mac_header(skb) >= skb->head
	    	...
	    	{
	    	//ptrMac1 = (unsigned char *)(eth_hdr(skb)->h_source);
			//ptrMac2 = (unsigned char *)(info->srcaddr.macaddr);
