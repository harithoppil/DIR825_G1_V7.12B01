...
asmlinkage int panic_printk(const char *fmt, ...)
{
	...
	va_start(args, fmt);
      #if defined (CONFIG_PRINTK)
	    r = vprintk(fmt, args);
       #elif defined(CONFIG_PRINTK_FUNC)
...
#if defined (CONFIG_PRINTK_FUNC)
int tbs_log_to_con = 0;
EXPORT_SYMBOL(tbs_log_to_con);
asmlinkage int scrlog_vprintk(const char *fmt, va_list args)
{
	...
	int current_log_level = default_message_loglevel;
        #else
	/*Modify by sdh 2012-4-21 默认不再使用扩展的日志级别，因为不想让那些没用的信息输出到日志文件
	实现输出日志的过滤*/
	//int current_log_level = (16 << 3) + 4; /* KERN_EXTRA_WARNING */
	int current_log_level = KERN_WARNING;
	/*End by sdh 2012-4-21*/
#endif
	unsigned long flags;
	int this_cpu;
	char *p;
	...
	for (p = printk_buf; *p; p++) {
		if (new_text_line) {
                            /* 使用大于127的loglevel表明要记录到sysenvent.txt中 */
                            #define LOGGER_USE_MULTI_BITS_LEVEL
                            #ifdef LOGGER_USE_MULTI_BITS_LEVEL
                             if (p[0] == '<' &&
                                    p[1] >= '0' && p[1] <= '9' &&
                                    p[2] >= '0' && p[2] <= '9' &&
                                    p[3] >= '0' && p[3] <= '9' &&
			                p[4] == '>') {
				            current_log_level = (p[1] -'0') * 100 + (p[2] -'0') * 10 + (p[3] -'0');
				            p += 5;
				            printed_len -= 5;
			            } else
                            #endif
			...
			/* Always output the token */
 #ifdef LOGGER_USE_MULTI_BITS_LEVEL
            if (current_log_level >= 128)
            {
                /* 128 - 135 */
                emit_log_char('<', tbs_log_to_con);
                emit_log_char(1 + '0', tbs_log_to_con);
                emit_log_char((current_log_level % 100) / 10 + '0', tbs_log_to_con);
                emit_log_char(current_log_level % 10 + '0', tbs_log_to_con);
                emit_log_char('>', tbs_log_to_con);
                printed_len += 5;
            }
            else
            {
                emit_log_char('<', tbs_log_to_con);
                emit_log_char(current_log_level + '0', tbs_log_to_con);
                emit_log_char('>', tbs_log_to_con);
                printed_len += 3;
            }
#else
			...
			if (printk_time) {
				...
				for (tp = tbuf; tp < tbuf + tlen; tp++)
					emit_log_char(*tp, tbs_log_to_con);
				printed_len += tlen;
			}
