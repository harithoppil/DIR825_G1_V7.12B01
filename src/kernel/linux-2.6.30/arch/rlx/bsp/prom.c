...
void __init bsp_init(void)
{
	...
	ptr = (char *)(BOOTARGS_ADDRESS); /* args address */
	strlcpy(arcs_cmdline, ptr, sizeof(arcs_cmdline));

	 ptr = strstr(arcs_cmdline, "mem=");
	if(ptr) {
		mem_size = memparse(ptr + 4, &ptr);
	} 
	if(mem_size < (2 << 20)) {/* Memory size should be larger than 2MB */
		//memsize = (CONFIG_DRAM_SIZE) << 20;  /* Default to CONFIG_DRAM_SIZE MB */
		mem_size = CONFIG_DRAM_SIZE;  /* Default to CONFIG_DRAM_SIZE MB */
	}
