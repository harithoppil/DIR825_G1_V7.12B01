...
#define RTL_HOMEKIT_SIZE 0x20000
...
#endif
#else
	//printk("FLASH: size=0x%x MB %s \r\n", my_sub_mtd[0]->size, my_sub_mtd[0]->name);
	np = parse_mtd_partitions(my_sub_mtd[0], part_probes, &mtd_parts, 0);
	if(np > 0) {		
		add_mtd_partitions(my_sub_mtd[0], mtd_parts, np);
		ret_val = 0;
   	} else {
