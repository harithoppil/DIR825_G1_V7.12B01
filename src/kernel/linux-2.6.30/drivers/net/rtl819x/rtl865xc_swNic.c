...
static void release_pkthdr(struct sk_buff  *skb, int idx)
{
	...
	//printk("skb point :%p pReadyForHw :%p,0x%x\n ",skb,pReadyForHw,(unsigned int)skb);
	//skb point :82514480 pReadyForHw :83366aa0,0x82514480

	if(0 == (((unsigned int)skb) & 0x80000000)) 
	{
        printk("!!!!!!!!!!!!!!release_pkthdr() invalid skb pointer 0x%x\n",((unsigned int)skb));
		return;
	}
	if(0 == (((unsigned int)pReadyForHw) & 0x80000000)) 
	{
        printk("!!!!!!!!!!!!!!release_pkthdr() invalid pReadyForHw pointer 0x%x\n",((unsigned int)pReadyForHw));
		return;
	}
	if(0 == (((unsigned int)skb) & 0x80000000)) 
	{
        printk("!!!!!!!!!!!!!!release_pkthdr() invalid pReadyForHw->ph_mbuf pointer 0x%x\n",((unsigned int)(pReadyForHw->ph_mbuf)));
		return;
	}
