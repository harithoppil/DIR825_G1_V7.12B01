...
//	local_irq_save(flags);
//	rtlglue_printf("[%s %d] ~~~~~~~~~~~~~~~~~IN~~~~~~~~~~~~~\n",__FUNCTION__,__LINE__);
	rtl865xC_dumpAsicDiagCounter(page, &len);
//	rtlglue_printf("[%s %d] ~~~~~~~~~~~~~~~~~OUT~~~~~~~~~~~~\n",__FUNCTION__,__LINE__);
...
#else
static int32 rtl865x_proc_asicCounter_read(struct seq_file *s, void *v)
{
	int len=0;
	static char page[6000];
	rtl865xC_dumpAsicDiagCounter(page, &len);
	seq_printf(s, "%s\n",page);
	return 0;
}
...
{
    return(single_open(file, rtl865x_proc_asicCounter_read, NULL));
}
