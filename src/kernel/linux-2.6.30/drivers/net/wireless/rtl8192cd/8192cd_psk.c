...
extern int rtk_wlan_event_indicate(char* ifname, int event, void* user);
#endif
...
//#ifdef RTL_WPA2
		...
		//		- IGTK Suite:
		//
		if (priv->pmib->dot1180211AuthEntry.dot11EnableSHA256 == TRUE) {
...
static int CheckMIC(OCTET_STRING EAPOLMsgRecvd, unsigned char *key, int keylen)
{
        ...
        ) 
    {
		...
		omac1_aes_128(key, (unsigned char*)tmpeapol, EAPOLMsgRecvd.Length - ETHER_HDRLEN , tmpeapolkey->key_mic);
...
void ft_set_unicastCipher(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	if (priv->pmib->dot1180211AuthEntry.dot11EnablePSK & BIT(1)) {
		if (priv->pmib->dot1180211AuthEntry.dot11WPA2Cipher == 2)
			...
			printk("unicastcipher in wpa = nothing\n");
	}else if (priv->pmib->dot1180211AuthEntry.dot11EnablePSK & (BIT(1)|BIT(0))){
		if (priv->pmib->dot1180211AuthEntry.dot11WPA2Cipher == 10)
			pstat->wpa_sta_info->UnicastCipher = DOT11_ENC_CCMP;
		else
			printk("unicastcipher in wpa = nothing\n");
	}

	//panic_printk("(%s)line=%d, pstat->wpa_sta_info->UnicastCipher = %d\n", __FUNCTION__, __LINE__, pstat->wpa_sta_info->UnicastCipher); 
}
