...
//#if ((RTL8192C_SUPPORT==1) || (RTL8192D_SUPPORT==1) || (RTL8188E_SUPPORT==1) || (RTL8812E_SUPPORT==1))
	...
	if ((priv->up_time % 3) == 0 )  {
		int disable_pwr_ctrl = (pDM_Odm->FalseAlmCnt.Cnt_all > 200 ) ? 1 : 0;
	...
	)
{
	...
	for (path = ODM_RF_PATH_A; path <= ODM_RF_PATH_D; ++path) {
		if (Adapter->MgntInfo.bScanInProgress || pHalData->RegFWOffload == 0)
			PHY_SetTxPowerLevelByPath(Adapter, Channel, (u1Byte)path);
		else {
