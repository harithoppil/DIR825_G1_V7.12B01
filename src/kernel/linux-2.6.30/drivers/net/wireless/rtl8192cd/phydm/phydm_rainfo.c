...
)
{
	...
	phydm_print_rate(pDM_Odm, rate_idx, ODM_COMP_RATE_ADAPTIVE);
...
phydm_UpdateHalRAMask(
	...
	IN	u1Byte			tx_rate_lowest
	)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	u4Byte			ratr_bitmap = 0x000ff015;
	...
	case PHYDM_WIRELESS_MODE_N_5G:
		{
			if (phydm_RfType == PHYDM_RF_2T2R && MimoPs_enable)
	...
	if (wirelessMode != PHYDM_WIRELESS_MODE_B) {
		...
		else if (tx_rate_lowest == 1) 
			ratr_bitmap &=  0xfffffff0;
		 else if (tx_rate_lowest == 2) 
			ratr_bitmap &=  0xffffefe0;
		else if (tx_rate_lowest == 3) 
			ratr_bitmap &=  0xffffcfc0;
		else if (tx_rate_lowest == 4) 
			ratr_bitmap &=  0xffff8f80;
		else if (tx_rate_lowest >= 5) 
			ratr_bitmap &=  0xffff0f00;
		
	}

	ODM_RT_TRACE(pDM_Odm, ODM_COMP_RA_MASK, ODM_DBG_LOUD, ("wirelessMode= (( 0x%x )), RfType = (( %x )), BW = (( %x )), MimoPs_en = (( %d )), tx_rate_lowest= (( %d ))\n", 
		wirelessMode, phydm_RfType, phydm_BW, MimoPs_enable, tx_rate_lowest));

	ODM_RT_TRACE(pDM_Odm, ODM_COMP_RA_MASK, ODM_DBG_LOUD, ("Update RA Mask = (( 0x%x ))\n", ratr_bitmap));

	return ratr_bitmap;
}

u1Byte 
phydm_RA_lowest_decision(
	IN		PVOID			pDM_VOID,
	IN		u4Byte			rssi,
	...
	)
{
	...
	u1Byte	new_Ratr_State;
	u1Byte	i;

	ODM_RT_TRACE(pDM_Odm, ODM_COMP_RA_MASK, ODM_DBG_LOUD, ("curr_ra_lowest_rate = ((%d)), Rate_floor_table ori [ %d , %d, %d , %d, %d, %d]\n", Ratr_State, 
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_RA_MASK, ODM_DBG_LOUD, ("RSSI = ((%d)), Rate_floor_table mod [ %d , %d, %d , %d, %d, %d]\n", 
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_RA_MASK, ODM_DBG_LOUD, ("New_Ratr_State = (( %d ))\n", new_Ratr_State));
...
#if 1
			Ratr_State = phydm_RA_lowest_decision(pDM_Odm, (u4Byte)pstat->rssi, pstat->rssi_level);

			if (pstat->rssi_level != Ratr_State) {
				
				ODM_PRINT_ADDR(pDM_Odm, ODM_COMP_RA_MASK, ODM_DBG_LOUD, ("Target AP addr :"), pstat->hwaddr);
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_RA_MASK, ODM_DBG_LOUD, ("Update tx lowest rate: ((%x)) -> ((%x)),  RSSI = ((%d))\n", 
					 pstat->rssi_level, Ratr_State, pstat->rssi));
				
				pstat->rssi_level = Ratr_State;
