...
int vap_opened_num(struct rtl8192cd_priv *priv)
{
	...
	for (i=0; i<RTL8192CD_NUM_VWLAN; i++) 
	{
		if (IS_DRV_OPEN(priv_root->pvap_priv[i]) && (((GET_MIB((priv_root->pvap_priv[i])))->dot11OperationEntry.opmode)& WIFI_AP_STATE))
			num++;
	}
...
#ifdef BEAMFORMING_SUPPORT
int is_ic_support_txbf(struct rtl8192cd_priv *priv)
{
	if(GET_CHIP_VER(priv) == VERSION_8812E  ||
...
#endif
{
	if(priv->pmib->dot11RFEntry.txbf && IS_VXD_INTERFACE(priv))
	{
		if(GET_ROOT(priv)->pmib->miscEntry.func_off && GET_ROOT(priv)->pmib->miscEntry.vap_enable==0) {
			DEBUG_INFO("Use VXD as pure client mode, OK to use TXBF\n");
		} else
		{
			priv->pmib->dot11RFEntry.txbf = 0;
			DEBUG_INFO("Use VXD for repeater mode, Disable TXBF\n");
		}
	}
}
#endif
...
/*cfg p2p cfg p2p*/
    ...
    if(rtk_p2p_is_enabled(priv)){
        if(priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_5G){
            ...
            if(rtk_p2p_is_enabled(priv)==PROPERTY_P2P){
                P2P_DEBUG("PROPERTY_P2P; ==>p2p_init\n");
                rtk_p2p_init(priv);            
            }
        }
    }    
	...
	*/
	...
	// construct operation and basic rates set
	{
		...
		{ //显示驱动可用信道情况
            int i = 0;
			printk("%s available_chnl: ",priv->dev->name);
			for(i=0;i<priv->available_chnl_num;i++)
			{
                printk(" %d",priv->available_chnl[i]);
			}
		...
		if ( !( (GET_CHIP_VER(priv) == VERSION_8188C) || (GET_CHIP_VER(priv) == VERSION_8192C) 
				|| (GET_CHIP_VER(priv) == VERSION_8188E) || (GET_CHIP_VER(priv) == VERSION_8192E)) ) {
			DEBUG_INFO("Only 88C/92C/88E/92E will use NHM-ACS currently\n");
...
#ifdef UNIVERSAL_REPEATER
		if (IS_VXD_INTERFACE(priv) && priv->pmib->ethBrExtInfo.macclone_enable) {			
...
static int rtl8192cd_stop_sw(struct rtl8192cd_priv *priv)
{
	...
	//printk("rtl8192cd_stop_sw() priv->dev->name=%s\n",priv->dev->name);
...
#endif // DFS
		//printk("rtl8192cd_stop_sw() priv->dev->name=%s LED_TYPE=%d\n",priv->dev->name,LED_TYPE);

		// for SW LED
		if ((LED_TYPE >= LEDTYPE_SW_LINK_TXRX) && (LED_TYPE < LEDTYPE_SW_MAX))
			disable_sw_LED(priv);
		else
			turnoff_hw_LED(priv,LED_TYPE);
...
&& (GET_VXD_PRIV(priv)->simple_config_status == 0)
		...
		if(priv->pptyIE && !priv->reset_hangup) {
...
#if defined(CONFIG_PCI_HCI) && defined(CONFIG_RTL_88E_SUPPORT) && defined(TXREPORT)
			if (GET_CHIP_VER(priv)==VERSION_8188E) {				
				priv->pshare->total_assoc_num++;
        ...
        switch ( ACT_STS(act, sts) )
        {
        	  case ACT_STS(RTW_ACT_IDLE, RTW_STS_NORMAL):
	      	  	if ( is_ps_cond_match(priv))  {
	    	  	    ...
	    	  	    priv->pshare->offload_function_ctrl = 1;
...
int rtl8192cd_open(struct net_device *dev)
{
	...
	DBFENTER;
	scrlog_printk(KERN_EXTRA_WARNING"[2][1]wireless interface %s open!\n",dev->name);
	scrlog_printk(KERN_EXTRA_WARNING"[2][2]wireless interface %s open!\n",dev->name);
	scrlog_printk(KERN_EXTRA_WARNING"[2][3]wireless interface %s open!\n",dev->name);
	priv = GET_DEV_PRIV(dev);
...
#ifdef CONFIG_SDIO_HCI
#ifdef SDIO_AP_OFFLOAD
		priv->pshare->offload_function_ctrl = 0;
		priv->pshare->ps_ctrl = RTW_ACT_IDLE;
		priv->pshare->pwr_state = RTW_STS_NORMAL;
...
#ifdef CONFIG_WSC_PBC_ON_WLAN
	{
		if((GET_CHIP_VER(priv) == VERSION_8192E) && IS_ROOT_INTERFACE(priv))
		{
			RTLWIFINIC_GPIO_config_proc(priv, GPIO_WPS_PIN_NUM, 0x01);
		}
	}
#endif
...
#endif
#endif
	scrlog_printk(KERN_EXTRA_WARNING"[2][1]wireless interface %s close!\n",dev->name);
	scrlog_printk(KERN_EXTRA_WARNING"[2][2]wireless interface %s close!\n",dev->name);
	scrlog_printk(KERN_EXTRA_WARNING"[2][3]wireless interface %s close!\n",dev->name);
...
#if	defined(CONFIG_SLOT_0_8192EE) 
	if (GET_CHIP_VER(priv) == VERSION_8192E)
		ODMPTR->dm_dig_offset = 22;		
#endif
...
#if defined(CONFIG_SLOT_0_RFE_TYPE_2) || defined(CONFIG_SLOT_1_RFE_TYPE_2)
		priv->pmib->dot11RFEntry.rfe_type = 2;
...
/*cfg p2p cfg p2p*/	
...
#endif
	{
		...
		_rtw_mutex_init(&priv->pshare->lock_mbssid);
...
#if !defined(CONFIG_NET_PCI) && (defined(__OSK__) || defined(CONFIG_RTL8196B) || defined(CONFIG_RTL_819X) || defined(CONFIG_RTL_8196E))
...
#ifdef MBSSID
#if defined(__KERNEL__) && (defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI))
	_rtw_mutex_free(&priv->pshare->lock_mbssid);
#endif
#endif // MBSSID
...
#endif
#else
#if defined(CONFIG_RTL_819X) && !defined(CONFIG_RTL8196C_KLD) && !defined(__OSK__)
...
#if defined(CONFIG_WSC_PBC_ON_WLAN)
int wlan_wps_gpio_read( void)
{
	...
	for(wlan_idx=0;(wlan_idx<sizeof(wlan_device)/sizeof(struct _device_info_));wlan_idx++)
	{
		...
		if(GET_CHIP_VER(priv) == VERSION_8192E)
		{
			if(priv->pshare->phw->GPIO_dir[GPIO_WPS_PIN_NUM] == 0x01)
			{
				state = RTLWIFINIC_GPIO_read_proc(priv, GPIO_WPS_PIN_NUM);
			}			
			return state;
		}
	}
