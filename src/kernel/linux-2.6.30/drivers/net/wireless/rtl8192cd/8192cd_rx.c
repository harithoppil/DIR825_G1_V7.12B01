...
#include "./romeperf.h"
// Sync AP team __MIPS16 define from r34457
#undef __MIPS16
#define __MIPS16
#else
...
int rx_shortcut(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
        ...
        (GetFragNum(pframe) == 0) && (GetMFrag(pframe) == 0))
    {
        ...
        if (pstat->rx_privacy == privacy)
        {
			...
			rx_sum_up(NULL, pstat, pfrinfo->pktlen, 0, 1);
#else
...
#ifdef DRVMAC_LB
#ifdef CONFIG_RTL8672
		rx_sum_up(priv, pstat, pfrinfo->pktlen, 0, pfrinfo->da);
#else
...
out:
	...
	*priv_p = priv;
#ifdef CONFIG_RTL8672
	rx_sum_up(priv, NULL, pfrinfo->pktlen, retry, pfrinfo->da);
#elif defined(TRX_DATA_LOG)
	rx_sum_up(priv, NULL, pfrinfo->pktlen, retry, 0);
#else
...
#ifdef CONFIG_RTL8672
	//rx_sum_up(priv, NULL, pfrinfo->pktlen, retry, pfrinfo->da);
	rx_sum_up(priv, NULL, pktlen, retry, da);
#elif defined(TRX_DATA_LOG)
	rx_sum_up(priv, NULL, pfrinfo->pktlen, retry, (reuse ? 0: 1));	
#else
	rx_sum_up(priv, NULL, pfrinfo->pktlen, retry);
#endif
#ifdef RX_CRC_EXPTIMER		
...
out:
	...
	*priv_p = priv;
	...
	rx_sum_up(priv, NULL, pktlen,retry, da);
#elif defined(TRX_DATA_LOG)
	rx_sum_up(priv, NULL, pfrinfo->pktlen,retry,1);
#else
	rx_sum_up(priv, NULL, pfrinfo->pktlen,retry);
#endif
#ifdef RX_CRC_EXPTIMER		
...
#elif defined(TRX_DATA_LOG)
	rx_sum_up(priv, NULL, pfrinfo->pktlen, GetRetry(pframe),1);
#else
...
void rtl88XX_rx_isr(struct rtl8192cd_priv *priv)
{
	...
	{
		n++;
		if ( (n > (NUM_RX_DESC *2/3)) || ((jiffies - start_time) >= RTL_MILISECONDS_TO_JIFFIES(20) ))
		{
			break;
		}
#ifdef RX_LOOP_LIMIT
	...
	//-------------------------------------------------------------------------------------------
				if(find_flag)
				{
					...
					#endif
#ifdef CONFIG_RTL8672
					rx_sum_up(priv, NULL, pfrinfo->pktlen, GetRetry(get_pframe(pfrinfo)), get_da(get_pframe(pfrinfo)));
#elif defined(TRX_DATA_LOG)
					rx_sum_up(priv, NULL, pfrinfo->pktlen, GetRetry(get_pframe(pfrinfo)), 0);
#else
...
void rtl8192cd_rx_dsr(unsigned long task_priv)
{
...
#endif
	{
		...
		if(GET_CHIP_VER(priv)== VERSION_8723B)
		{
			mask = RTL_R32(REG_8723B_HIMR);
			mask_rx = mask & HIMR_8723B_ROK;		
			RTL_W32(REG_8723B_HIMR, mask & ~mask_rx);
			
			mask_ext = RTL_R32(REG_8723B_HIMRE);
			mask_ext_rx = mask_ext & HIMRE_8723B_RXFOVW;
			RTL_W32(REG_8723B_HIMRE, mask_ext & ~mask_ext_rx);
		}
		else
#endif
		{
...
#endif
	{
		...
		if(GET_CHIP_VER(priv)== VERSION_8723B)
		{
			RTL_W32(REG_8723B_HIMR, RTL_R32(REG_8723B_HIMR) | mask_rx);
			RTL_W32(REG_8723B_HIMRE, RTL_R32(REG_8723B_HIMRE) | mask_ext_rx);
		}
		else
#endif
		{
            ...
            if(pstat->txpause_flag == 1) {
                ...
                if(pstat->txpause_flag){
                    ...
                    if(IS_HAL_CHIP(priv))
                        GET_HAL_INTERFACE(priv)->SetMACIDSleepHandler(priv, 0 , REMAP_AID(pstat));
                    #endif
                    #ifdef CONFIG_RTL_8812_SUPPORT || defined(CONFIG_RTL_8723B_SUPPORT)
                    if (GET_CHIP_VER(priv)==VERSION_8812E || GET_CHIP_VER(priv) == VERSION_8723B)
                        RTL8812_MACID_PAUSE(priv, 0, REMAP_AID(pstat));
                    #endif
                    #ifdef CONFIG_RTL_88E_SUPPORT
