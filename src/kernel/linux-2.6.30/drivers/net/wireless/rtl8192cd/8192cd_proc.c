...
//	if ((GET_CHIP_VER(priv)==VERSION_8192D) && (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G)) {
	...
	PRINT_SINGL_ARG("    TXPowerOffset:     ", priv->pshare->phw->TXPowerOffset, "%lu");
	...
	if (priv->pmib->miscEntry.vap_enable) {
		for (i = 0; i < RTL8192CD_NUM_VWLAN; i++) {
			...
			if (IS_DRV_OPEN(priv_vap)) {
				PRINT_ONE(priv_vap->vap_init_seq, "   [%d]q_num: ", 0);
...
#ifdef MBSSID
		PRINT_ONE(priv_vxd->vap_init_seq, "   [%d]q_num: ", 0);
		PRINT_ONE(priv_vxd->tx_mgnt_queue.q_num, "%d", 0);
#else
		PRINT_ONE(priv_vxd->tx_mgnt_queue.q_num, "   [0]q_num: %d", 0);
#endif
		PRINT_ONE(" tx_pending: ", "%s", 0);
...
#endif
#ifdef BT_COEXIST
	size = rtl8192cd_proc_bt_coexist(buf+len, start, offset, length, eof, data);
	CHECK_LEN;
#endif
...
#endif			
{
	...
	if(pstat->current_tx_rate >= VHT_RATE_ID){
...
#if 1
		PRINT_ONE("    current_tx_rate VHT: VHT NSS", "%s", 0);
        ...
        PRINT_SINGL_ARG("    current_tx_rate: ", rate, "%d");
		CHECK_LEN_B;
#endif		
	}
	else
	...
	if (is_MCS_rate(pstat->current_tx_rate)) {
#if 1		
		PRINT_ONE("    current_tx_rate MCS: MCS", "%s", 0);
        ...
        //rate = (unsigned char *)MCS_DATA_RATEStr[(pstat->ht_current_tx_info&BIT(0))?1:0][(pstat->ht_current_tx_info&BIT(1))?1:0][(pstat->current_tx_rate - HT_RATE_ID)];
        PRINT_SINGL_ARG("    current_tx_rate: ", rate, "%s");
		CHECK_LEN_B;
#endif
...
#endif			
{
    ...
    while (plist != phead) {
        ...
        plist = plist->next;
#ifdef CONFIG_RTL_PROC_NEW
        size = dump_one_sta_dbginfo(s, num++, pstat);
#else
        size = dump_one_sta_dbginfo(num++, pstat, buf+len, start, offset, length, eof, data);
#endif
        CHECK_LEN;
    }
#if defined(__KERNEL__) && !defined(CONFIG_RTL_PROC_NEW)
...
static struct _proc_table_ proc_table[] =
{
    ...
    {"sdio_dbginfo",		rtl8192cd_proc_sdio_dbginfo},
#endif
    ...
    {"flow_stats",					mesh_proc_flow_stats_read},
...
#endif	// _MESH_DEBUG_
#ifdef BT_COEXIST
	{"bt_coexist", rtl8192cd_proc_bt_coexist},
#endif
#ifdef CONFIG_IEEE80211R
...
#endif
    {"thermal",				rtl8192cd_proc_thermal}
};
