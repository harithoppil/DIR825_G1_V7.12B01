...
static unsigned long _ioctl_copy_from_user (struct rtl8192cd_priv *priv, void * to, const void * from, unsigned long n, unsigned long *flags);
static unsigned long _ioctl_copy_to_user (struct rtl8192cd_priv *priv, void * to, const void * from, unsigned long n, unsigned long *flags);
...
#endif
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_WLAN_HAL_8814AE)
...
//	{"antSw_dump",			RFFT_T, _OFFSET_RFFT(antSw_dump), _SIZE_RFFT(antSw_dump), 0},
	...
	{"amsdu_th2",        RFFT_T, _OFFSET_RFFT(amsdu_th2), _SIZE_RFFT(amsdu_th2), 300},
    ...
    {"stactrl_param_1",      INT_T,    _OFFSET(staControl.stactrl_param_1),       _SIZE(staControl.stactrl_param_1), 40}, //rssi thredshold
    ...
    {"stactrl_param_3",      INT_T,    _OFFSET(staControl.stactrl_param_3),       _SIZE(staControl.stactrl_param_3), 2},  //timerX
    {"stactrl_param_4",      INT_T,    _OFFSET(staControl.stactrl_param_4),       _SIZE(staControl.stactrl_param_4), 1}, //retry Y
    {"stactrl_param_5",      INT_T,    _OFFSET(staControl.stactrl_param_5),       _SIZE(staControl.stactrl_param_5), 100}, //timer Z
#endif    
#endif
...
#endif
#endif
	{"pwr_by_rate",        RFFT_T, _OFFSET_RFFT(pwr_by_rate), _SIZE_RFFT(pwr_by_rate), 1},
	...
	{"rssi_min_search",		RFFT_T, _OFFSET_RFFT(rssi_min_search), _SIZE_RFFT(rssi_min_search), 1},
#endif
...
// Tx Power Diff
	...
	{"ldpc_92e", RFFT_T, _OFFSET_RFFT(ldpc_92e), _SIZE_RFFT(ldpc_92e), 1},	
#endif
#ifdef RTK_NL80211
...
//	{"mp_dig_enable",	RFFT_T, _OFFSET_RFFT(mp_dig_enable), _OFFSET_RFFT(mp_dig_enable), 0},
	...
	{"dm_dig_offset", ODM_DM_1UT, _OFFSET_ODM_DM(dm_dig_offset), _SIZE_ODM_DM(dm_dig_offset), 15},	
...
#endif 
#ifdef BT_COEXIST
	{"bt_dump",	RFFT_T, _OFFSET_RFFT(bt_dump), _SIZE_RFFT(bt_dump), 0},
	{"btc", RFFT_T, _OFFSET_RFFT(btc), _SIZE_RFFT(btc), 1},
#endif
};
...
static int get_atm_info(struct rtl8192cd_priv *ppriv)
{
	...
	panic_printk("	atm_chk_txtime=%d atm_chk_hista=%d\n", 
		priv->pshare->rf_ft_var.atm_chk_txtime, priv->pshare->rf_ft_var.atm_chk_hista);
...
#ifdef TXRETRY_CNT	
			panic_printk("	pkt retry ratio:%d%%\n", (pstat->atm_txtetry_ratio>100)?pstat->atm_txtetry_ratio-100:0);
#endif
...
static unsigned long _ioctl_copy_from_user (struct rtl8192cd_priv *priv, void * to, const void * from, unsigned long n, unsigned long *inflags)
{
	unsigned long ret;
...
static unsigned long _ioctl_copy_to_user (struct rtl8192cd_priv *priv, void * to, const void * from, unsigned long n, unsigned long *inflags)
{
	unsigned long ret;
	unsigned long flags = *inflags;
...
#define VERIFY_WRITE 1
			...
			wrq->u.data.length = sizeof(privtab) / sizeof(privtab[0]);
				...
				if (copy_to_user(&wrq->u.data.length, &dtblSize, sizeof(__u16)))
				{
