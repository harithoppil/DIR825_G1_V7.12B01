...
int PHY_ConfigTXLmtWithParaFile_new(struct rtl8192cd_priv *priv)
{
	...
	unsigned char table_index[24]={0,1,2,3,4,14,15,5,6,16,17,7,8,9,18,19,10,11,20,21,12,13,22,23};
	...
	if(GET_CHIP_VER(priv) == VERSION_8814A){
		int i;
		for(i=BAND2G_20M_1T_CCK; i<=BAND5G_80M_4T_VHT; i++){
			table_index[i] = i;
		}
	}
#endif
	...
	while (1) {
		...
		if(channel > 0)
		{
			...
			if(priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G)
			if ((table_idx >= table_index[BAND2G_20M_1T_CCK]) && (table_idx <= table_index[BAND2G_40M_4T_HT])){
			...
			if ( (table_idx >= table_index[BAND5G_20M_1T_OFDM]) && (table_idx <= table_index[BAND5G_80M_4T_VHT])){
				
				int j = ch2idx(channel);
				
				if (table_idx == table_index[BAND5G_20M_1T_OFDM]){
					priv->pshare->ch_pwr_lmtOFDM[j] = limit;
...
#endif
void turnoff_hw_LED(struct rtl8192cd_priv *priv, unsigned int led_type)
{
...
#endif
printk("turnoff_hw_LED() priv->dev->name=%s,LED_TYPE=%d\n",priv->dev->name,LED_TYPE);
...
#ifdef CONFIG_RTL_92D_SUPPORT
		if (GET_CHIP_VER(priv) == VERSION_8192D)
			RTL_W32(LEDCFG, LED_OFF << LED1CM_SHIFT_92D | LED1DIS_92D);
#endif
#ifdef CONFIG_RTL_92C_SUPPORT
		if ((GET_CHIP_VER(priv) == VERSION_8192C) || (GET_CHIP_VER(priv) == VERSION_8188C))
...
#endif
#ifdef CONFIG_RTL_8812_SUPPORT
		printk("disable_sw_LED(VERSION_8812E) priv->dev->name=%s,LED_TYPE=%d\n",priv->dev->name,LED_TYPE);

		if (GET_CHIP_VER(priv) == VERSION_8812E)
			RTL_W32(LEDCFG, (RTL_R32(LEDCFG) & 0xffff00ff) | BIT(13) | (LED_OFF << LED1CM_SHIFT));
#endif
		break;
	case LEDTYPE_HW_LINKACT_INFRA:
		RTL_W32(LEDCFG, LED_OFF << LED0CM_SHIFT);
...
#if defined(CONFIG_RTL_92C_SUPPORT) || defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_88E_SUPPORT) || defined(CONFIG_WLAN_HAL_8192EE)
	if (GET_CHIP_VER(priv) == VERSION_8188E || GET_CHIP_VER(priv) == VERSION_8192C || GET_CHIP_VER(priv) == VERSION_8192D|| GET_CHIP_VER(priv) == VERSION_8192E) {
...
//	Path A only
			...
			/* 
			*  Do LCK for 8814 MP chip
			*/	
#if 1
#ifdef CONFIG_WLAN_HAL_8814AE			
			printk("Doing LCK: PHY_LCCalibrate_8814A\n");
			phy_LCCalibrate_8814A(ODMPTR, TRUE);
			priv->pshare->ThermalValue_LCK = (unsigned char)PHY_QueryRFReg(priv, RF_PATH_A, 0x42, 0xfc00, 1); /*set LCK base thermal as current thermal*/
#endif			
#else
			printk("Disable LCK due to RX hang in normal driver\n");
#endif
			}
...
#elif defined(CONFIG_SDIO_HCI)
			PHY_SetBBReg(priv,0x4c,0xffffffff,0x70628282);
#endif
...
//					RESTORE_INT(x);
...
#if defined(CONFIG_PCI_HCI) && defined(USE_OUT_SRC)
	if ( IS_OUTSRC_CHIP(priv) ) {
		if ( ODMPTR->Carrier_Sense_enable ) {
#ifdef  CONFIG_WLAN_HAL
			if (IS_HAL_CHIP(priv)) {		
				// Disable BCN when carrier sense
				unsigned char H2CCommand[1]={0};
				H2CCommand[0] = 0;
				GET_HAL_INTERFACE(priv)->FillH2CCmdHandler(priv, H2C_88XX_BCN_IGNORE_EDCCA, 1, H2CCommand);
			}
	...
	if ( ODMPTR->Carrier_Sense_enable ) {
#ifdef CONFIG_WLAN_HAL_8192EE
	   unsigned char H2CCommand[1] = {0x00};
	   printk("set bcn care edcca\n");
	   GET_HAL_INTERFACE(priv)->FillH2CCmdHandler(priv, H2C_88XX_BCN_IGNORE_EDCCA, 1, H2CCommand);
#endif
...
#if defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8814AE)
...
static void ReadEFuse(struct rtl8192cd_priv *priv, unsigned short _offset, int _size_byte, unsigned char *pbuf)
{
	...
	UINT8 bSupportExtendHdr = ((GET_CHIP_VER(priv) == VERSION_8192D) || (GET_CHIP_VER(priv) == VERSION_8188E)
			|| (GET_CHIP_VER(priv) == VERSION_8192E) || (GET_CHIP_VER(priv) == VERSION_8812E) || (GET_CHIP_VER(priv) == VERSION_8814A));
#endif
...
void ReadMacAddressFromEfuse(struct rtl8192cd_priv *priv)
{
	...
	if (GET_CHIP_VER(priv) == VERSION_8188E) {
	#ifdef CONFIG_PCI_HCI
		efuse_MAC = EEPROM_MAC_ADDR_88EE;
	#elif defined(CONFIG_USB_HCI)
		efuse_MAC = EEPROM_MAC_ADDR_88EU;
	#elif defined(CONFIG_SDIO_HCI)
		efuse_MAC = EEPROM_MAC_ADDR_88ES;
	#endif
	}
#endif // CONFIG_RTL_88E_SUPPORT
...
void ReadThermalMeterFromEfuse(struct rtl8192cd_priv *priv)
{
	...
	if ((GET_CHIP_VER(priv) == VERSION_8188E) || (GET_CHIP_VER(priv) == VERSION_8812E) || (GET_CHIP_VER(priv) == VERSION_8192E) || (GET_CHIP_VER(priv) == VERSION_8814A)) {
		if ((priv->pmib->dot11RFEntry.ther < 0x07) || (priv->pmib->dot11RFEntry.ther > 0x32)) {
			DEBUG_ERR("TPT: unreasonable target ther %d, disable tpt\n", priv->pmib->dot11RFEntry.ther);
			priv->pmib->dot11RFEntry.ther = 0;
		}
...
void ReadLNATypeFromEfuse(struct rtl8192cd_priv *priv)
{
    ...
    if((priv->pshare->rf_ft_var.lna_type!=LNA_TYPE_0) && (priv->pshare->rf_ft_var.lna_type!=LNA_TYPE_1) && (priv->pshare->rf_ft_var.lna_type!=LNA_TYPE_2) && (priv->pshare->rf_ft_var.lna_type!=LNA_TYPE_3)){
...
static int getEEPROMOffset(struct rtl8192cd_priv *priv, int type)
{
	...
	if (GET_CHIP_VER(priv) == VERSION_8188E) {
		...
		case 2:
			...
			break;
		case 3:
			offset = EEPROM_MAC_ADDR_88EE; // HW_WLAN0_WLAN_ADDR
			break;
		case 4:
			offset = 0x00;
			break;
...
static UINT16 efuse_GetCurrentSize(struct rtl8192cd_priv *priv)
{
	...
	UINT8 bSupportExtendHdr = ((GET_CHIP_VER(priv) == VERSION_8192D) || (GET_CHIP_VER(priv) == VERSION_8188E) || (GET_CHIP_VER(priv) == VERSION_8192E)
			|| (GET_CHIP_VER(priv) == VERSION_8812E) || (GET_CHIP_VER(priv) == VERSION_8814A));
#endif
...
static void shadowMapWrite(struct rtl8192cd_priv *priv, int type, char *value, unsigned char *hwinfo)
{
	...
	if(GET_CHIP_VER(priv) == VERSION_8188E){
		...
		}else if (offset == EEPROM_MAC_ADDR_88EE) {
			for (i = 0; i < MACADDRLEN; i++) {
				get_array_val(hwinfo + offset + i, value + i * 2, 2);
			}
		...
		else if(offset==EEPROM_RFE_OPTION_8192E)
		{
				...
				hwinfo[offset] = (RFE_value << 4); //0xCA[6:4] : LNA TYPE
                        ...
                        if ( *value == '2' ) {
                              ...
                              hwinfo[offset+2] = 0x02;
                              hwinfo[offset+3] = 0x23;
                              hwinfo[offset+4] = 0x00;
                              hwinfo[offset+5] = 0x00;
                        } else if ( *value == '3' ) {
                              hwinfo[offset]   = 0x6F;
                              hwinfo[offset+1] = 0x51;
                              hwinfo[offset+2] = 0x03;
                              hwinfo[offset+3] = 0x34;
                              hwinfo[offset+4] = 0xFF;
...
#ifdef CONFIG_RTL_88E_SUPPORT
	if(GET_CHIP_VER(priv) == VERSION_8188E){
		if (offset == EEPROM_MAC_ADDR_88EE) {
			for (i = 0; i < MACADDRLEN; i++) {
				sprintf(out + len, "%02x", hwinfo[offset + i]);
				len += 2;
			}
