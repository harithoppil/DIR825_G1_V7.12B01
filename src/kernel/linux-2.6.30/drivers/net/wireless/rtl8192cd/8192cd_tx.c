...
static void rtl8192cd_tx_get_da(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info *pstat, struct tx_insn *txcfg, unsigned char *buf)
{
	struct wlan_ethhdr_t *pethhdr;
	unsigned char * da;
	pethhdr = (struct wlan_ethhdr_t *)(skb->data - WLAN_ETHHDR_LEN);
	...
	if( isSpecialFloodMac(priv,skb) || IS_MDNSV4_MAC(skb->data)||IS_MDNSV6_MAC(skb->data)||IS_IGMP_PROTO(skb->data) || isICMPv6Mng(skb) || IS_ICMPV6_PROTO(skb->data)|| isMDNS(skb->data))
	{
		...
		while ((plist = asoc_list_get_next(priv, plist)) != phead) {
			...
			if ((privacy == _NO_PRIVACY_ || (privacy == _CCMP_PRIVACY_
				...
				&& !UseSwCrypto(priv, pstat, (pstat ? FALSE : TRUE),0)))
#else
				&& !UseSwCrypto(priv, pstat, (pstat ? FALSE : TRUE))))
#endif
		...
		if (
			...
			if (txcfg->pstat->ADDBA_ready[TID] && !txcfg->pstat->low_tp_disable_ampdu) {
				...
				||((txcfg->pstat->current_tx_rate >= _NSS2_MCS0_RATE_) && (txcfg->pstat->current_tx_rate <= _NSS2_MCS2_RATE_))
					...
					pdesc->Dword3 |= set_desc( (0x04 & TXdesc_92E_MAX_AGG_NUMMask)<< TXdesc_92E_MAX_AGG_NUMSHIFT);		// MAX_AGG_NUM=8
#endif
			}
		}
...
#endif
#if 0
		else if (txcfg->pstat->rssi < 30 || priv->pshare->total_assoc_num > 10) {
			pdesc->Dword4 |= set_desc(TXdesc_92E_RtyLmtEn);
...
#if defined(CONFIG_WLAN_HAL_8192EE) && defined(MULTI_STA_REFINE)
			else if((GET_CHIP_VER(priv) == VERSION_8192E) && (priv->pshare->total_assoc_num > 30)) {
	            pdesc_data->rtyLmtEn = TRUE;
...
#endif
#if defined(CONFIG_WLAN_HAL_8192EE) && defined(MULTI_STA_REFINE)
		if((GET_CHIP_VER(priv) == VERSION_8192E) && (priv->pshare->total_assoc_num > 10) 
...
#endif
			{
				...
				unsigned int wispWlanIndex=(passThruStatusWlan&WISP_WLAN_IDX_MASK)>>WISP_WLAN_IDX_RIGHT_SHIFT;
				  	 ...
				  	 (((GET_MIB(wlan_device[wispWlanIndex].priv))->dot11OperationEntry.opmode) & WIFI_STATION_STATE))
				  {
					...
					#else	
						dev=skb->dev=wlan_device[passThruWanIdx].priv->dev;
					#endif
				  } else {
					goto free_and_stop;
				  }
...
#endif
{
	...
	if (amsdu && pstat && (pstat->aggre_mthd & AGGRE_MTHD_MSDU) && (pstat->amsdu_level > 0)
#ifdef SUPPORT_TX_MCAST2UNI
...
//#ifdef __KERNEL__
            ...
            if(
#ifdef CONFIG_WLAN_HAL
				IS_HAL_CHIP(priv) ? (compareAvailableTXBD(priv, (uiCurr_tx_desc/2), q_num, 2)) :
#endif // CONFIG_WLAN_HAL
...
//#endif
...
#endif // CONFIG_WLAN_HAL
                	(((GET_TX_EARLY_MODE && GET_EM_SWQ_ENABLE) ? 14 : 4) > CIRC_SPACE_RTK(*tx_head, *tx_tail, uiCurr_tx_desc))
            )
#else
			if(priv->pshare->rf_ft_var.lmtdesc) {
				if (GET_CHIP_VER(priv)==VERSION_8814A) {				
					if (priv->pshare->phw->VO_pkt_count) {
					...
					else if (priv->pshare->phw->VI_pkt_count) {
						if(q_num == BE_QUEUE || q_num == BK_QUEUE)
							needTxBD += priv->pshare->rf_ft_var.lmt1;		
						if(q_num == BK_QUEUE)
							needTxBD += priv->pshare->rf_ft_var.lmt2;		
					} else if (priv->pshare->phw->BE_pkt_count) {
						if(q_num == BK_QUEUE)
							needTxBD += priv->pshare->rf_ft_var.lmt1;		
					}
				} else {
			...
			if(
#ifdef CONFIG_WLAN_HAL
            IS_HAL_CHIP(priv) ? (compareAvailableTXBD(priv, needTxBD, q_num, 2)) : 
#endif // CONFIG_WLAN_HAL			
#ifdef TX_SCATTER
...
#ifdef CONFIG_RTL8672
			unsigned char da[6];
			rtl8192cd_tx_get_da(priv, skb, pstat, txcfg, da);
			tx_sum_up(priv, pstat, txcfg->fr_len+txcfg->hdr_len+txcfg->iv+txcfg->llc+txcfg->mic+txcfg->icv, txcfg, da);
	...
	if (priv->pshare->rf_ft_var.tcpack_agg && pstat && (pstat->state & WIFI_ASOC_STATE)) {
		if (*(u16 *)(skb->data + ETH_ALEN*2) == htons(ETH_P_IP)) {
			...
			if (IPPROTO_TCP == iph->protocol) {	// 6
				...
				// only handle TCP ACK(no TCP data) packet
...
//one_txdesc:
	...
	if (GET_CHIP_VER(priv)==VERSION_8814A) {
        ...
        desc_data.rateId = txcfg->pstat->ratr_idx;
        desc_data.dataBW = priv->pshare->is_40m_bw;
    }
#endif // WLAN_HAL_HW_TX_SHORTCUT_REUSE_TXDESC
#ifdef WLAN_HAL_HW_TX_SHORTCUT_HDR_CONV
...
//		}
...
#endif
#ifdef OSK_LOW_TX_DESC
			((val32 + 2) > CIRC_SPACE_RTK(*tx_head, *tx_tail, ((q_num==BE_QUEUE || q_num==HIGH_QUEUE)?BE_TXDESC:NONBE_TXDESC))) //per mpdu, we need 2 desc...
#else
		((val32 + 2) > CIRC_SPACE_RTK(*tx_head, *tx_tail, CURRENT_NUM_TX_DESC)) //per mpdu, we need 2 desc...
			 ...
			 // 2 is for spare...
#ifdef USE_TXQUEUE
			if (txcfg->fr_type == _SKB_FRAME_TYPE_ && GET_ROOT(priv)->pmib->miscEntry.use_txq && priv->pshare->iot_mode_enable) {
				if (priv->pshare->txq_isr) {
					append_skb_to_txq_head(&priv->pshare->txq_list[q_num], priv, txcfg->pframe, priv->dev, &priv->pshare->txq_pool);
					priv->pshare->txq_stop = 1;
				} else {
...
#endif
		da = get_da((unsigned char *)txcfg->phdr);
	tx_sum_up(priv, txcfg->pstat, txcfg->fr_len+txcfg->hdr_len+txcfg->iv+txcfg->llc+txcfg->mic+txcfg->icv, txcfg, pethhdr ? pethhdr->daddr : da);
#elif defined(TRX_DATA_LOG)
