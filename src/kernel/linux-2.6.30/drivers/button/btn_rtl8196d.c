...
/* 定义在rtl8196d_product.c中 */
extern struct btn_dev rtl8196d_btns[];
...
/* 核心驱动通过此函数获得按钮状态 */
static btn_status rtl8196d_get_status(struct btn_dev *btn)
{
	int ret;
...
static int __init rtl8196d_btn_init(void)
{
	int ret;
	int i;

	for(i=0;rtl8196d_btns[i].name != btn_end; i++)
	{
		/*设置为GPIO模式*/
		gpioConfigCNR(rtl8196d_btns[i].gpio, 0);

		/* 初始化GPIO接口的工作状态，设置为输入模式，避免误触发 */
		gpioConfig(rtl8196d_btns[i].gpio, GPIO_FUNC_INPUT);

		/* 设置获取按钮状态句柄 */
		rtl8196d_btns[i].get_status = rtl8196d_get_status;

		ret = btn_dev_register(&rtl8196d_btns[i]);
		if (ret != 0) {
			printk (KERN_ERR "Unable to register btn_dev %d(error %d)\n", rtl8196d_btns[i].name, ret);
			return -1;
		}
	}
...
static void __exit rtl8196d_btn_exit(void)
{
    int i;
    ...
    for(i=0;rtl8196d_btns[i].name != btn_end; i++)
	{
		btn_dev_unregister(&rtl8196d_btns[i]);
	}
}
...
void rtl8196d_button_irq(unsigned long data)
{
...
static irqreturn_t rtl8196d_gpio_irq_dispatch(int cpl, void *dev_id)
{
	int i;
	...
	if((int)dev_id == GPIO_ABCD_IRQ)
	{
		for(i=0;rtl8196d_btns[i].name != btn_end;i++)
		{
			if( gpioGetIntrStatus(rtl8196d_btns[i].gpio))
			{
                          ...
                          REG32(BSP_GIMR), REG32(GPIO_PABCD_CNR), REG32(GPIO_PABCD_PTYPE), REG32(GPIO_PABCD_DIR),
                          REG32(GPIO_PABCD_DAT), REG32(GPIO_PABCD_ISR), REG32(GPIO_PAB_IMR), REG32(GPIO_PCD_IMR),
                          rtl8196d_btns[i].gpio, gpioRead(rtl8196d_btns[i].gpio)
                          );

                          gpioClearIntr(rtl8196d_btns[i].gpio);
                          rtl8196d_button_irq((unsigned long)(&rtl8196d_btns[i]));  /* 对触发中断的GPIO进行处理 */
			}
		}
...
/* 核心驱动通过此函数获得按钮状态 */
btn_status rtl8196d_get_status(struct btn_dev *btn)
{
...
static int __init rtl8196d_btn_init(void)
{
	...
	for(i=0;rtl8196d_btns[i].name != btn_end; i++)
    {
        /* 设置指定的GPIO为GPIO模式 */
		gpioConfigCNR(rtl8196d_btns[i].gpio, 0);
        printk("---rtl8196d_btns[%d].gpio-%d-\n",i,rtl8196d_btns[i].gpio);
		/* 设置指定的GPIO为输入模式 */
		gpioConfig(rtl8196d_btns[i].gpio, GPIO_FUNC_INPUT);

        #if 0
        /* 设置指定的GPIO的中断触发模式为按下时触发(下降沿触发) */
        gpioConfigEdge(rtl8196d_btns[i].gpio, GET_EDGE(rtl8196d_btns[i].level));
        #endif
        /*
        if(16 == rtl8196d_btns[i].gpio)
        {
            REG32(0xB8000040) |=0x3000;
		}
		*/
		/*JTAG引脚复用*/
        if(5 == rtl8196d_btns[i].gpio)
        {
            REG32(0xB8000040) &= ~0x7;
            REG32(0xB8000040) |=0x6;
        }
        /* 设置指定的GPIO的中断触发模式为双向触发 */
        gpioConfigEdge(rtl8196d_btns[i].gpio, GPIO_EDGE_BOTH);

		/* 初始各个BTN设备的初始状态 */
		rtl8196d_btns[i].ignore = 0;
		rtl8196d_btns[i].cur_status = BTN_UP;
		rtl8196d_btns[i].get_status = rtl8196d_get_status;
	...
	ret = request_irq (GPIO_ABCD_IRQ, rtl8196d_gpio_irq_dispatch, 0, "GPIO_ABCD", (void *)GPIO_ABCD_IRQ);
	if (ret != 0)
	{
...
module_init(rtl8196d_btn_init);
module_exit(rtl8196d_btn_exit);

MODULE_AUTHOR("KuangSuwen");
MODULE_DESCRIPTION("Rtl8196d button driver");
MODULE_LICENSE("GPL");
