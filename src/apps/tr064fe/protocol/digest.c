...
static char *getArg(char *p, char *argname, char **argval)
{
   ...
   *argval = NULL;
   do {
      if ((s=(char*)strcasestr((const char*)s, (const char*)argname)) != NULL ) {
         if ( isalpha(*(s-1))) {
            s += nameLth;
            continue;
         }
         ...
         if ( *s == '='){
            ...
            if (lth)
               *argval = (char *)strndup((const char*)s,lth);
            return *argval;
         }
...
static void generateRequestDigest( sessionAuth *sa, char *user, char* pwd)
{
   ...
   snprintf(md5inbuf, sizeof((char*)md5inbuf),"%s:%s", sa->method, sa->uri);
   tr64_md5it((unsigned char *)md5buf, (const unsigned char *)md5inbuf);
...
static int parseAuthorizationHdr(char *ahdr, sessionAuth *sa, char *username, char *password)
{
   ...
   if ( ahdr && ((p=(char*)strcasestr((const char*)ahdr, (const char*)"Digest")) != NULL)) {
      free(sa->realm);
