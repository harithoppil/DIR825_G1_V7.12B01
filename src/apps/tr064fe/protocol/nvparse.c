...
get_autofw_port(int which, netconf_app_t *app)
{
	...
	if (!nvram_invmatch(name, ""))
		return FALSE;
	strncpy(value, _nvram_get(name), sizeof(value));
...
del_autofw_port(int which)
{
	...
	snprintf(name, sizeof(name), "autofw_port%d", which);
	return (_nvram_unset(name) == 0) ? TRUE : FALSE;
}
...
get_forward_port(int which, netconf_nat_t *nat)
{
	...
	strncpy(nat->match.in.name, nvram_safe_get("wan_ifname"), IFNAMSIZ);
#endif
...
convert_filter_port(void)
{
	...
	for (i = 1; i <= 6; i++) {
		...
		if (!(value = _nvram_get(name)) || !*value)
			goto fail;
...
convert_maclist(char *prefix)
{
	...
	if ((value = _nvram_get(mac))) {
		_nvram_set(maclist, value);
		/* An empty *_mac used to imply *_macmode=disabled */
		if (!*value)
			_nvram_set(macmode, "disabled");
		_nvram_unset(mac);
	}
	nvram_commit();
}
...
convert_pppoe(void)
{
	...
	for (i = 0; old[i] != NULL; i ++) {
		if ((value = _nvram_get(old[i]))) {
			snprintf(new, sizeof(new), "wan_%s", old[i]);
			_nvram_set(new, value);
			_nvram_unset(old[i]);
		}
	}
	nvram_commit();
...
convert_static_route(void)
{
	...
	char lan_route[1000] = "", wan_route[1000] = "";
	char *lan_cur = lan_route, *wan_cur = wan_route;
	...
	if (lan_cur != lan_route)
		_nvram_set("lan_route", lan_route);

	if (wan_cur != wan_route)
		_nvram_set("wan_route", wan_route);

	_nvram_unset("static_route");
	nvram_commit();
}
...
convert_deprecated(void)
{
	...
	/* PR12761 WAR: meaning of empty filter_mac and wl_mac changed */
	...
	if (nvram_invmatch("pppoe_ifname", "") && nvram_match("wan_proto", "pppoe"))
		_nvram_set("wan_ifname", _nvram_get("pppoe_ifname"));
	_nvram_unset("pppoe_ifname");
