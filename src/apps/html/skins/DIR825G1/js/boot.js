        ...
        for(var j in json)
		{
                if(typeof(j)=='string')
				{
                        ...
                        }else if(typeof(json[j])=='object')
						{
								...
								{
                                        if(typeof(json[j][i])=='number')
                                                var y = 1;
                                                var x = json[j][i];break;
                                }
                        }
                        if(y==1){$(j).parentNode.style.display = (x==1?'':'none');}else{
                                         $(j).style.display = (x==1?'':'none');}
...
var Table = {
	...
	Create_ddns: function(Node,Value,index){ // 传入的Value必须是符合要求的数组
		var Table = $(Node);
		var Tbody = Table.getElementsByTagName('tbody')[0];
		//如果没有Tbody,就创建一个; 如果有,就共用一个;
		if(Tbody == null){
		...
		for(var i = 0; i < Value.length; i++){
			...
			for(var j = 0; j < Value[i].length; j++){
				...
				Td[j].innerHTML = Value[i][j];
				Td[j].id        = Node + "_" + index + j.toString();
				Tr[i].appendChild(Td[j]);
			}
			Tbody.appendChild(Tr[i]);
		}
