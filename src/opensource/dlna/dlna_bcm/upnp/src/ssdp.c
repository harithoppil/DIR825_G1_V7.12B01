...
//        if (!IMATCH_PREFIX(dev, "::upnp:rootdevice"))
        if (!strstr(dev, "::device:ManageableDevice"))
            return;

/* modify by tuhanyu, for upnp device management of separate ap, 2011/04/15 */
#ifdef DMP_X_CT_COM_UPNP_DM_CP_1
        BUPnPControlPoint_AddDevice(uuid, nMaxAge, location, hContext);
#else
...
static void Ssdp_ProcessMSearchResponse( HttpContextHandle hContext )
{
    ...
    usn = Http_GetHeaderValue(hContext, "USN");
    dev = usn?Ssdp_ParseUuid(usn, uuid):NULL;
...
static const char* Ssdp_ParseUuid(const char* usn, char* uuid)
{
    ...
    if(strchr(usn, '[') != NULL && strchr(usn, ']') != NULL)
    {
        return NULL;
    }
