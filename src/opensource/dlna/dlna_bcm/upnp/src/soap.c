...
/* Add by tuhanyu, for upnp device management of separate, 2011/04/15, start */
#ifdef DMP_X_CT_COM_UPNP_DM_CP_1
            char *body = Http_GetBody(&context);
            char *temp, *value;
            if(body && (value = strstr(body, "<errorCode>")) && (temp = strstr(body, "</errorCode>")))
            {
                value += 11;
                *temp = 0;
                result = atoi(value);;
