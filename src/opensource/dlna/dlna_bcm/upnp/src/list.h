...
#define BLST_DICT_FIND_BY_STR(head, elm, key, field, link) do { \
    for((elm) = BLST_FIRST(head); (elm)!=NULL ; (elm) = BLST_NEXT((elm), link)) { \
    if(0 == cmsUtl_strcmp(key, (elm)->field)) { break; } \
        } } while(0)

#define BLST_DICT_FIND_BY_PARAM(head, elm, key, link) do { \
    for((elm) = BLST_FIRST(head); (elm)!=NULL ; (elm) = BLST_NEXT((elm), link)) { \
    if(0 == cmsUtl_strcmp(key->productClass, (elm)->deviceInfo->productClass)  && \
        0 == cmsUtl_strcmp(key->manufacturerOUI, (elm)->deviceInfo->manufacturerOUI) && \
        0 == cmsUtl_strcmp(key->serialNumber, (elm)->deviceInfo->serialNumberDev)) { break; } \
        } } while(0)
#else
#define BLST_DICT_FIND_BY_STR(head, elm, key, field, link) do { \
    for((elm) = BLST_FIRST(head); (elm)!=NULL ; (elm) = BLST_NEXT((elm), link)) { \
