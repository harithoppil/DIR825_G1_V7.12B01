...
/*
 * TBS_TAG:add by pengyao 20100801
...
" --log-ip-options		Log IP options.\n\n"
" --log-uid			Log UID owning the local socket.\n\n"
" --log-sysevent			Log sysenvent for tbs.\n\n");
}
...
static const struct option LOG_opts[] = {
	...
	{ .name = "log-ip-options",   .has_arg = 0, .val = '3' },
	{ .name = "log-uid",          .has_arg = 0, .val = '4' },
    { .name = "log-sysevent",     .has_arg = 1, .val = '5' },
	{ .name = NULL }
};

#define LOG_EX_EMERG    128
#define LOG_EX_ALERT    129
#define LOG_EX_CRIT     130
#define LOG_EX_ERR      131
#define LOG_EX_WARNING  132
#define LOG_EX_NOTICE   133
#define LOG_EX_INFO     134
#define LOG_EX_DEBUG    135
#endif
/*
 * TBS_TAG:end
*/
...
= { { .name = "alert",   .level = LOG_ALERT },
    ...
    { .name = "ex_alert",   .level = LOG_EX_ALERT },
    { .name = "ex_crit",    .level = LOG_EX_CRIT },
    { .name = "ex_debug",   .level = LOG_EX_DEBUG },
    { .name = "ex_emerg",   .level = LOG_EX_EMERG },
    { .name = "ex_error",   .level = LOG_EX_ERR },		/* DEPRECATED */
    { .name = "ex_info",    .level = LOG_EX_INFO },
    { .name = "ex_notice",  .level = LOG_EX_NOTICE },
    { .name = "ex_warning", .level = LOG_EX_WARNING }
};
...
*/
    ...
    if(!xtables_strtoui(level, NULL, &lev, 0, LOG_EX_DEBUG)) {
		unsigned int i = 0;
    ...
    if(!((lev >= 0 && lev <= 7 ) ||
        (lev >= LOG_EX_EMERG && lev <= LOG_EX_DEBUG))) {
        xtables_error(PARAMETER_PROBLEM,
               "log-level `%s' unknown", level);
...
* Desc:Add logsysevent support for tbs
*/
#define IPT_LOG_OPT_SYSEVENT 0x40
/*
 * TBS_TAG:end
*/
                     ...
                     const void *entry, struct xt_entry_target **target)
{
    ...
    */
    case '5':
        if (*flags & IPT_LOG_OPT_SYSEVENT)
			xtables_error(PARAMETER_PROBLEM,
				   "Can't specify --log-sysevent twice");

		if (strlen(optarg) > sizeof(loginfo->logsysevent) - 1)
			xtables_error(PARAMETER_PROBLEM,
				   "Maximum sysevent length %u for --log-sysevent",
				   (unsigned int)sizeof(loginfo->logsysevent) - 1);

        strcpy(loginfo->logsysevent, optarg);
		loginfo->logflags |= IPT_LOG_SYSEVENT;
		*flags |= IPT_LOG_OPT_SYSEVENT;
                      ...
                      int numeric)
{
	...
	if (numeric)
        ...
        * Desc:Add logsysevent support for tbs
        */
		if (loginfo->logflags & IPT_LOG_SYSEVENT)
			printf("sysevent: %s ", loginfo->logsysevent);
        /*
         * TBS_TAG:end
        */
...
static void LOG_save(const void *ip, const struct xt_entry_target *target)
{
    ...
    */
    if (loginfo->logflags & IPT_LOG_SYSEVENT) {
        printf("--log-sysevent ");
        xtables_save_string(loginfo->logsysevent);
    }
    /*
     * TBS_TAG:end
    */
