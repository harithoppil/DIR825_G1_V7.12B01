...
//TBS TAG : begin (20110830)
#define AUTH_STR_LEN 64
char username[AUTH_STR_LEN] = {0};
char password[AUTH_STR_LEN] = {0};
//TBS TAG : end
...
//TBS TAG : begin (20110830)
                else if (option_index == 15) { /* --user */
		    snprintf(username, AUTH_STR_LEN, "%s", optarg);
                }
                else if (option_index == 16) { /* --password */
		    snprintf(password, AUTH_STR_LEN, "%s", optarg);
                }
//TBS TAG : end
                break;
...
void launch_pppd(char *ttydev, int argc, char **argv)
{
    char *new_argv[argc + 7];/* XXX if not using GCC, hard code a limit here. */
    int i = 0, j;
