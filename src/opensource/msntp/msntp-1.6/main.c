    ...
    if (initial) {
        ...
        if (verbose > 2)
            fprintf(stderr,"prev=%.6f when=%.6f retry=%d\n",
                previous,when,retry);
        for (i = 0; i < nhosts; ++i)
			    open_socket(i,hostnames[i],delay);
...
triggered if the signal handling works. */
    ...
    g_avaliableNtpServer = 0;
	fprintf(stderr,"run_client(1)==>g_avaliableNtpServer=%d!!\n",g_avaliableNtpServer);
    for (k = 0; k < nhosts; ++k) 
		open_socket(k,hostnames[k],delay);
	fprintf(stderr,"run_client(2)==>g_avaliableNtpServer=%d!!\n",g_avaliableNtpServer);
	if(0 == g_avaliableNtpServer)
	{
	    fprintf(stderr,"no avaliable ntp server,exit!!\n");
		exit(EXIT_SUCCESS);
	}
