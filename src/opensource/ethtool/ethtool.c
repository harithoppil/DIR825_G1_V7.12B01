...
static int do_rmark(int fd, struct ifreq *ifr);
...
static struct option {
    ...
    { "-Q", "--qinq", MODE_SQINQ, "Configure QinQ",
		"		[ enable | disable ]\n"
		"		[ stag VID ]\n" },
    { "-M", "--rmark", MODE_RMARK, "Set/Show Route Mark in Netdevice",
		"		[ rmark ]\n"},
    {}
};
...
static u32 rmark_type = 0; /* 0 - get rmark, 1 - set rmark */
static u32 route_mark = 0;
static enum {
	ONLINE=0,
	OFFLINE,
} test_type = OFFLINE;
		...
		case 3:
			if (mode == MODE_TEST) {
				...
				if (phys_id_time < 0)
					show_usage(1);
				break;
			} else if (mode == MODE_RMARK) {
			    rmark_type = 1;
			    route_mark = strtol(argp[i], NULL, 0);
                break;
			}
			/* fallthrough */
		default:
...
/*
 * TBS_TAG: by yuanke at 2010-05-20
 * Desc:Add for policy route support
 */
...
static int do_rmark(int fd, struct ifreq *ifr)
{
    int markxx;
    int err;
    struct ethtool_rmark ermark;
    memset(&ermark, 0, sizeof(ermark));

    ermark.cmd  = rmark_type ? ETHTOOL_SRMARK : ETHTOOL_GRMARK;
    ermark.mark = route_mark;

    ifr->ifr_data = (caddr_t)&ermark;
    err = ioctl(fd, SIOCETHTOOL, ifr);

    if (err < 0){
        perror("Cannot set/get route mark");
        return err;
    }

    if(ermark.cmd == ETHTOOL_GRMARK){
		fprintf(stdout,
            "Device:		    %s\n"
    		"Route Mark:		%u\n",
    		devname, ermark.mark);
