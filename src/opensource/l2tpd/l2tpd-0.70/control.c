 ...
 * don't ask for sequencing */
        ...
        log (LOG_LOG, "%s[%d]: start_pppd,ICRP,%d,%s\n",
			__FUNCTION__, __LINE__,ICRP,"Incoming-Call-Reply");
        start_pppd (c, po);
        opt_destroy (po);
        if (c->lac)
            c->lac->rtries = 0;
    ...
    case ICCN:
        ...
        log (LOG_LOG, "%s[%d]: start_pppd,ICCN,%d,%s\n",
			__FUNCTION__, __LINE__,ICCN,"Incoming-Call-Connected");
        start_pppd (c, po);
        opt_destroy (po);
        log (LOG_LOG,
             "%s: Call established with %s, Local: %d, Remote: %d, Serial: %d\n",
    ...
    case OCCN:                 /* jz: get OCCN, so the only thing we must do is to start the pppd */
        ...
        log (LOG_LOG, "%s[%d]: start_pppd,OCCN,%d,%s\n",
			__FUNCTION__, __LINE__,OCCN,"Outgoing-Call-Connected");
        start_pppd (c, po);
