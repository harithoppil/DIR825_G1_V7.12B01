...
/*
 * TBS_TAG: by pengyao 2012-11-01
 * Desc: Added for tcwatchdog
 */
#include "autoconf.h"
/*
 * TBS_END_TAG
...
//#ifndef CONFIG_RALINK_TCWATCHDOG
	...
	sync();
        ...
        /*
          因MTK提供的2.6.36 kernel还存在缺陷kill所有进程可能会hang住
          注掉下面sleep语句可以规避该问题
          add by pengyao 20130227
        */
        //sleep(1);
	...
	sync();
        ...
        /*
          因MTK提供的2.6.36 kernel还存在缺陷kill所有进程可能会hang住
          这里先屏蔽kill所有进程，在内核reboot处理后通过tcwatchdog来重启
          add by pengyao 20121101
        */
        /* Send signals to every process _except_ pid 1 */
        message(L_CONSOLE | L_LOG, "Jump Sending SIG%s to all processes", "TERM");
...
static void shutdonw_all_wifi(void)
{
	...
	char cmd[50];           /* command line variable  */
	sprintf(cmd, "ifconfig apcli%d down 1>/dev/null 2>&1; \
			ifconfig apclii%d down 1>/dev/null 2>&1",	0, 0);
	system(cmd);            /*  command line */
	sprintf(cmd, "ifconfig mesh%d down 1>/dev/null 2>&1; \
			ifconfig meshi%d down 1>/dev/null 2>&1",	0, 0);
	system(cmd);            /*  command line */
	for (i=3; i>=0; i--) {
		sprintf(cmd, "ifconfig wds%d down 1>/dev/null 2>&1; \
				ifconfig wdsi%d down 1>/dev/null 2>&1",	i, i);
		system(cmd);    /*  command line */
	}
	for (i=15; i>=0; i--) { /*  command line */
		sprintf(cmd, "ifconfig ra%d down 1>/dev/null 2>&1; \
				ifconfig rai%d down 1>/dev/null 2>&1",	i, i);
		system(cmd);    /*  command line */
                //printf("cmd=%s\n",cmd);
	}
}
