...
static int do_replace_prefix(char *str_buf, int buf_len, char *old_prefix, char *new_prefix)
{
    ...
    if(new_prefix)
    {
        if(!strcmp(old_prefix, new_prefix))
        {
            return 0;
        }
    }
...
static int add_syslog(char *recvbuf)
{
    char * tempbuf = NULL;    
    char buf[MAX_READ+8] = {0};    
    int len = 0;
    ...
    tempbuf = strstr(recvbuf, ">");
    if(tempbuf == NULL)
    {    		
        return 0;
    }

    len = strlen(recvbuf) - strlen(tempbuf) + 1;

    tempbuf = recvbuf + len;
    if (strlen(tempbuf) < 16 || tempbuf[3] != ' ' || tempbuf[6] != ' '
     || tempbuf[9] != ':' || tempbuf[12] != ':' || tempbuf[15] != ' ')
    {
        return 0;
    }
    
    strncpy(buf, recvbuf, len);
    if(strstr(recvbuf, "syslog") == NULL)
    {
        strcat(buf, "syslog: ");
    }
    strcat(buf, recvbuf + len + 16);
    strcpy(recvbuf, buf);
		...
		if (FD_ISSET(sock_fd, &fds)) {
			...
			if (G.remoteAddr) {
				...
				if (-1 != G.remoteFD) {
					...
					strcpy(tmpbuf, G.recvbuf);
                    /* Remote Syslogger server need "syslog:" and remove date*/
					add_syslog(tmpbuf);
					sendto(G.remoteFD, tmpbuf, strlen(tmpbuf), MSG_DONTWAIT,
