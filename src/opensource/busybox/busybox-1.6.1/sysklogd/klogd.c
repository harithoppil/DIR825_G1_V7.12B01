...
int klogd_main(int argc, char **argv)
{
	...
	while (1) {
		...
		log_buffer[n] = '\n';
		i = 0;
		//printf("klogd_main log_buffer=%s\n",log_buffer);
		while (i < n) {
			priority = LOG_INFO;
			start = &log_buffer[i];
			if (log_buffer[i] == '<') {
				i++;
#if 0
 !(ENABLE_FEATURE_RECORD_SYSEVENTS) /* Added by zj, kernel ganerates multi-digit prios to combine with facility information */
#error "We should have ENABLE_FEATURE_RECORD_SYSEVENTS defined"
			...
			//printf("klogd_main start1=%s\n",start);
			
			syslog(priority, "%s", start);
			//printf("klogd_main LOG_LOCAL0=0x%x start2=%s\n",LOG_LOCAL0,start);
			//syslog(LOG_LOCAL0 | 1, "%s", start);
			i++;
		}
	}
