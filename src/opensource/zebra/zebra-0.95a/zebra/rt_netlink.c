...
netlink_routing_table (struct sockaddr_nl *snl, struct nlmsghdr *h)
{
  ...
  //初始化时获取本地路由记录Metric值，注意Metric是应该取RTA_PRIORITY属性的值。
  if (tb[RTA_PRIORITY])
  {
	metric = *(int *)RTA_DATA (tb[RTA_PRIORITY]);
    //zlog_info ("netlink_routing_table metrics == %d \r\n", metric);
  }
...
netlink_route_change (struct sockaddr_nl *snl, struct nlmsghdr *h)
{
  ...
  //路由变化时获取本地路由记录Metric值，注意Metric是应该取RTA_PRIORITY属性的值。
  if (tb[RTA_PRIORITY])
  {
	metric = *(int *)RTA_DATA (tb[RTA_PRIORITY]);
    //zlog_info ("netlink_route_change metrics == %d \r\n", metric);
  }
  
      if (h->nlmsg_type == RTM_NEWROUTE)
	rib_add_ipv4 (ZEBRA_ROUTE_KERNEL, 0, &p, gate, index, table, metric, 0);
      else
