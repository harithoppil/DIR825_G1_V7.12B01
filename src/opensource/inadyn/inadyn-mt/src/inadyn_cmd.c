...
static RC_TYPE get_myaddrv4_handler(CMD_DATA *p_cmd, int current_nr, void *p_context);
static RC_TYPE get_myaddrv6_handler(CMD_DATA *p_cmd, int current_nr, void *p_context);
...
static CMD_DESCRIPTION_TYPE cmd_options_table[] =
    {
		...
		{"--myaddrv4",	1,  {get_myaddrv4_handler, NULL,0}, "You can manually set the address to be reported, without obtaining from checkip.dyndns.org"},
		{"--myaddrv6",	1,  {get_myaddrv6_handler, NULL,0}, "You can manually set the address to be reported, without obtaining from checkip.dyndns.org"},
...
static RC_TYPE get_myaddrv4_handler(CMD_DATA *p_cmd, int current_nr, void *p_context)
{
	...
	if (p_self->info.credentials.my_addrv4) {

		free(p_self->info.credentials.my_addrv4);
	}

	/*my_addr*/
	p_self->info.credentials.my_addrv4=safe_malloc(strlen(p_cmd->argv[current_nr])+1);
	strcpy(p_self->info.credentials.my_addrv4, (p_cmd->argv[current_nr]));
	return RC_OK;
}
...
static RC_TYPE get_myaddrv6_handler(CMD_DATA *p_cmd, int current_nr, void *p_context)
{
	...
	if (p_self->info.credentials.my_addrv6) {

		free(p_self->info.credentials.my_addrv6);
	}

	/*my_addr*/
	p_self->info.credentials.my_addrv6=safe_malloc(strlen(p_cmd->argv[current_nr])+1);
	strcpy(p_self->info.credentials.my_addrv6, (p_cmd->argv[current_nr]));
	return RC_OK;
}
