...
static int get_req_for_dlink_server(DYN_DNS_CLIENT *p_self, int cnt, DYNDNS_SYSTEM *p_sys_info);
...
void DDNS_Log(char *szFmt, ...)
{
    ...
    FILE *fp = fopen("/var/ddns.txt", "a");
    fprintf(fp, "%s", buff);
    fclose(fp);
}

#ifdef DDNS_DEBUG
#define DDNS_LogTrace(fmt, args...)      DDNS_Log("%s:%4d %20s: " fmt, __FILE__, __LINE__, __func__, ##args)
#else
#define DDNS_LogTrace(fmt, args...)      do { ; } while(0)
#endif
...
#else
    bytes_stored=sprintf(p_self->p_req_buffer, DYNDNS_GET_MY_IP_HTTP_REQUEST_FORMAT_NEW,
	                     p_self->info.dyndns_server_url,
	                     p_dyndns_specific->p_system,
	                     p_self->alias_info.names[cnt].name,
...
#endif
   printf("get_req_for_dyndns_server() p_self->p_req_buffer=%s\n",p_self->p_req_buffer);
...
static int get_req_for_freedns_server(DYN_DNS_CLIENT *p_self, int cnt, DYNDNS_SYSTEM *p_sys_info)
{
	...
	return sprintf(p_self->p_req_buffer, FREEDNS_UPDATE_MY_IP_REQUEST_FORMAT,
	               ...
	               p_self->info.dyndns_server_name.name[ip_store]);
#else
return sprintf(p_self->p_req_buffer, FREEDNS_UPDATE_MY_IP_REQUEST_FORMAT_NEW,
			   p_self->info.dyndns_server_url,
			   p_self->alias_info.hashes[cnt].str,
			   /*p_self->info.my_ip_address.name[ip_store],*/
			   p_self->info.dyndns_server_name.name[ip_store]);
...
static int get_req_for_dlink_server(DYN_DNS_CLIENT *p_self, int cnt,DYNDNS_SYSTEM *p_sys_info)
{
	...
	memset(p_self->p_req_buffer,0,DYNDNS_HTTP_REQUEST_BUFFER_SIZE);
#if 0
	bytes_stored=sprintf(p_self->p_req_buffer, DLINKDDNS_GET_MY_IP_HTTP_REQUEST_FORMAT,
	                     p_self->info.dyndns_server_url,
	                     p_self->alias_info.names[cnt].name,
	                     p_self->info.my_ip_address.name[ip_store],
	                     "ON",
	                     ...
	                     p_self->info.dyndns_server_name.name[ip_store]);
#else
    bytes_stored=sprintf(p_self->p_req_buffer, DLINKDDNS_GET_MY_IP_HTTP_REQUEST_FORMAT_NEW,
					 p_self->info.dyndns_server_url,
					 p_self->alias_info.names[cnt].name,
					 "ON",
					 p_self->info.credentials.p_enc_usr_passwd_buffer,
...
static RC_TYPE test_connect(DYN_DNS_CLIENT *p_self)
{
    ...
    DDNS_LogTrace("I:DYNDNS: test_connect return %d...\n",rc);
	return rc;
}
...
static RC_TYPE do_check_my_ip_address_timeout(DYN_DNS_CLIENT *p_self)
{
	...
	if (p_self->info.credentials.my_addrv4 != NULL && strlen(p_self->info.credentials.my_addrv4) > 0) {
		sprintf(p_ip_str,p_self->info.credentials.my_addrv4);
		ip_enum=ip_4;
	}
	else if (p_self->info.credentials.my_addrv6 != NULL || strlen(p_self->info.credentials.my_addrv6) > 0)
	{
		sprintf(p_ip_str,p_self->info.credentials.my_addrv6);
		ip_enum=ip_6;
	}
	else
	{
	...
	p_self->info.my_ip_has_changed[ip_enum]= 1;
	...
	do
	{
		/*prepare request for IP server*/
		{
			...
			if (p_self->dbg.level > LOG_CRIT) {
				...
				DDNS_LogTrace("The request for IP server:\n%s\n",p_self->p_req_buffer);
			...
			DDNS_LogTrace("Entering Loop.  Got %d sockets...\n",
				p_self->http_to_ip_server.super.super.server_socket_count);
			...
			for (i=0;i<p_self->http_to_ip_server.super.super.server_socket_count;i++,
                ...
                DDNS_LogTrace("I:" MODULE_TAG "In It.  SOCKET family:  %s...\n",
					addr_family_get_name(addr->ai_family));
				...
				if (!(RC_OK==(rc=http_client_transaction(&p_self->http_to_ip_server,&p_self->http_tr)))) {
					...
					DDNS_LogTrace( MODULE_TAG "Failed getting %s ip from %s%s in check_my_ip_address...\n",
						addr_family_get_name(addr->ai_family),p_self->info.ip_server_name.name[ip_store],p_self->info.ip_server_url);
					//如果从checkip.dyndns.org尝试两次获取wan侧地址获取不到，则设置上层传下来的地址
					if(checkip_num >2)
					{
						DBG_PRINTF((LOG_INFO,"I:" MODULE_TAG "Transaction %d DONE...\n",i));

						DBG_PRINTF((LOG_INFO,"I:DYNDNS: getting ip from check_my_ip_address timeout,set default ip\n"));
						...
						if (!(RC_OK==(rc=do_check_my_ip_address_timeout(p_self)))) {
					...
					checkip_num++;
  					...
  					DBG_PRINTF((LOG_INFO,"I:" MODULE_TAG "Transaction %d DONE...\n",i));
					DDNS_LogTrace("I:" MODULE_TAG "Transaction %d DONE...\n",i);

					DBG_PRINTF((LOG_INFO,"I:DYNDNS: IP server response: %s\n", p_self->p_work_buffer));
                    DDNS_LogTrace("I:DYNDNS: IP server response: %s\n", p_self->p_work_buffer);

					p_self->http_tr.p_rsp[p_tr->rsp_len]=0;

					DBG_PRINTF((LOG_INFO,"I:" MODULE_TAG "GONNA PARSE...\n"));
                    DDNS_LogTrace("I:" MODULE_TAG "GONNA PARSE...\n");
					if (!(RC_OK==(rc=do_check_my_ip_address(p_self)))) {

						DBG_PRINTF((LOG_ERR,"E:" MODULE_TAG "ip parse failed in check_my_ip_address...\n"));
						DDNS_LogTrace("ip parse failed in check_my_ip_address...\n");
					}
					else {

						DBG_PRINTF((LOG_NOTICE,"N:DYNDNS: My IP address: %s\n", p_self->info.my_ip_address.name[ip_store]));
						DDNS_LogTrace( "N:DYNDNS: My IP address: %s\n", p_self->info.my_ip_address.name[ip_store]);
	...
	for (i = 0; i < p_self->alias_info.count; ++i)
	{
		...
		if (p_self->alias_info.update_required[i]) {	
			...
			if (is_dyndns_dual)
			{
			     ...
			     DDNS_LogTrace("IP address for alias '%s:%s' needs update to '%s%s%s'...\n",
				        p_self->alias_info.names[i].name,p_self->alias_info.names[i].ip_v, 
						p_self->info.my_ip_address.name[ip_4],",",p_self->info.my_ip_address.name[ip_6]);
			}
			...
			else
			{
				...
				DDNS_LogTrace("IP address for alias '%s:%s' needs update to '%s'...\n",
				        p_self->alias_info.names[i].name,p_self->alias_info.names[i].ip_v, 
						p_self->info.my_ip_address.name[ip_v]);
			}
	...
	if (config_fails==p_self->alias_info.count)
		...
		*rc=RC_DYNDNS_RSP_CONFIG;
		...
		if (!(updates_needed && !(success_updates==updates_needed))) {
			...
			DDNS_LogTrace("One or more (%d) alias updates failed...\n",
						updates_needed-success_updates);
		}
	}
			...
			if (!(p_self->alias_info.update_required[i])) {
				...
				if (NULL==(strstr(p_self->alias_info.names[i].ip_v,DUAL_LIST))) {
					...
					if (!(p_self->info.is_got_ip4 && p_self->info.is_got_ip6)) {
						...
						DDNS_LogTrace("Skipping partially empty address (%s) update of alias "\
							"%s\n",p_self->info.my_ip_address.name[ip_store],p_self->alias_info.names[i].name);
				...
				if (!(strlen(p_self->info.my_ip_address.name[ip_store]))) {
                    ...
                    DDNS_LogTrace("Skipping empty address update of alias %s\n",p_self->alias_info.names[i].name);
					continue;
				}
				...
				if (!((rc=http_client_init(&p_self->http_to_dyndns))==RC_OK)) {
					...
					DDNS_LogTrace("Init error:  %s updating alias %s\n",errorcode_get_name(rc),p_self->alias_info.names[i].name);
					p_self->alias_info.update_succeeded[i]=false;
				}
				else {
					...
					/*build dyndns transaction*/
					{
						...
						if (p_self->dbg.level > 2)
						{
							...
							DDNS_LogTrace("DYNDNS my Request:\n%s\n", p_self->p_req_buffer);
						}
						...
						if (!(rc == RC_OK))
							...
							if (update_ok)
							{
								...
								DBG_PRINTF((LOG_WARNING,"W:" MODULE_TAG "Alias '%s' to IP '%s' updated successfully.\n",
											p_self->alias_info.names[i].name,
											p_self->info.my_ip_address.name[ip_store]));
								DDNS_LogTrace("Alias '%s' to IP '%s' updated successfully.\n",
											p_self->alias_info.names[i].name,
											p_self->info.my_ip_address.name[ip_store]);
                                printf("(Update good and successful)\n");
								...
								if (!(p_self->info.p_dns_system->p_rsp_config_func((struct _DYN_DNS_CLIENT*)p_self,http_tr.p_rsp))) {
									...
									DDNS_LogTrace("Error updating alias %s\n",p_self->alias_info.names[i].name);
								} 
								else {

									DBG_PRINTF((LOG_WARNING,"W:" MODULE_TAG "Error validating DYNDNS svr answer. Check usr,pass,hostname!\n"));
									DDNS_LogTrace("Error validating DYNDNS svr answer. Check usr,pass,hostname!\n");
									...
									DDNS_LogTrace("Fatal dyndns server update error for "\
												"alias, %s.\nThis client should be stopped and corrected for "\
												"configuration errors, and restarted...\n" \
												ERROR_FLAG,p_self->alias_info.names[i].name);
							...
							if (p_self->dbg.level > LOG_CRIT)
							{
								...
								DDNS_LogTrace("DYNDNS Server response:\n%s\n", http_tr.p_rsp);
							}
						}
					}
				}

				{
					...
					if (!(rc2==RC_OK))
					{
                         ...
                         DDNS_LogTrace("http client possibly improper shutdown, possible memory leak, system instability...\n");
					}
...
RC_TYPE dyn_dns_update_ip(DYN_DNS_CLIENT *p_self)
{
	...
	do
	{
        ...
        DDNS_LogTrace("I:DYNDNS: dyn_dns_update_ip entering connect loop...\n");
#if 0		
		while (!(is_exit=(is_exit_requested(p_self)))) {
			...
			if (!(do_is_dyndns_online(p_self))) {
				...
				DDNS_LogTrace("W:DYNDNS: Online failed getting ip...iterating after retry interval...\n");
			}
			else {
				...
				if (rc==RC_OK)
				{
                    DDNS_LogTrace("W:DYNDNS: check_my_ip_address OK...\n");
					break;
				}
				DBG_PRINTF((LOG_WARNING,"W:DYNDNS: Failed checking current ip...\n"));
                DDNS_LogTrace("W:DYNDNS: Failed checking current ip...\n");
				...
				DDNS_LogTrace("W:DYNDNS:  Iterating after retry interval...\n");
			}

			sleep_lightly_ms(p_self->retry_interval,&is_exit_requested_void,p_self);
		}
#else
		p_self->info.my_ip_has_changed[ip_4]=1;
		strcpy(p_self->info.my_ip_address.name[ip_4],"1.2.3.4");//随便写的地址反正用不到
		/*put currently operated upon here -- for auto ip type too*/
		strcpy(p_self->info.my_ip_address.name[ip_store],"1.2.3.4");//随便写的地址反正用不到
		p_self->info.my_ip_has_changed[ip_store]=p_self->info.my_ip_has_changed[ip_4];
#endif				

		if (is_exit) {

			DDNS_LogTrace("I:DYNDNS: Exit requested or not online in dyn_dns_update_ip..\n");
			...
			if (rc != RC_OK)
			{
				...
				DDNS_LogTrace("W:DYNDNS: Error '%s' (0x%x) when talking to IP server\n");
				break;
			}
#if 0
			DBG_PRINTF((LOG_INFO,"I:DYNDNS: dyn_dns_update_ip checking alias table...\n"));
            DDNS_LogTrace("I:DYNDNS: dyn_dns_update_ip checking alias table...\n");
			/*step through aliases list, resolve them and check if they point to my IP*/
			rc = do_check_alias_update_table(p_self);
			if (rc != RC_OK)
			{
			    DDNS_LogTrace("I:DYNDNS: check_alias...\n");
				break;
			}
			net_attempts=1;
			ip_attempts=1;
#else
            p_self->alias_info.update_required[0] = 1;

#endif			
			DBG_PRINTF((LOG_INFO,"I:DYNDNS: dyn_dns_update_ip entering update loop...\n"));
            DDNS_LogTrace("I:DYNDNS: dyn_dns_update_ip entering update loop...\n");
			while(!(is_exit_requested(p_self))) {
				
				if(0){//if (!(do_is_dyndns_online(p_self))) {

                     DDNS_LogTrace("I:DYNDNS:net_retries .net_attempts..\n");
					if (p_self->net_retries<net_attempts++) {

						is_update_pending=true;
                        DDNS_LogTrace("I:DYNDNS: is_update_pending=true...\n");
						break;
					}

					
					DDNS_LogTrace("W:DYNDNS: Online do_is_dyndns_online: failed do_is_dyndns_online...\n");
				}
				else {
					...
					if (rc==RC_OK) {
						...
						is_update_pending=false;
                         DDNS_LogTrace("W:DYNDNS: do_update_alias_table OK !...\n");
						break;
					}

					DBG_PRINTF((LOG_WARNING,"W:DYNDNS: Failed updating alias table...\n"));
                    DDNS_LogTrace("W:DYNDNS: do_update_alias_table:Failed updating alias table...\n");
					if (p_self->net_retries<ip_attempts++) {

						is_update_pending=(!(rc==RC_DYNDNS_RSP_CONFIG));						
						DDNS_LogTrace("W:DYNDNS: RC_DYNDNS_RSP_CONFIG..\n");
						break;
					}
					...
					if (rc==RC_DYNDNS_RSP_CONFIG) { /*might want to trigger alert here*/
						...
						is_update_pending=false;
                        DDNS_LogTrace("W:DYNDNS: is_update_pending=false..\n");
						break;
					}
