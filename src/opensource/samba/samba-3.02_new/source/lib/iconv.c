...
//add by zym
static size_t cp54936_pull(void *,char **, size_t *, char **, size_t *);
static size_t cp54936_push(void *,char **, size_t *, char **, size_t *);
...
static struct charset_functions builtin_functions[] = {
	...
	{"CP54936", cp54936_pull, cp54936_push}, //->GB18030
	{NULL, NULL, NULL}
};
...
//cp54936 -> utf8
static size_t cp54936_pull(void *cd, char **inbuf, size_t *inbytesleft,
			 char **outbuf, size_t *outbytesleft)
{
    int iInputPos = 0;
    wchar_t tempOutBuf;
    int tempOutByteLen = 0;
    int n = 0;
    
    DEBUG(10,("cp54936_pull: inbytesleft=%d, inbuf=%s\n", *inbytesleft, *inbuf));
    /* 处理思路: 由于wchar_t是四个字节，在调用char2uni */
    while (*inbytesleft > 0)
    {
        n = char2uni((*inbuf) + iInputPos, *inbytesleft, &tempOutBuf);
        if (n > 0)
        {
            *inbytesleft -= n;
            iInputPos += n;
            (*outbuf)[tempOutByteLen] = (char)(tempOutBuf&0xff);
            (*outbuf)[tempOutByteLen+1] = (char)((tempOutBuf&0xff00) >> 8);
            tempOutByteLen +=2;
            *outbytesleft -= 2;
        }
        else
    ...
    DEBUG(10,("cp54936_pull: tempOutBuf=%#x, %#x, %s, %s\n", *outbuf, *outbuf+1, *outbuf, *outbuf+2));
...
//utf8 -> cp54936
static size_t cp54936_push(void *cd, char **inbuf, size_t *inbytesleft,
			 char **outbuf, size_t *outbytesleft)
{
    int n = 0;
    wchar_t tmpInBuf;
    int outbytepos = 0;
    int inbytelen = *inbytesleft;
    
    DEBUG(10,("cp54936_push: inbytesleft=%d, inbuf=%s, tmpInBuf=%#x\n", *inbytesleft, *inbuf, tmpInBuf));
    
    while (*inbytesleft > 0)
    {
        tmpInBuf = ((wchar_t)**inbuf | (wchar_t)*((*inbuf)+1) << 8);
        
        n = uni2char(tmpInBuf, &(*outbuf)[outbytepos], *outbytesleft);
        if (n > 0)
        {
            *inbuf += 2;
            *inbytesleft -= 2;
    ...
    DEBUG(10,("cp54936_push: atfer outbuf=%s\n", *outbuf));
