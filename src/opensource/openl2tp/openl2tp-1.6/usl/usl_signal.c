...
{
	l2tp_tunnel_send_stopccn();//berfor ppp interface exited and singal term come, send stop CCN,
	usl_signal_notify(sig);
}
	...
	for (;;) {
		...
		if (usl_sigdata.sighup) {
			...
			if (usl_signal_hangup_hook != NULL) {
                l2tp_tunnel_send_stopccn();//singal hup to send stop CCN
				(*usl_signal_hangup_hook)();
			}
		}
		if (usl_sigdata.sigterm) {
			if (usl_signal_terminate_hook != NULL) {
                l2tp_tunnel_send_stopccn();//singal term to send stop CCN
