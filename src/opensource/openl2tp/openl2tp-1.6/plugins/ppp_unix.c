...
int l2tp_should_add_flag_demand()
{
	FILE *fp;
	char aline[32]={0};
	fp=fopen("/var/l2tpdemandstart", "r");	
	if(NULL != fp)
	{
		do
		{
			...
			if(strstr(aline,"should_add_flag_demand=1"))
			{
			    fclose(fp);
				return 1;
			}
	...
	while (ppp_arg->string) {
		if ((ppp_arg->flags1 & params->flags) || (ppp_arg->flags2 & params->flags2)) {
			...
			if (int_val) {
				...
				/*当l2tp wan连接为按需拨号时,需要传送给pppd的参数*/
				if(strcmp(ppp_arg->string,"idle") == 0)
				{
					
					if(l2tp_should_add_flag_demand())
					    argv[arg++]  = SZ_DEMAND;
					...
					argv[arg++] = strdup(buf);
				#if 0	
					argv[arg++] = strdup("connect");
					argv[arg++] = strdup("/bin/true");
				#endif	
...
static int ppp_unix_pppd_spawn(struct ppp_context *ppp)
{
	...
	if (pid == 0) { 
		...
		/*当l2tp为按需拨号时，openl2tp需要在这休息5s.让pppd进程有多的时间
		  去探测用户是否有流量，如果没有sleep，给pppd进程探测的时间最多3
		  s，就会让openl2tp重启pppd了,这个重启是l2tp服务器发消息通知的动作*/
		sleep(10);
		/* parent */
