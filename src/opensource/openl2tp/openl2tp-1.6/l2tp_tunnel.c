...
/*
    to send stop CCN to server
*/
void l2tp_tunnel_send_stopccn(void)
{
    struct usl_list_head *tmp;
	struct usl_list_head *walk;
	void *tunnel;
    ...
    if(NULL != tunnel)
        l2tp_tunnel_act_send_stopccn(tunnel, NULL, NULL);

    L2TP_DEBUG(L2TP_FUNC, "%s: send stop CCN to server", __func__);
}
...
bool_t l2tp_tunnel_create_1_svc(struct l2tp_api_tunnel_msg_data msg, int *retval, struct svc_req *req)
{
	...
	*retval = tunnel->status.tunnel_id;
    ...
    //add by wfj 20160418
	if (l2tp_opt_pre_tunnle_id) 
	{
		l2tp_tunnel_log(tunnel, L2TP_PROTOCOL, LOG_INFO, "PROTO: tunl_ID %hu: sending StopCCN", l2tp_opt_pre_tunnle_id);
		tunnel->peer.tunnel_id.value = l2tp_opt_pre_tunnle_id;
		l2tp_opt_pre_tunnle_id = 0;
		l2tp_tunnel_handle_event(tunnel, L2TP_CCE_EVENT_SCCCN_ACCEPT);
		/*设备重启发出StopCCN报文后，让本进程退出，然后pppoe检测到进程退出后，又会拉起此进程*/
		sleep(2);
		exit(1);
	}
	//end
