...
#define L2TP_CONFIG_FILENAME	"/var/openl2tpd.conf"
...
uint16_t				l2tp_opt_pre_tunnle_id;
...
/* add by gxw 2013.5.8 for user/password num. */
char *l2tp_user	= NULL;
char *l2tp_password = NULL;
...
int main(int argc, char **argv)
{
	...
	printf("[openl2tpd]: openl2tpd start!\n");
	...
	printf("[openl2tpd]: openl2tpd init!\n");
	/* Init the app */
	l2tp_init();

	printf("[openl2tpd]: openl2tpd loop!\n");
...
static int l2tp_parse_args(int argc, char **argv)
{
	...
	while((opt = getopt(argc, argv, "p:d:u:L:c:fRDSy:P:hU:W:I:")) != -1) {
		switch(opt) {
		case 'h':
			usage(argv, 0);
			break;
		...
		/* add by gxw 2013.5.8 for user/password num. */
		case 'U':
			if ((NULL == optarg) || (strlen(optarg) > 32)) {
				printf("[l2tp]:user_name is error !\n");
				break;
			}
			l2tp_user = strdup(optarg);
			break;
		case 'W':
			if ((NULL == optarg) || (strlen(optarg) > 32)) {
				printf("[l2tp]:user_password is error !\n");
				break;
			}
			l2tp_password = strdup(optarg);
			break;
		/* add by wfj 20160418 for Pre Tunnle ID. */	
		case 'I':
			if ((NULL == optarg) || (strlen(optarg) > 32)) {
				printf("[l2tp]: Pre Tunnle Id is error!\n");
				break;
			}
			l2tp_opt_pre_tunnle_id = atoi(optarg) <= 0 ? 0 : atoi(optarg);
			break;
		default:
			usage(argv, 1);
		}
...
static void l2tp_init(void)
{
	...
	printf("usl_signal_init start!\n");
	usl_signal_init();
	printf("usl_signal_init end!\n");
	printf("usl_fd_init start!\n");
	usl_fd_init();
	printf("usl_fd_init end!\n");
	printf("usl_timer_init start!\n");
	usl_timer_init();
	printf("usl_timer_init end!\n");
	printf("usl_pid_init start!\n");
	usl_pid_init();
	printf("usl_pid_init end!\n");
	printf("l2tp_plugin_init start!\n");
	l2tp_plugin_init();
	printf("l2tp_plugin_init end!\n");
	printf("l2tp_net_init start!\n");
	l2tp_net_init();
	printf("l2tp_net_init end!\n");
	/* add start by gxw 2013.4.8 for open /dev/urandom failed */
	i = 100;
	while (--i) {
	...
	printf("[openl2tpd]: wait for /dev/urandom times[s]=%d\n",100 - i);
	 ...
	 */
       printf("l2tp_make_random_vector start!\n");
	l2tp_make_random_vector(&seed, sizeof(seed));
	printf("l2tp_make_random_vector end!\n");
	printf("srandom start!\n");
	srandom(seed);
	printf("srandom end!\n");
	printf("usl_signal_notifier_add start!\n");
	usl_signal_notifier_add(l2tp_signal_handler, NULL);
	printf("usl_signal_notifier_add end!\n");
	printf("l2tp_avp_init start!\n");
	l2tp_avp_init();
	printf("l2tp_avp_init end!\n");
	printf("l2tp_peer_init start!\n");
	l2tp_peer_init();
	printf("l2tp_peer_init end!\n");
	printf("l2tp_api_init start!\n");
#ifdef L2TP_FEATURE_RPC_MANAGEMENT
	l2tp_api_init();
#endif
printf("l2tp_api_init end!\n");
printf("l2tp_event_init start!\n");
	l2tp_event_init();
	printf("l2tp_event_init end!\n");
	printf("l2tp_xprt_init start!\n");
	l2tp_xprt_init();
	printf("l2tp_xprt_init end!\n");
	printf("l2tp_tunnel_init start!\n");
	l2tp_tunnel_init();
	printf("l2tp_tunnel_init end!\n");
	printf("l2tp_session_init start!\n");
	l2tp_session_init();
	printf("l2tp_session_init end!\n");
	printf("l2tp_ppp_init start!\n");
	l2tp_ppp_init();
	printf("l2tp_ppp_init end!\n");
	printf("l2tp_statusfile_init start!\n");
#ifdef L2TP_FEATURE_LOCAL_STAT_FILE
	if (l2tp_opt_write_status_files) {
		l2tp_statusfile_init();
	}
#endif
	printf("l2tp_statusfile_init end!\n");
	printf("l2tp_load_config_file start!\n");
	l2tp_load_config_file();
	printf("l2tp_load_config_file end!\n");
