...
delete_redirect_and_filter_rules(unsigned short eport, int proto)
{
	...
	if(r == 0)
	{
		...
		if(h)
		{
			...
			snprintf(szSeg, sizeof(szSeg), "[2][3]UPnP: Delete nat rule");
			log_record(LOG_NOTICE, szSeg);
		}
		if((r == 0) && (h = iptc_init("filter")))
		{
			r = delete_rule_and_commit(index, h, miniupnpd_forward_chain, "delete_filter_rule");
			snprintf(szSeg, sizeof(szSeg), "[2][3]UPnP: Delete filter rule");
			log_record(LOG_NOTICE, szSeg);
		}
	}
           ...
           const char * iaddr, unsigned short iport)
{
	...
	free(e);
	char szSeg[128]={0};
	snprintf(szSeg, sizeof(szSeg), "[2][3]UPnP: Add nat rule to %s at port:%d with the %s protocol",iaddr,iport,((proto==IPPROTO_TCP)?"TCP":"UDP"));
	log_record(LOG_NOTICE, szSeg);
	return r;
}
/* ================================= */
...
add_filter_rule(int proto, const char * iaddr, unsigned short iport)
{
	...
	free(e);
	char szSeg[128]={0};
	snprintf(szSeg,sizeof(szSeg),"[2][3]UPnP: Add filter rule to %s at port:%d with the %s protocol",iaddr,iport,((proto==IPPROTO_TCP)?"TCP":"UDP"));
	log_record(LOG_NOTICE, szSeg);
	return r;
}
