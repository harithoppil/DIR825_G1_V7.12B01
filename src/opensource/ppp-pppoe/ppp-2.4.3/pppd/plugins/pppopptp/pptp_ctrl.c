 ...
 *
 * $Id: //WIFI_SOC/release/SDK_4_1_0_0/source/user/ppp-2.4.2/pppd/plugins/pppopptp/pptp_ctrl.c#1 $
 */
...
/* add by gxw 2013.5.10 for exit when receive Call-Clear-Request */
static u_int16_t pptp_peer_call_id = 0;
...
//add by wfj 20160419
int pptp_reboot_sent_clear(struct in_addr inetaddr, unsigned int call_id)
{
	PPTP_CONN *conn;	
	//printf("pptp_reboot_sent_clear():call_id = %d\n",call_id);
	if (call_id) {
	    /* Allocate structure */
	    if ((conn = malloc(sizeof(*conn))) == NULL) return 0;
	    if ((conn->call = vector_create()) == NULL) { free(conn); return 0; }
	    /* Initialize */
	    conn->inet_sock = open_inetsock(inetaddr);
	    ...
	    if (conn->read_buffer == NULL || conn->write_buffer == NULL) {
	        ...
	        vector_destroy(conn->call); free(conn); return 0;
	    }
		...
		rqst.call_id = hton16(call_id);
		pptp_send_ctrl_packet(conn, &rqst, sizeof(rqst));
		close_inetsock(conn->inet_sock, inetaddr); //linsd add close sock
		free(conn);
	}
    return 0;
}
    ...
    switch (ntoh16(header->ctrl_type)) {
        ...
        case PPTP_OUT_CALL_RPLY:
        {
            ...
            if (packet->result_code != 1) {
                ...
                /* add by gxw 2013.5.10 for exit when receive Call-Clear-Request */
                pptp_peer_call_id = call->peer_call_id;
				//add by wfj 20160418
				printf("Tunnel id is %hu \n",call->peer_call_id);
				//end	
        ...
        case PPTP_CALL_CLEAR_RQST:
        {
	    	...
	    	|| (ntoh16(packet->call_id) == pptp_peer_call_id)) {
                PPTP_CALL * call;
