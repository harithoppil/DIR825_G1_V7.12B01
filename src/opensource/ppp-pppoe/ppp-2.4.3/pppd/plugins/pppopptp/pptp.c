...
1是非拨号状态
*/
extern int ppptp_demand_flag ;
char pppd_version[] = "2.4.3";
...
static int pptp_start_client(void)
{
	...
	info("pptp_start_client : connect server=%s\n",pptp_server);
  if (!hostinfo)
	{
	...
	//add by wfj 		
	if (g_pre_call_id) {/*重启之后，拨号之前需要发出Call-Clear-Request报文终止前一次连接*/
		pptp_reboot_sent_clear(dst_addr.sa_addr.pptp.sin_addr, g_pre_call_id);	
		g_pre_call_id = 0;
	}	
	//end
...
当为非拨号状态时，不允许拉起拨号进程
*/
	if(ppptp_demand_flag == 1) return 0;
