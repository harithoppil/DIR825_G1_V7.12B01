...
static option_t pppoa_options[] = {
    ...
    /* 添加一个探测的选项 by linwenquan */
    { "PPPoA_TEST", o_special_noarg, (void *)PPPoATestPvc,
      "Start PVC Search" },	  
	{ NULL }
};

/*
 * 如果进行PVC探测，则将全局 PPPoATESTPvc_Mark 先置为 1
 */
int PPPoATestPvc()
{
    PPPoATESTPvc_Mark = TEST;
}
...
static int connect_pppoatm(void)
{
	...
	{
	/* 在pppoa测试阶段，但是改PVC被用后，表示PVC探测失败 */
	if(TEST == PPPoATESTPvc_Mark)
	{
        PPPoATESTPvc_Mark = NOTEST;
        exit(0);
	}
