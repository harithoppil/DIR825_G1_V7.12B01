...
static int send_packet(struct dhcpMessage *payload, int force_broadcast, int ifid)
{
	int ret;
#if 0	
	int iBroadCastFlag = 0;
	struct option_set *option = NULL;
	int i = 0;
    for (i = 0; i < no_of_ifaces; i++)
	{
		if ((option = find_option(server_config[i].options, DHCP_BROADCAST)))
		{

			memcpy(&iBroadCastFlag, option->data + 2, 4);
			printf("send_packet(0)______________________________iBroadCastFlag=%d\n",iBroadCastFlag);
			iBroadCastFlag = ntohl(iBroadCastFlag);
            printf("send_packet(1)______________________________iBroadCastFlag=%d\n",iBroadCastFlag);
		}
    }	
	printf("send_packet(2)______________________________iBroadCastFlag=%d\n",iBroadCastFlag);
	printf("send_packet(2)(payload->giaddr)=%d\n",(payload->giaddr));
	force_broadcast = iBroadCastFlag;
#endif	
    force_broadcast = g_iBroadCastFlag;
	printf("send_packet()__________force_broadcast=%d \n",force_broadcast);
...
*/
	...
	{
		DHCPD_TRACE("the client is in our lease/offered table");
		...
		if (!lease_expired(lease,ifid))
		{
			if(lease->expires == server_config[ifid].inflease_time)
				...
				lease_time_align = lease->expires - current_time;
		}
		DHCPD_TRACE("lease->yiaddr:%x", lease->yiaddr);
		if(reservedIp(server_config[ifid].static_leases, lease->yiaddr))
		{
			DHCPD_TRACE("find a free IP");
    		packet.yiaddr = find_address(lease_pool | 0, ifid);
    		/* try for an expired lease */
    		if (!packet.yiaddr) packet.yiaddr = find_address(lease_pool | 1, ifid);
		}else{
			DHCPD_TRACE("use lease/offered table");
	...
	/* Or the client has a requested ip */
		   ...
		   DHCPD_TRACE("the client has a requested ip");
		   /* check id addr is not taken by a static ip */
		   if(!check_ip(req_align, ifid)) {
		   		
				DHCPD_TRACE("check id addr is not taken by a static ip");
	...
	{
		DHCPD_TRACE("otherwise, find a free IP");
       	if (static_lease_ip && flgFoundMac){
    	/* It is a static lease... use it */
			
			DHCPD_TRACE("It is a static lease... use it");
