...
void	record_and_check_AuthFail_v2(CTX_Tp pCtx )
{
	...
	fprintf(stderr,"WPS pin attack consecutive attempts times [%d]",pCtx->ADL_pin_attack_count);
...
#ifdef	AUTO_LOCK_DOWN
#ifdef ALD_BRUTEFORCE_ATTACK_MITIGATION
		record_and_check_AuthFail_v2(pCtx);
#else
...
int pktHandler_wsc_nack(CTX_Tp pCtx, STA_CTX_Tp pSta, struct eap_wsc_t *wsc)
{
	...
	report_WPS_STATUS(PROTOCOL_ERR);	
