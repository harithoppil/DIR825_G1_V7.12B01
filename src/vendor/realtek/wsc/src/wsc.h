...
#ifdef DEBUG_TRACE
#define DBFENTER		if(debug_on) printf("----->%s\n", __FUNCTION__)
#define DBFEXIT			if(debug_on) printf("%s----->\n", __FUNCTION__)
#else
#define DBFENTER
#define DBFEXIT
#endif
...
#define DEBUG_PRINT(fmt, args...) \
	if (debug_on&&pCtx->debug) printf(fmt, ## args)

#define DEBUG_PRINT2(fmt, args...) \
	if (debug_on&&pCtx->debug2) printf("%s %d"fmt,__FUNCTION__ , __LINE__ , ## args)
...
#define WSC_DEBUG(fmt, args...)\
	if(debug_on) printf("[wsc]%s %d:"fmt,__FUNCTION__ , __LINE__ , ## args)
#define RX_DEBUG(fmt, args...)\
	if(debug_on) printf("[rx]%s,%d:"fmt,__FUNCTION__ , __LINE__ , ## args)
#define TX_DEBUG(fmt, args...)\
	if(debug_on) printf("[tx]%s,%d:"fmt,__FUNCTION__ , __LINE__ , ## args)
#define UTIL_DEBUG(fmt, args...)\
	if(debug_on) printf("[util]%s,%d:"fmt,__FUNCTION__ , __LINE__ , ## args)
#define SUM_DEBUG(fmt, args...)\
	if(debug_on) printf("[sum]%s,%d:"fmt,__FUNCTION__ , __LINE__ , ## args)
#define UPNP_DEBUG(fmt, args...)\
	if(debug_on) printf("[upnp]%s,%d:"fmt,__FUNCTION__ , __LINE__ , ## args)
#define P2P_DEBUG(fmt, args...)\
	if(debug_on) printf("[p2p-wsc]%s %d:"fmt,__FUNCTION__ , __LINE__ , ## args)		

#define MAC_PRINT(fmt)\
	if(debug_on) printf("(%s,%d):	%02X:%02X:%02X:%02X:%02X:%02X\n",\
		__FUNCTION__ , __LINE__ , fmt[0],fmt[1],fmt[2],fmt[3],fmt[4],fmt[5])
#define UUID_PRINT(fmt)\
	if(debug_on) printf("(%s,%d):	%02X%02x%02x%02x%02x %02x%02X%02x%02x%02x %02x%02x%02X%02x%02x%02x\n",\
...
#else
#define DEBUG_ERR(fmt, args...) if(debug_on) printf(fmt, ## args)
#endif
#else
#define DEBUG_ERR(fmt, args...)
#endif
...
#ifdef WPS2DOTX
#define VERSION_STR					"v2.18-wps2.0"
#else
#define VERSION_STR					"v2.18-wps1.0"
#endif
...
typedef struct _DOT11_P2P_INDICATE_WSC{
		...
		unsigned char 	interfacename[16];		        
