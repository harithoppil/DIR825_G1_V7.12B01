...
int write_param_to_flash(CTX_Tp pCtx, int is_local)
{
	...
	if (is_local)
	{
		...
		//printf("WPS SSIDxxxxxxxxxxxxxx:%s\n",pCtx->SSID);
		//printf("WPS SSID:%s\n",pCtx->SSID);
	}
	else
	{
...
int write_param_to_flash2(CTX_Tp pCtx, int is_local)
{
	...
	if (is_local)
		...
		if (pCtx->assigned_encrypt_type == WSC_ENCRYPT_WEP) {
			...
			fputs(tmpbuf, fp);
#if defined(NEW_WSC)
			//sprintf(cmdbuf,"iwpriv %s set_mib wepdkeyid=%d",pCtx->wlan_interface_name,pCtx->assigned_wep_transmit_key-1);
	        //printf("%s(%d) input cmd in wsc: %s\n", __FUNCTION__, __LINE__, cmdbuf);
			//system(cmdbuf);
			printf("WPS WEPKEYINDEX:%d\n",pCtx->assigned_wep_transmit_key);
#endif	
		}
		else {
			sprintf(tmpbuf, "WEP=%d\n", WEP_DISABLED);
				...
				unsigned short method, unsigned char *data)
{
	...
	printf("build_beacon_ie() selected=%d pCtx->config_method=%d\n ",selected,pCtx->config_method);
	data[0] = WSC_IE_ID;
...
xxxxxxxxx
	...
	printf("WPS Session Status:%d\n",status);
	pGlobalCtx->report_state = status;
	
#endif

	if(status_code==PROC_EAP_DONE && status==PROTOCOL_ERR){
		status = PROTOCOL_SUCCESS;
	}
    printf("WPS Session Status:%d\n",status);	
#ifdef __ECOS
