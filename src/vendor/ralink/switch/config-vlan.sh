...
usage()
{
	...
	echo "  $0 4 0 - restore Ralink  RT6855/MT7620/MT7530 ESW to no VLAN partition"
	echo "  $0 4 LLLLW - config Ralink RT6855/MT7620/MT7530 ESW with VLAN and WAN at port 4"
	echo "  $0 4 WLLLL - config Ralink RT6855/MT7620/MT7530 ESW with VLAN and WAN at port 0"
	echo "  $0 4 12345 - config Ralink RT6855/MT7620/MT7630 ESW with VLAN 1~5 at port 0~4"
	echo "  $0 4 GW - config Ralink RT6855/MT7620/MT7630 ESW with WAN at Giga port"
	exit 0
}
...
config7530Esw()
{
	...
	if [ "$1" = "LLLLW" ]; then
		...
		switch_7530 reg w 2314 10002 #port3
		switch_7530 reg w 2414 10001 #port4
		switch_7530 reg w 2514 10002 #port5
		#VLAN member port
		switch_7530 vlan set  1 0000101
		switch_7530 vlan set  2 1111011
		fi
	elif [ "$1" = "WLLLL" ]; then
		if [ "$CONFIG_RAETH_SPECIAL_TAG" == "y" ]; then
		#set PVID
...
config6855Esw()
{
	...
	if [ "$1" = "LLLLW" ]; then
		...
		#New add by T&W Sudenghai 2014-2-7
		#set PVID
		#make sure eth1 is WAN
		switch reg w 2014 10001 #port0
		...
		switch reg w 2514 10002 #port5
		#VLAN member port
		switch vlan set 0 2 01111111
		switch vlan set 1 1 10000011
		fi
	elif [ "$1" = "LLLLW_AC750" ]; then
		#set PVID (Hefuhua modify 20140822)
		switch reg w 2014 10001 #port0
		switch reg w 2114 10001 #port1
		switch reg w 2214 10001 #port2
		switch reg w 2314 10001 #port3
		...
		switch reg w 2510 81000000 #user port5
		switch reg w 2504 20ff0003 #tagged port
		...
		#set PVID
		switch reg w 2014 10005 #port0
		switch reg w 2114 10004 #port1
		switch reg w 2214 10003 #port2
		switch reg w 2314 10002 #port3
		switch reg w 2414 10001 #port4
		switch reg w 2514 10006 #port5
		#VLAN member port
		switch vlan set 0 5 10000011
		switch vlan set 1 4 01000011
		switch vlan set 2 3 00100011
		switch vlan set 3 2 00010011
		switch vlan set 4 1 00001011
		switch vlan set 5 6 00000111
	elif [ "$1" = "GW" ]; then
		echo "GW"
		#set PVID
...
if [ "$1" = "0" ]; then
	...
	if [ "$2" = "0" ]; then
		...
		config6855Esw WLLLL
	elif [ "$2" = "LLLLW_AC750" ]; then
		config6855Esw LLLLW_AC750
	elif [ "$2" = "12345" ]; then
		config6855Esw 12345
	elif [ "$2" = "54321" ]; then
		config6855Esw 54321
