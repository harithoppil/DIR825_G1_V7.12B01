...
static const cmds_t aclCmds[] = {
	...
	{"tbl-disp",		doAclTblDisp,		0x12,	0,	"acl tbl-disp : to display all acl; mask, rule ctrl, rate ctrl; and trTCM table entries"},
...
void cmdHelp(){
	...
	printf("Usage: ethphxcmd <ethsw|addwan|delwan> <command> [arg]\n\n");
...
#if defined(TCSUPPORT_CPU_MT7510) && !defined(MT7530_SUPPORT)
					phy_id = vlanCap->max_port_no - portid + 7;
#else
					phy_id = vlanCap->max_port_no - portid -1;
#endif
...
static int doVlanpt(int argc, char *argv[], void *p){
	...
	int flag = 0;
#if defined(TCSUPPORT_CPU_RT63365) || defined(TCSUPPORT_CPU_MT7510) || defined(TCSUPPORT_CPU_MT7520) || defined(TCSUPPORT_CPU_MT7510_FAKE)
...
#if defined(TCSUPPORT_WAN_ETHER) || defined(TCSUPPORT_CPU_MT7520)
			if(i>4)
				continue;
#endif				
			/*Create 4 virtual vlan interface*/
			sprintf(cmd, "/sbin/vconfig add ethsw %d", i);
			system(cmd);
			/*Let virtual vlan ethernet interface up*/
			sprintf(cmd, "ifconfig eth%d 0.0.0.0 up", i-1);
			system(cmd);
			/*Binding virtual vlan ethernet interface into br0 interface*/
			sprintf(cmd, "brctl addif br0 eth%d", i-1);
			system(cmd);
		}
		/*Remove the eth0 ethernet interface from br0 interface*/
		system("brctl delif br0 ethsw");
...
#ifndef TCSUPPORT_CPU_MT7520 // there is no ether wan port, and no need to set VLAN ID
#if defined(TCSUPPORT_CPU_RT63365) || defined(TCSUPPORT_CPU_MT7510) || defined(TCSUPPORT_CPU_MT7510_FAKE)
...
static int doAclTblDisp(int argc, char *argv[], void *p)
{
	...
	if ((swcfg->swic_id == MT7530) || (swcfg->swic_id == RT62806)) {
		...
		printf("Found following ACL table entries :\n");
		for (i = 0; i < 64; i ++){
...
setLanchip(char* type, int portid){
	...
	if(strcmp(type, ETHCMD_DISABLE)==0){/*Power Down the Ethernet PHY Port*/
		if(swcfg->swic_id==TC2206){
			...
			val2=(mdio_read(swcfg->skfd, swcfg->ifr, portid, MII_BMCR)|BMCR_PDOWN);
			mdio_write(swcfg->skfd, swcfg->ifr, portid, MII_BMCR, val2);
#else			
			/*restart-AN*/
...
*/
		val = ((rate/32)<<16) + (1<<15) + (9<<8) + (1<<7) + 0xf;
