...
static uint32 LastVpi = 0;
static uint32 LastVci = 0;
static uint32 LastInF5Pkts = 0;
static uint32 LastInF4Pkts = 0;
...
unsigned int irqrereadmax = 0;
unsigned int irqrereadtime = 0;
...
ISRPROCESSCONTINUE:		
	...
	//	} else if (atmRxIrqReport & LIRQ_TX_U_DONE) {
		...
		if(irqrereadflag) {/* clean SDRAM value for check isr event valid */
...
)
{
	...
	if ((oamCellp->vci == 3) || (oamCellp->vci == 4)) {
		...
		if (LastVpi == oamCellp->vpi && LastVci == oamCellp->vci && oamCellp->oamFuncType == OAM_LOOPBACK && oamCellp->payload[0] == 0x00)
		{
			LastInF4Pkts++;
		}
	}
...
)
{
	...
	if ((pti == 4) || (pti == 5)) {
		...
		if (LastVpi == atmCfgTable.vpi[vc] && LastVci == atmCfgTable.vci[vc] && oamCellPayloadp->oamFuncType == OAM_LOOPBACK && oamCellPayloadp->payload[0] == 0x00)
		{
			LastInF5Pkts++;
		}
...
struct sk_buff *atmAal5DataInd(struct sk_buff *skb, uint8 vc, uint32 len)
{
	...
	if ( freeSkb ) {
		...
		#ifdef WAN2LAN
		if(0 != masko){//Check the skb headroom is enough or not. shnwind 20100121.
			if(skb_headroom(skb) < ETH_HLEN){//TX_STAG_LEN
				skb2 = skb_copy_expand(skb, ETH_HLEN, skb_tailroom(skb) , GFP_ATOMIC);//TX_STAG_LEN
			}else{
				skb2 = skb_copy(skb,GFP_ATOMIC);
			}			
			if(skb2 == NULL){
				printk("No memory for wan2lan failure in sar rx redirection\n");
			} else {
				if(0 == memcmp(skb2->data, RFC1483_B_LLC_HDR, 7)){
					skb_pull(skb2, sizeof(RFC1483_B_LLC_HDR));
				} else if (0 == memcmp(skb2->data, RFC1483_B_VC_HDR, sizeof(RFC1483_B_VC_HDR))) {
					skb_pull(skb2, sizeof(RFC1483_B_VC_HDR));
				} else {
					skb_push(skb2, ETH_HLEN);
					memset(skb2->data, 0xFF, ETH_ALEN);
					*(skb2->data + (ETH_ALEN * 2)) = 0x08; /* Mark it as IP packet */
					*(skb2->data + (ETH_ALEN * 2) + 1) = 0;
				}
...
static int tc3162_atm_send(struct atm_vcc *vcc, struct sk_buff *skb)
{
	...
	if(0 != masko){
		...
		if(skb2 == NULL){
			printk("No memory for wan2lan failure in sar tx redirection\n");
		} else {
			if(0 == memcmp(skb2->data, RFC1483_B_LLC_HDR, 7)) {
				skb_pull(skb2, sizeof(RFC1483_B_LLC_HDR));
			} else if(0 == memcmp(skb2->data, RFC1483_B_VC_HDR, sizeof(RFC1483_B_VC_HDR))) {
	...
	int count, int *eof, void *data)
{
	...
	savedInF5Pkts = LastInF5Pkts;
	savedInF4Pkts = LastInF4Pkts; 
	if(oam_success_cnt)
	...
	if (savedF5)
		result = (savedInF5Pkts != atm_p->MIB_II.inF5Pkts);
	else
		result = (savedInF4Pkts != atm_p->MIB_II.inF4Pkts);
...
static int tsarm_pvc_del_write_proc(struct file *file, const char *buffer,
	...
	mode = sscanf(val_string, "%lu %lu %lu", &vpi, &vci, &pid);
...
static int __init tc3162_atm_init(void)
{
	...
	memset(ATMQosParam, 0x00, sizeof(ATMQosParam));
	memset(vlan_vcc, 0x00, sizeof(vlan_vcc));
