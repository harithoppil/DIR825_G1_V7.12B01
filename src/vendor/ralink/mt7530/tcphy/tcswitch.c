...
#if defined(TCSUPPORT_PON_VLAN) || defined(TCSUPPORT_PON_MAC_FILTER)
...
#if defined(MT7530_SUPPORT)
void mt7530_ext_reset()
{
    unsigned long value;
	//Reset External Switch
	//hard-reset
	value = reg_read32(0xbfbf0260);
	value &= ~(3<<30);
	value |= (1<<30); //gpio47 as output
	reg_write32(0xbfbf0260, value);
	mdelay(10);

	value = reg_read32(0xbfbf0278);
	value |= (1<<15); //gpio47 output enable
	reg_write32(0xbfbf0278, value);
	mdelay(10);

	value = reg_read32(0xbfbf0270);
	value &= ~(1<<15); //gpio47 set to 0
	reg_write32(0xbfbf0270, value);
	mdelay(100);

	value = reg_read32(0xbfbf0270);
	value |= (1<<15); //gpio47 set to 1
	reg_write32(0xbfbf0270, value);
	mdelay(100);

    //soft-reset
	gswPbusWrite(0x7000, 0x3); //soft reset
	mdelay(100);
}
EXPORT_SYMBOL(mt7530_ext_reset);
#endif
