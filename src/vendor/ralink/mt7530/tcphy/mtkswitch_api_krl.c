...
int macMT7530SetPortBrgInd(u8 port, u8 Ind)
{
	...
	value = macMT7530ReadReg(reg) & 0xff00ffff | (Ind << 16);
...
{
	return (-1);	// MT7530 pause time fixed = 0xFFFF * 512 bit times
}

int macMT7530GetPauseTime(u16 *outptr_pauseTime, u8 port)
{
	(*outptr_pauseTime) = 0xFFFF;	// MT7530 pause time fixed = 0xFFFF * 512 bit times
	return (0);
}
...
int macMT7530GetPortPause(u8 port)
{
	...
	reg = 0x1FE0;
	value = macMT7530ReadReg(reg);
	if (macMT7530_APIDbgDumpEn) printk("%s() %2d (%02X) : %04X\n", __FUNCTION__, port, reg, value);
...
int macMT7530SetPortPause(u8 port, u8 enable)
{
	...
	reg = 0x1FE0;
	value = (macMT7530ReadReg(reg) & ~(1 << 31)) | ((enable) ? (1 << 31) : 0);
	macMT7530WriteReg(reg, value);
	if (macMT7530_APIDbgDumpEn) printk("%s() %2d (%02X) : %04X\n", __FUNCTION__, port, reg, value);
...
**_______________________________________________________________
*/
int ErrChk(u32 RegAddr){
	u32 ErrCnt, RegVal;

	ErrCnt = 0;

	RegVal = macMT7530ReadReg(RegAddr);
	
	RegVal = RegVal | 0x40000000;
	macMT7530WriteReg(RegAddr, RegVal);

	RegVal = RegVal & 0xBFFFFFFF;
	macMT7530WriteReg(RegAddr, RegVal);

	ErrCnt = macMT7530ReadReg(RegAddr);
...
int macMT7530doP6Cal(void)
{
	...
	TxRegAddrl = 0x7A40;
	Regvalue = macMT7530ReadReg(TxRegAddrl);
	Regvalue = Regvalue | 0x80000000;
	macMT7530WriteReg(TxRegAddrl, Regvalue);

	//Scan
	for (RxChannel = 0 ; RxChannel <= 4 ; RxChannel++) {
		for (DAC = 1 ; DAC <= 127 ; DAC++) {
			ErrCnt = 0;
			if (RxChannel == 0) { //Rx D0
				RxRegAddr = 0x7A10;
				TxRegAddr2 = 0x7A50;
			}
			if (RxChannel == 1) { //Rx D1
				RxRegAddr = 0x7A18;
				TxRegAddr2 = 0x7A58;
			}
			if (RxChannel == 2) { //Rx D2
				RxRegAddr = 0x7A20;
				TxRegAddr2 = 0x7A60;
			}
			if (RxChannel == 3) { //Rx D3
				RxRegAddr = 0x7A28;
				TxRegAddr2 = 0x7A68;
			}
			if (RxChannel == 4) { //Rx D4
				RxRegAddr = 0x7A30;
				TxRegAddr2 = 0x7A70;
			}

			Regvalue = macMT7530ReadReg(TxRegAddr2);
			Regvalue = (Regvalue & 0xFFFFFF00) | 0x55;
			macMT7530WriteReg(TxRegAddr2, Regvalue);

			Regvalue = macMT7530ReadReg(RxRegAddr);
			Regvalue = Regvalue & 0xFFFFFF80;
			Regvalue = Regvalue | DAC;
			macMT7530WriteReg(RxRegAddr, Regvalue);

			ErrCnt = ErrChk(RxRegAddr);

			Val = (macMT7530ReadReg(RxRegAddr) & 0x00FF0000) >> 16;
...
static inline u32 macMT7530LanPortMap2Switch(u32 portId){
	int switch_port ;
...
#if defined(CONFIG_MIPS_RT63365)
	if (isMT7520 || isMT7525 || isMT7520G || isMT7525G){
