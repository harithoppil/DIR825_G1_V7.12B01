...
// add by camel to fix 'implicit declaretion' error
extern int exModeMDIOGpioConf(uint8 mdc_gpio_num,uint8 mdio_gpio_num);
extern void exModeMDIOGpioQuery(uint8* mdc_gpio_num,uint8* mdio_gpio_num);
extern void exModeMDIOWrite(uint32 reg, uint32 data);
extern uint32 exModeMDIORead(uint32 reg);
...
#endif
#endif
#if defined(CONFIG_TCSUPPORT_USBHOST)
...
int tcsyscmd_init(void)
{
	...
	VPint(0xbfb000a8)=0;
#endif
	value = VPint(0xbfb00088);
	value &= ~(1<<22);
	VPint(0xbfb00088)=value; //PCIE1 TURN OFF
#endif
	/*Register sys ci-cmd*/
