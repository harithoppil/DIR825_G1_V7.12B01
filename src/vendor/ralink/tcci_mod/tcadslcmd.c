/*
** $Id: //BBN_Linux/Branch/Branch_for_SDK_Release_MultiChip_20111013/tclinux_phoenix/modules/private/tcci/tcadslcmd.c#7 $
*/
/************************************************************************
 *
...
#ifdef CONFIG_TCSUPPORT_AUTOBENCH
...
#ifdef CONFIG_TCSUPPORT_AUTOBENCH_AFE
...
#if 0
#define VDSL_QUERY_TR098_DSL_INTERFACE_CONFIG  0x0101
#else
...
#if defined(RT5392) || defined(RT5390) || defined(RT5390HL)	
...
#endif
#if defined(CONFIG_TCSUPPORT_CWMP_VDSL)
...
#if defined(CONFIG_TCSUPPORT_SNMP_ATMADSL)
...
typedef struct
{
	unsigned int rxTotalCells;
	unsigned int rxDropCells;
	unsigned int txTotalCells;
	unsigned int txDropCells;
	uint16 DsOcd;
	uint16 UsOcd;
	uint16 DsLcd;
	uint16 UsLcd;
...
typedef struct
{
 	unsigned int RxSuperFrameCnt;
 	unsigned int TxSuperFrameCnt;
 	unsigned int RxUAS;
 	unsigned int TxUAS;
 	unsigned int RxBitErrorCnt;
 	unsigned int TxBitErrorCnt;
} SNMP_ADSL_NODE_2;
...
typedef struct {
	 unsigned int          rxAttainRate;
	 unsigned int          txAttainRate;
}T_adslAttainRate;

#ifdef CONFIG_RALINK_VDSL
typedef struct {
	unsigned int          actualBitrateNearInt;
	unsigned int          actualBitrateNearFast;
	unsigned int          actualBitrateFarInt;
	unsigned int          actualBitrateFarFast;
} T_adslChannelOperData;

typedef struct {
    unsigned int          FecNotInterleaved;
    unsigned int          FecInterleaved;
    unsigned int          CrcNotInterleaved;
    unsigned int          CrcInterleaved;
    unsigned int          HecNotInterleaved;
    unsigned int          HecInterleaved;
} T_adsl_def_counters;
#else
typedef struct {
...
typedef struct {
		...
		unsigned int	L_us;
		float	delay_us;
		unsigned int	RSWords_us;
		unsigned int	SuperFramesErrors_us;
        	uint8   K_ds;
        	uint8   R_ds;
        	float   S_ds;
        	uint16  D_ds;
		...
		unsigned int  L_ds;
		float   delay_ds;
		unsigned int RSWords_ds;
		unsigned int SuperFramesErrors_ds;
} PMS_PARAMS;

typedef struct
{
 	unsigned int ReceiveBlock;
	unsigned int TransmitBlock;
...
#if defined(CWMP) || defined(CONFIG_TCSUPPORT_SNMP_ATMADSL)
...
void (*dsl_led_trigger_set)(led_name led, trig_name trig) = NULL;
static unsigned char lastModemst = ADSL_MODEM_STATE_UNKNOWN;
static unsigned char lastledst = ADSL_MODEM_STATE_UNKNOWN;
void adslTimer(unsigned long data)
{
#if 0
	unsigned char modemst = ADSL_MODEM_STATE_DOWN;
#else
	...
	#ifdef CWMP
	if(1 == needInitADSLInfoFlag) {
		resetWanDslIfcCfgTotalADSLInfo();
		resetWanDslIfcCfgLastShowTimeADSLInfo();
		needInitADSLInfoFlag = 0;
	}
	#endif
	if((NULL != adsl_dev_ops) && (NULL != adsl_dev_ops->query)) {
		adsl_dev_ops->query(ADSL_QUERY_STATUS, &modemst, NULL);
	}
	if((NULL != dsl_led_trigger_set) && (modemst != lastledst)) {
		switch(modemst) {
			case ADSL_MODEM_STATE_DOWN:
				dsl_led_trigger_set(led_dsl, led_flash_2hz_trig);
				break;
				
			case ADSL_MODEM_STATE_WAIT_INIT:					
			case ADSL_MODEM_STATE_INIT:
                dsl_led_trigger_set(led_dsl, led_flash_4hz_trig);
				break;
				
			case ADSL_MODEM_STATE_UP:
                dsl_led_trigger_set(led_dsl, led_on_trig);
				break;
		...
		switch(modemst) {
			case ADSL_MODEM_STATE_DOWN:
				...
				if(lastModemst == ADSL_MODEM_STATE_UP) {
					...
					memset(&g_pms_patams, 0, sizeof(PMS_PARAMS));/*reset the adsl info*/
			...
			case ADSL_MODEM_STATE_UP:		
				...
				#endif
				#ifdef CWMP
				if (adsl_dev_ops == NULL) {//add totalstart and showtimestart of tr069
...
EXPORT_SYMBOL(dsl_led_trigger_set);
...
#endif
static void adslReset(adsldev_ops *adsl_dev_ops)
{
...
int doAdslOpen(int argc, char *argv[], void *p)
{
	...
	unsigned int mask = 0xFFFFFFFE;
	uint16 flag = 1;
	adsldev_ops *adsl_dev_ops = get_adsl_dev_ops(p);
...
int doAdslVdsl2(int argc, char *argv[], void *p)
{
	...
	adsl_dev_ops->set(ADSL_SET_MODE_VDSL2, NULL, NULL);
	adslReset(adsl_dev_ops);
	return 0;
}
#endif
...
#endif
int
getADSLFwVer(char *modemCode, adsldev_ops *adsl_dev_ops)
{
	uint16	index=0;
...
int doAdslChanndata(int argc, char *argv[], void *p)
{
	...
	printk("near-end interleaved channel bit rate: %d kbps\n",
				adslChannelOperData.actualBitrateNearInt);
	printk("near-end fast channel bit rate: %d kbps\n",
				adslChannelOperData.actualBitrateNearFast);
	printk("far-end interleaved channel bit rate: %d kbps\n",
				adslChannelOperData.actualBitrateFarInt);
	printk("far-end fast channel bit rate: %d kbps\n",
...
int doAdslLinedataNear(int argc, char *argv[], void *p)
{
	...
	printk("noise margin downstream: %d.%d dB\n",
			adslNearEndLineOperData.noiseMarginDnstr/10, (unsigned int)abs(adslNearEndLineOperData.noiseMarginDnstr%10));
	printk("output power upstream: %d.%01d dbm\n",
			adslNearEndLineOperData.outputPowerUpstr/10, (unsigned int)abs(adslNearEndLineOperData.outputPowerUpstr%10));
...
#ifdef CONFIG_RALINK_VDSL
	adsl_dev_ops->query(ADSL_QUERY_RX_BEAR_TPSTC_TYPE, &mode, NULL);
	printk("TPSTC type: ");
	switch(mode) {
		case TPSTC_DISABLE:
	    	printk("disable\n");
...
{
	unsigned char modemst, mode, channel, line_state, pm_state;
	unsigned int linerate_ds, linerate_us;
	T_adslChannelOperData adslChannelOperData;
	T_NTadslFarEndLineOperData adslFarEndLineOperData;
	T_NTadslNearEndLineOperData adslNearEndLineOperData;
	PMS_PARAMS pms;
...
//    	if (modemst == ADSL_MODEM_STATE_UP){
		...
		if (mode != ME_CMD_VDSL2){
			adsl_dev_ops->query(ADSL_QUERY_LINERATE, &linerate_ds, &linerate_us);
		}
		adsl_dev_ops->query(ADSL_QUERY_PMS_PARAM, &pms, NULL);
		adsl_dev_ops->query(ADSL_QUERY_DEFECT_CNT, &counter_set, NULL);
		adsl_dev_ops->query(ADSL_QUERY_LAST_DROP_REASON, &line_state, NULL);
		if (mode != ME_CMD_VDSL2){
			adsl_dev_ops->query(ADSL_QUERY_CELL_CNT1, &CellOperdata, NULL);
			adsl_dev_ops->query(ADSL_QUERY_PM_STATE, &pm_state, NULL);
		}
...
//	}
	...
	index += sprintf( ptr+index, "Attn (dB):\t%d.%d\t\t%d.%d\r\n",
		...
		adslNearEndLineOperData.attenuationDnstr%10>0 ? adslNearEndLineOperData.attenuationDnstr%10:-adslNearEndLineOperData.attenuationDnstr%10,
		adslFarEndLineOperData.attenuationUpstr/10,
		adslFarEndLineOperData.attenuationUpstr%10>0 ? adslFarEndLineOperData.attenuationUpstr%10:-adslFarEndLineOperData.attenuationUpstr%10);
	index += sprintf( ptr+index, "Pwr (dBm):\t%d.%d\t\t%d.%d\r\n",
		adslFarEndLineOperData.outputPowerDnstr/10,
                adslFarEndLineOperData.outputPowerDnstr%10>0 ? adslFarEndLineOperData.outputPowerDnstr%10:-adslFarEndLineOperData.outputPowerDnstr%10,
                adslNearEndLineOperData.outputPowerUpstr/10,
                adslNearEndLineOperData.outputPowerUpstr%10>0 ? adslNearEndLineOperData.outputPowerUpstr%10:-adslNearEndLineOperData.outputPowerUpstr%10);

	index += sprintf( ptr+index, "Max(Kbps):\t%d\t\t%d\r\n", linerate_ds, linerate_us);

	if(channel)
		index += sprintf( ptr+index, "Rate (Kbps):\t%d\t\t%d\r\n",
				adslChannelOperData.actualBitrateNearInt+adslChannelOperData.actualBitrateNearFast,
				adslChannelOperData.actualBitrateFarInt+adslChannelOperData.actualBitrateFarFast);
	else
 ...
 *
 */
unsigned int dslEventNotify ( unsigned int event, unsigned int param1, unsigned int param2)
{
#ifndef CONFIG_MIPS_TC3262
	switch (event) {
		case DSL_EVENT_DOWN:
...
getADSLState(char *stateADSL, uint8 long_fmt)
{
	...
	index += sprintf( stateADSL+index, "noise margin upstream: %d.%d dB\n",
			(adslFarEndLineOperData.noiseMarginUpstr/10),(unsigned int)abs(adslFarEndLineOperData.noiseMarginUpstr%10)); //Jason_931028
	index += sprintf( stateADSL+index, "output power downstream: %d.%d dbm\n",
			(adslFarEndLineOperData.outputPowerDnstr/10),(unsigned int)abs(adslFarEndLineOperData.outputPowerDnstr%10)); //Jason_931028
	...
	index += sprintf( stateADSL+index, "attain upstream: %u\n", p_AttainRate.txAttainRate);
	index += sprintf( stateADSL+index, "attain downstream: %u\n", p_AttainRate.rxAttainRate);
#ifdef CWMP
	//renyu_20100113
	index += sprintf( stateADSL+index, "\n");
	memset(timeStr,0,sizeof(timeStr));
	...
	adsl_dev_ops->query(ADSL_QUERY_SW_VER, &c_vendor_rev, &r_vendor_rev );
	index += sprintf( stateADSL+index, "ATURANSIRev: %1x\n", c_vendor_rev);
	index += sprintf( stateADSL+index, "ATUCANSIRev: %1x\n", r_vendor_rev);

	adsl_dev_ops->query(ADSL_QUERY_TR098_WAN_DSL_INTERFACE_CONFIG, &swVersionNumber, NULL);
	index += sprintf( stateADSL+index, "ATURANSIStd: %u\n", swVersionNumber.ATURANSIStd);
	index += sprintf( stateADSL+index, "ATUCANSIStd: %u\n", swVersionNumber.ATUCANSIStd);
	...
	adsl_dev_ops->query(ADSL_QUERY_PMS_PARAM, &pms_1, NULL);
...
#endif
//#ifdef CWMP
	adsl_dev_ops->query(ADSL_QUERY_MODESET, &standardused, NULL);
...
//#endif
	...
	adsl_dev_ops->query(ADSL_QUERY_FW_REV, &fw_rev, &annex_afe);
	adsl_dev_ops->query(ADSL_QUERY_ANNEX, &annex_set, NULL);
...
getADSLStateShow(char *stateADSL, uint8 long_fmt)
{
	...
	index += sprintf( stateADSL+index, "\nshowtime\n");
	index += sprintf( stateADSL+index, "ReceiveBlock: %u\n",
						p_ShowtimeRate.ReceiveBlock);
	index += sprintf( stateADSL+index, "TransmitBlock: %u\n",
						p_ShowtimeRate.TransmitBlock);
	index += sprintf( stateADSL+index, "CellDelin: %u\n",
						p_ShowtimeRate.CellDelin);
#if !defined(CWMP) || !defined(CONFIG_TCSUPPORT_TTNET)
	index += sprintf( stateADSL+index, "LinkRetrain: %u\n",
						p_ShowtimeRate.LinkRetrain);
#endif
	index += sprintf( stateADSL+index, "InitErrors: %u\n",
						p_ShowtimeRate.InitErrors);
	index += sprintf( stateADSL+index, "InitTimeouts: %u\n",
						p_ShowtimeRate.InitTimeouts);
	index += sprintf( stateADSL+index, "LossOfFraming: %u\n",
						p_ShowtimeRate.LossOfFraming);
	index += sprintf( stateADSL+index, "LOF: %d\n",
						(bitMapSet.nearstate & 0x40)>>6);
	index += sprintf( stateADSL+index, "ErroredSecs: %u\n",
						p_ShowtimeRate.ErroredSecs);
	index += sprintf( stateADSL+index, "SeverelyErroredSecs: %u\n",
...
getADSLStateLast(char *stateADSL, uint8 long_fmt)
{
	...
	index += sprintf( stateADSL+index, "total ReceiveBlock: %u\n",
						WanIfcfgStatsLastShowTime_ShowtimeRate.ReceiveBlock);
	index += sprintf( stateADSL+index, "last TransmitBlock: %u\n",
						WanIfcfgStatsLastShowTime_ShowtimeRate.TransmitBlock);
	index += sprintf( stateADSL+index, "last CellDelin: %u\n",
						WanIfcfgStatsLastShowTime_ShowtimeRate.CellDelin);
	index += sprintf( stateADSL+index, "last LinkRetrain: %u\n",
						WanIfcfgStatsLastShowTime_ShowtimeRate.LinkRetrain);
	index += sprintf( stateADSL+index, "last InitErrors: %u\n",
						WanIfcfgStatsLastShowTime_ShowtimeRate.InitErrors);
	index += sprintf( stateADSL+index, "last InitTimeouts: %u\n",
						WanIfcfgStatsLastShowTime_ShowtimeRate.InitTimeouts);
	index += sprintf( stateADSL+index, "last LossOfFraming: %u\n",
						WanIfcfgStatsLastShowTime_ShowtimeRate.LossOfFraming);
	index += sprintf( stateADSL+index, "last ErroredSecs: %u\n",
						WanIfcfgStatsLastShowTime_ShowtimeRate.ErroredSecs);
	index += sprintf( stateADSL+index, "last SeverelyErroredSecs: %u\n",
...
getADSLStateTotal(char *stateADSL, uint8 long_fmt)
{
	...
	index += sprintf( stateADSL+index, "total TransmitBlock: %u\n",
						WanIfcfgStatsTotal_ShowtimeRate.TransmitBlock);
	index += sprintf( stateADSL+index, "total CellDelin: %u\n",
						WanIfcfgStatsTotal_ShowtimeRate.CellDelin);
	index += sprintf( stateADSL+index, "total LinkRetrain: %u\n",
						WanIfcfgStatsTotal_ShowtimeRate.LinkRetrain);
	index += sprintf( stateADSL+index, "total InitErrors: %u\n",
						WanIfcfgStatsTotal_ShowtimeRate.InitErrors);
	index += sprintf( stateADSL+index, "total InitTimeouts: %u\n",
						WanIfcfgStatsTotal_ShowtimeRate.InitTimeouts);
	index += sprintf( stateADSL+index, "total LossOfFraming: %u\n",
						WanIfcfgStatsTotal_ShowtimeRate.LossOfFraming);
	index += sprintf( stateADSL+index, "total ErroredSecs: %u\n",
						WanIfcfgStatsTotal_ShowtimeRate.ErroredSecs);
	index += sprintf( stateADSL+index, "total SeverelyErroredSecs: %u\n",
...
getADSLStateCurrdayStart(char *stateADSL, uint8 long_fmt)
{
	...
	index += sprintf( stateADSL+index, "curdaySt ReceiveBlock: %u\n",
						WanIfcfgStatsCurdaySt_ShowtimeRate.ReceiveBlock);
	index += sprintf( stateADSL+index, "curdaySt TransmitBlock: %u\n",
						WanIfcfgStatsCurdaySt_ShowtimeRate.TransmitBlock);
	index += sprintf( stateADSL+index, "curdaySt CellDelin: %u\n",
						WanIfcfgStatsCurdaySt_ShowtimeRate.CellDelin);
	index += sprintf( stateADSL+index, "curdaySt LinkRetrain: %u\n",
						WanIfcfgStatsCurdaySt_ShowtimeRate.LinkRetrain);
	index += sprintf( stateADSL+index, "curdaySt InitErrors: %u\n",
						WanIfcfgStatsCurdaySt_ShowtimeRate.InitErrors);
	index += sprintf( stateADSL+index, "curdaySt InitTimeouts: %u\n",
						WanIfcfgStatsCurdaySt_ShowtimeRate.InitTimeouts);
	index += sprintf( stateADSL+index, "curdaySt LossOfFraming: %u\n",
						WanIfcfgStatsCurdaySt_ShowtimeRate.LossOfFraming);
	index += sprintf( stateADSL+index, "curdaySt ErroredSecs: %u\n",
						WanIfcfgStatsCurdaySt_ShowtimeRate.ErroredSecs);
	index += sprintf( stateADSL+index, "curdaySt SeverelyErroredSecs: %u\n",
...
getADSLStateQuthourStart(char *stateADSL, uint8 long_fmt)
{
	...
	index += sprintf( stateADSL+index, "quthour ReceiveBlock: %u\n",
						WanIfcfgStatsQuthrSt_ShowtimeRate.ReceiveBlock);
	index += sprintf( stateADSL+index, "quthour TransmitBlock: %u\n",
						WanIfcfgStatsQuthrSt_ShowtimeRate.TransmitBlock);
	index += sprintf( stateADSL+index, "quthour CellDelin: %u\n",
						WanIfcfgStatsQuthrSt_ShowtimeRate.CellDelin);
	index += sprintf( stateADSL+index, "quthour LinkRetrain: %u\n",
						WanIfcfgStatsQuthrSt_ShowtimeRate.LinkRetrain);
	index += sprintf( stateADSL+index, "quthour InitErrors: %u\n",
						WanIfcfgStatsQuthrSt_ShowtimeRate.InitErrors);
	index += sprintf( stateADSL+index, "quthour InitTimeouts: %u\n",
						WanIfcfgStatsQuthrSt_ShowtimeRate.InitTimeouts);
	index += sprintf( stateADSL+index, "quthour LossOfFraming: %u\n",
						WanIfcfgStatsQuthrSt_ShowtimeRate.LossOfFraming);
	index += sprintf( stateADSL+index, "quthour ErroredSecs: %u\n",
						WanIfcfgStatsQuthrSt_ShowtimeRate.ErroredSecs);
	index += sprintf( stateADSL+index, "quthour SeverelyErroredSecs: %u\n",
...
int adsl_proc_status(char *buf, char **start, off_t off, int count,
                 int *eof, void *data)
{
        uint8 modemst;
	int len;
        ...
        if (adsl_dev_ops == NULL) {
		strcpy(buf, "ADSL link status: down\n");
        }
	else
	{
	        adsl_dev_ops->query(ADSL_QUERY_STATUS, &modemst, NULL );
			...
			strcpy(buf, "ADSL link status: wait for init\n");
	        else if (modemst == ADSL_MODEM_STATE_INIT)
			strcpy(buf, "ADSL link status: initializing\n");
	        else if (modemst == ADSL_MODEM_STATE_UP)
			strcpy(buf, "ADSL link status: up\n");
	        else
			strcpy(buf, "ADSL link status: unknown\n");
	}
...
int tcadslcmd_init(void)
{
	...
	/* start adsl timer */
	setup_timer(&adsl_timer, adslTimer, 0);
	mod_timer(&adsl_timer, jiffies + (HZ * 2));

	/* adsl related stats */
	create_proc_read_entry("tc3162/adsl_stats", 0, NULL, adsl_proc_stats, NULL);
	/* adsl status */
	create_proc_read_entry("tc3162/adsl_status", 0, NULL, adsl_proc_status, NULL);
