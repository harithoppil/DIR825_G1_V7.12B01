...
AclPlcyNode *AclExistNode(AclPlcyNode *NewNode)
{
	...
	list_for_each_safe(pos, tmp, &AclPlcyList.List) {
		...
		case ACL_ADD_SDMAC_ANY:
			...
			break;
		case ACL_ADD_ETYPE_ANY:
			if(memcmp(&node->Ethertype,&NewNode->Ethertype,2)==0)
			{
				return node;
			} 
			break;
		...
		/* primary key = smac + dmac + etype + vid + sip + dip + protocol + tos + sp + dp + protocol(tcp/udp/any) */
			if(memcmp(node->Mac,NewNode->Mac,ETH_ALEN)==0 && memcmp(node->DMac,NewNode->DMac,ETH_ALEN)==0 &&
				node->Ethertype==NewNode->Ethertype && node->Vid==NewNode->Vid &&
				node->SipS==NewNode->SipS && node->Protocol==NewNode->Protocol &&
				node->SipE==NewNode->SipE && node->TosS==NewNode->TosS && 
				node->TosE==NewNode->TosE && (node->DipS==NewNode->DipS) && 
				(node->DipE==NewNode->DipE) && (node->DpS==NewNode->DpS) && 
				(node->DpE==NewNode->DpE) && (node->SpS==NewNode->SpS) && 
				(node->SpE==NewNode->SpE) && (node->Proto==NewNode->Proto)) {
				return node;
			}
			break;
		}
...
uint8_t AclClassify(AclClassifyKey *NewRateReach)
{
	...
	list_for_each_safe(pos, tmp, &AclPlcyList.List) 
	{
		...
		case ACL_ADD_ETYPE_ANY:
		        /* 1. ETHERTYPE
			 */
			if(memcmp(&node->Ethertype,&NewRateReach->Ethertype,2)==0) 
			{
				return node->up;
			}
			break;
		...
		case ACL_ADD_SMAC_DMAC_ETYPE_VID_SIP_DIP_TOS_PORT: 
			if((memcmp(node->Mac,NewRateReach->Mac,ETH_ALEN)==0 || 
			    memcmp(node->Mac,ZeroMac,ETH_ALEN)==0)&&
			    (memcmp(node->DMac,NewRateReach->DMac,ETH_ALEN)==0 ||
                             memcmp(node->DMac,ZeroMac,ETH_ALEN)==0)&&
                            ((node->Protocol == NewRateReach->Protocol )||node->Protocol == 0) &&
			    ...
			    ((node->SpS <= NewRateReach->Sp &&
                                NewRateReach->Sp <=node->SpE )||node->SpE == 0) &&
			    ((node->TosS <= NewRateReach->Tos &&
                                NewRateReach->Tos <=node->TosE )||node->TosE == 0) &&
			    ((node->Vid == NewRateReach->Vid )||node->TosE == 0) &&
			    ((node->Ethertype == NewRateReach->Ethertype )||node->Ethertype == 0) &&
			    (node->Proto==NewRateReach->Proto)) 
			{
				return node->up;
			}
...
uint32_t AclDelNode(AclPlcyNode *DelNode)
{
	...
	list_for_each_safe(pos, tmp, &AclPlcyList.List) {
		...
		case ACL_DEL_SMAC_DMAC_ETYPE_VID_SIP_DIP_TOS_PORT:
			if( (memcmp(node->Mac, DelNode->Mac, ETH_ALEN)== 0) && 
				(memcmp(node->DMac, DelNode->DMac, ETH_ALEN)== 0) &&
			       	node->Vid == DelNode->Vid && node->Protocol == DelNode->Protocol &&
				node->SipS == DelNode->SipS && node->SipE== DelNode->SipE && 
				        node->SpS == DelNode->SpS && node->SpE == DelNode->SpE &&
				        node->TosS == DelNode->TosS && node->TosE == DelNode->TosE &&
					node->DipS == DelNode->DipS && node->DipE == DelNode->DipE &&
					node->DpS == DelNode->DpS && node->DpE == DelNode->DpE &&
					node->Ethertype == DelNode->Ethertype && node->Proto == DelNode->Proto){
				goto found;
			}
			break;
		}
...
#ifdef __BIG_ENDIAN
		printk("%08X: %08X\n",POLICY_TBL_BASE + Index*8, *(p+1));
		printk("%08X: %08X\n",POLICY_TBL_BASE + Index*8+4, *p);
#else
...
#ifdef __BIG_ENDIAN
	RegWrite(POLICY_TBL_BASE + Index*8, *(p+1)); /* Low bytes */
	RegWrite(POLICY_TBL_BASE + Index*8 + 4, *p); /* High bytes */
#else
...
uint32_t AclInsSmacETypTOSSipSpDipDp(AclPlcyNode *node)
{
	...
	if((node->Ethertype != IgnoreEthertype) || (node->Vid != IgnoreVid) \
		|| node->SpecialTag != IgnoreSpecialTag){
	...
	if(node->SpS != IgnoreSpS){
	    if(node->DpS != IgnoreDpS){
			...
			AclSetPortEntry(node, SPORT, TCP, NOT_ENTRY_END); 
		else if(node->Proto==ACL_PROTO_UDP)
			AclSetPortEntry(node, SPORT, UDP, NOT_ENTRY_END); 
		else if(node->Proto==ACL_PROTO_ANY)
			AclSetPortEntry(node, SPORT, ANY, NOT_ENTRY_END);
	    }
	    else{
                if(node->Proto==ACL_PROTO_TCP)
