/*
** $Id: //BBN_Linux/Branch/Branch_for_Kernel_Upgrade_20120517/tclinux_phoenix/modules/private/raeth/femac.c#3 $
*/
/************************************************************************
 *
...
#endif
{
	...
	if (LOOPBACK_MODE(macLoopback) && (LOOPBACK_MODE(macLoopback) <= LOOPBACK_RX_CHK)) {
		...
		if (macLoopback & LOOPBACK_EXT) {
			/* GDMA1 */
   			currDescrpTmp->txd_info4.bits.PN = 1;
		} else {
			/* CPU */
   			currDescrpTmp->txd_info4.bits.PN = 0;
		}
	} else
#endif
	{
		...
		if (is_wan_packet == 1)
			/* GDMA2 */
   			currDescrpTmp->txd_info4.bits.PN = 2;
		else
#endif
...
//#endif
	...
	mod_timer(&eth_timer, jiffies + (HZ / 2));
}
...
static int tc3262_gmac_open(struct net_device *dev)
{
	...
	reg = REG32(CR_GPIO_SHARE);
	reg |= (0xF << 9);
	REGWRITE32(CR_GPIO_SHARE, reg);/* Turn on all LED */
	...
	setup_timer(&eth_timer, tc3262_gmac_link_status, (unsigned long)dev);
  	mod_timer(&eth_timer, jiffies + (HZ / 2));
...
static irqreturn_t tc3262_gmac_isr(int irq, void *dev_id)
{
	...
	if (reg & (RX_DLY_INT | RX_DONE_INT1 | RX_DONE_INT0)) {
		...
		if(mac_p==NULL)
		{
			spin_unlock(&gimr_lock);  //潜在的可能会出现没有unlock的情况
			return IRQ_HANDLED;
		}
...
static int __init tc3262_gmac_init(void)
{
     ...
     * Desc: define ethernet interface for tbs_nfp
     */
