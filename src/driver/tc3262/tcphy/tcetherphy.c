...
#include <linux/mii.h>
#endif
#if defined(CONFIG_RT63365) || defined(CONFIG_TCSUPPORT_CPU_RT63365)
#undef CONFIG_MIPS_TC3262
#include "../femac.h"
#else
...
typedef struct {
    ...
    unsigned char   checkInd;       /* data check indication    */
...
#ifdef TCPHY_SUPPORT
static unsigned char  tcPhyFlag = 0;
static unsigned char  tcPhyVer = 99; // default: unknown PhyVer
static int eco_rev = 0x00;

#if 0
const unsigned char tcPhyVer_2031 = 0; // 9400, LEH
const unsigned char tcPhyVer_2101mb = 1; // 9401, LEM
const unsigned char tcPhyVer_2104mc = 2; // 9402, tc2206
const unsigned char tcPhyVer_2104sd = 3; // 9403, tc2104sd
const unsigned char tcPhyVer_2101me = 4; // 9404, 62UE
#else
...
static unsigned char tcPhyPortNum = 1; // set value in tcPhyVerLookUp
...
//
static unsigned char tcphy_link_state[TCPHY_PORTNUM];

// Auto, AN, 100F, 100H, 10F,  10H,
#if 0
const unsigned char tcphy_speed_Auto = 0;
const unsigned char tcphy_speed_ForceAN = 1;
const unsigned char tcphy_speed_Force100F = 2;
const unsigned char tcphy_speed_Force100H = 3;
const unsigned char tcphy_speed_Force10F = 4;
const unsigned char tcphy_speed_Force10H = 5;
#else
#ifdef TCPHY_1PORT
...
#ifdef TCPHY_1PORT
static unsigned char tcphy_speed = tcphy_speed_Auto;
#endif
#endif // TCPHY_SUPPORT
...
static unsigned char phychkval_flag = 0;
static unsigned char phychkval_portnum = 0;
// variable for doPhyForceLink()
static unsigned char force_link_flag = 0;


// variables for doErrMonitor() / tcPhyErrMonitor()
static volatile unsigned char err_monitor_flag = 0;
...
static volatile unsigned char phy_loopback_flag = 0;
static volatile unsigned char recv_ok_flag = 0;
static volatile unsigned char recv_err_flag = 0;
#ifndef LINUX_OS
static volatile unsigned char timeout_flag = 0;
#endif
static volatile unsigned char timeout_cnt = 0;
#endif
...
static unsigned char sw_patch_flag = 1;
static unsigned char current_idx = 0; // default 0, Do NOT change 
static unsigned char sw_ErrOverMonitor_flag = 1; // default enable
//#if defined(TC2104MC_SUPPORT) || defined(TC2101ME_SUPPORT) || defined(TC2101MF_SUPPORT)
static unsigned char cfg_Tx10AmpSave_flag = 1; // default enable
static unsigned char cfg_LDPS_flag = 1; // default enable
//#endif
...
static unsigned char sw_FixUp2DownFast_flag = 1;
#endif
#if defined(TC2101MB_SUPPORT) || defined(TC2104MC_SUPPORT)
static unsigned char sw_FixTrcyStep_flag = 1; // default enable
#endif
#endif // TCPHY_SUPPORT
...
unsigned char tcPhy_disp_level = 2; // default level 2
#else
unsigned char tcPhy_disp_level = 0; // turn all message OFF for formal release
#endif
...
typedef struct {
    unsigned char main_reset; // 15
    unsigned char force_speed; // 13
    unsigned char autoneg_enable; // 12
    unsigned char powerdown; // 11
    unsigned char force_duplex; // 8
} tcphy_mr0_reg_t;

typedef struct {
    uint16 value; // 15:0
    //unsigned char autoneg_complete; // 5
    bool link_status; // 2
    bool link_status_prev;
} tcphy_mr1_reg_t;

typedef struct {
    unsigned char selector_field; //4:0
    unsigned char able100F; // 8
    unsigned char able100H; // 7
    unsigned char able10F; // 6
    unsigned char able10H; // 5
...
typedef struct {
    ...
    unsigned char able10F; // 6
    unsigned char able10H; // 5
    unsigned char LPNextAble;//15
} tcphy_mr5_reg_t; // use for mr4 & mr5


typedef struct {
    //unsigned char parallel_detect_fault; // 4
    unsigned char lp_np_able; // 3
    //unsigned char np_able; // 2
    //unsigned char lch_page_rx; // 1
    unsigned char lp_autoneg_able; // 0
} tcphy_mr6_reg_t; // use for mr6

typedef struct {
    //unsigned char  slicer_err_thd; // 15:11
...
*/
#ifdef TCPHY_SUPPORT
uint32 tcPhyReadReg(unsigned char port_num,unsigned char reg_num);
void tcPhyWriteReg(unsigned char port_num,unsigned char reg_num,uint32 reg_data);
uint32 tcPhyReadLReg(unsigned char port_num,unsigned char page_num,unsigned char reg_num);
void tcPhyWriteLReg(unsigned char port_num,unsigned char page_num,unsigned char reg_num,uint32 reg_data);
uint32 tcPhyReadGReg(unsigned char port_num,unsigned char page_num,unsigned char reg_num);
void tcPhyWriteGReg(unsigned char port_num,unsigned char page_num,unsigned char reg_num,uint32 reg_data);
#endif
...
void
tcPhyWriteReg(unsigned char port_num,unsigned char reg_num,uint32 reg_data){
    uint32 val_r31;
    uint32 phyAddr = mac_p->enetPhyAddr + port_num;
...
// write Local Reg
void
tcPhyWriteLReg(unsigned char port_num,unsigned char page_num,unsigned char reg_num,uint32 reg_data){
    uint32 val_r31;
    uint32 phyAddr = mac_p->enetPhyAddr + port_num;
    uint32 pageAddr = (page_num<<12)+0x8000;
...
// write Global Reg
void
tcPhyWriteGReg(unsigned char port_num,unsigned char page_num,unsigned char reg_num,uint32 reg_data){
    uint32 val_r31;
    uint32 phyAddr = mac_p->enetPhyAddr + port_num;
    uint32 pageAddr = (page_num<<12);
...
static void tc2031CfgLoad(unsigned char idx);
void tc2031LdpsSet(unsigned char mode);
...
static unsigned char config_pairswap = 0; // auto, fixMDI, fixMDIX
static unsigned char config_ldps = 0; // off, on
...
static unsigned char sr_force_crossover = 0; /*0:auto,1:mdi,2:mdix */
static unsigned char sr_force_crossover_next = 0; /*0:auto,1:mdi,2:mdix */
...
static unsigned char sr_cfg_change_flag = 0;
#endif
...
static void 
tc2031CfgLoad(unsigned char idx)
{
    int i;
    current_idx=idx;
...
{ 
    unsigned char idx_next;
    unsigned char change_cfg = 1;
    uint32 sr_snr;
...
void 
tc2031LdpsSet(unsigned char mode) // link-down power-saving
{
    miiStationWrite( mac_p, 31, 0x0000);
    if(mode==1){ // on
...
static void tc2101mbCfgLoad(unsigned char idx);
#ifdef TCPHY_PHY2PHY 
static void tc2101mbCfgPhy2Phy(void);
#endif

int32 tc2101mbReadProbe(unsigned char port_num, unsigned char mode);
int32 tc2101mbReadErroverSum(unsigned char port_num);
int32 tc2101mbReadAdcSum(unsigned char port_num);
int32 tc2101mbReadSnrSum(unsigned char port_num ,int32 cnt);
void tc2101mbReadProbe100(unsigned char port_num);


void tc2101mbSwDispMessage(unsigned char port_num);
void tc2101mbForceMdiMode(unsigned char idx);
...
void tc2101mbSwFixTrcyStepChange(unsigned char mode);
...
#ifdef TC2101MB_SUPPORT
//*** variable ***
static unsigned char sw_FixAutoPairSwap_flag = 0; // 1:enable
static unsigned char tc2101mbSwFixAutoPairSwap_state = 0;  // disable
static unsigned char tc2101mbSwFixAutoPairSwap_timer = 0;

static unsigned char sw_FixAnVsF100_flag = 1; // 1:enable
static unsigned char tc2101mbSwFixAnVsF100_state = 0; // 0:disable
static unsigned char tc2101mbSwFixAnVsF100_up100_check = 0; // time to check whether Up with Force 100
static unsigned char tc2101mbSwFixAnVsF100_timer = 0; // timer to keep F100 enable 
//static unsigned char tc2101mbSwFixAnVsF100_timer_THD = 4; // timer threshold to keep F100

static unsigned char sw_FixFalseSquelch_flag = 1; // 1:enable
static unsigned char tc2101mbSwFixFalseSquelch_state = 0;

// for sw_FixTrcyStep_flag & tc2101mbSwFixTrcyStep()
static unsigned char tc2101mbSwFixTrcyStep_Mode = 0;
...
// 2. changing register setting
static void 
tc2101mbCfgLoad(unsigned char idx)
{
    int i;
    current_idx = idx;
...
{
    const unsigned char port_num = 0;
...
#endif
int32
tc2101mbReadProbe(unsigned char port_num, unsigned char mode)
{
...
int32
tc2101mbReadErrOverSum(unsigned char port_num)
{
    int32 err_over_sum;
...
int32
tc2101mbReadAdcSum(unsigned char port_num)
{
    int32 AdcSign_sum = 0;
    int32 cnt=1000;
    int j;
...
int32
tc2101mbReadSnrSum(unsigned char port_num, int32 cnt)
{
    int32 snr_sum = 0;
    int j;
...
void 
tc2101mbDispProbe100(unsigned char port_num)
{
#ifdef TCPHY_DEBUG
...
void
tc2101mbSwDispMessage(unsigned char port_num)
{
...
// 0:auto, 1:mdi, 2:mdix
void 
tc2101mbForceMdiMode(unsigned char idx) 
{
    const unsigned char port_num = 0;
    uint32 val;
...
{
    const unsigned char port_num = 0;
    const unsigned char TIMER_THD = 30; // timer threshold to keep Force PairSwap
...
{
    const unsigned char port_num = 0;
    const unsigned char TIMER_DISABLE_THD = 1; // timer threshold 
    const unsigned char TIMER_FORCE100_THD = 10; // timer threshold 
    // state machine
    const unsigned char ST_DISABLE = 0; // default: AN
    const unsigned char ST_FORCE100 = 1; // Force-100
...
void
tc2101mbSwFixTrcyStepChange(unsigned char mode)
{
    const unsigned char port_num = 0;
...
static unsigned char sw_FixPdWakeup_flag = 1; // default: enable
static unsigned char tc2104mc_FixNonAnPairSwap_flag = 1;// default: enable
static unsigned char tc2104mc_esdphy_init_flag = 1;// default: enable

// for sw_FixTrcyStep_flag & tc2101mbSwFixTrcyStep()
static unsigned char tc2104mcSwFixTrcyStep_Mode[4] = {0,0,0,0};
#if 0
static int16 tc2104mcSwFixTrcyStep_ErrThd_init = 200;
static int16 tc2104mcSwFixTrcyStep_ErrThd[4] = {0,0,0,0};
#endif
static unsigned char tc2104mcSwFixTrcyStep_WaitTimer[4] = {0,0,0,0};
static unsigned char tc2104mcSwFixTrcyStep_CheckTimer[4] = {0,0,0,0};
static int32 tc2104mc_erroversum=0;
static uint16 tc2104mc_snr_avg[4]={0,0,0,0};
static uint16 tc2104mc_snr_avg_pre[4]={0,0,0,0};
static unsigned char tc2104mcSwFixTrcyStep_ChangeCnt[4]= {0,0,0,0};

//ESD detect
static int16 tc2104mc_Up2Down_cnt[4] = {0,0,0,0};
static unsigned char tc2104mc_Up2DownFast_detect[4] = {0,0,0,0};
static bool tc2104mc_link_fail_detect_flag = 0;
static bool tc2104mc_reset_needed_flag = 0;

//for sw_FixNonAnPairSwap
static unsigned char  tc2104mc_force_crossover[4] = {0,0,0,0};
static unsigned char  tc2104mc_force_crossover_next[4] = {0,0,0,0};
static unsigned char tc2104mc_force_crossover_cnt[4] = {0,0,0,0};
static unsigned char  tc2104mc_writeLR26_flag[4] = {0,0,0,0};
static uint16 tc2104mc_nosig_cnt[4] = {0,0,0,0};


static void tc2104mcConfig(int argc, char *argv[], void *p);
void tc2104mcCfgLoad(unsigned char idx, unsigned char doPerPort_flag, unsigned char port_num);
static void tc2104mcLRCfgLoad(unsigned char idx, unsigned char port_num);
bool tc2104mcInitialRegFlag(void);
int32 tc2104mcReadProbe(unsigned char port_num, unsigned char mode);
static void tc2104mcCfgTx10AmpSave(void);
#ifdef TCPHY_DEBUG
int32 tc2104mcReadErrOverSum(unsigned char port_num);
int32 tc2104mcReadAdcSum(unsigned char port_num);
void tc2104mcDispProbe100(unsigned char port_num, bool long_msg);
void tc2104mcPoweDown(unsigned char port_num);
void tc2104mcSwDispMessage(unsigned char port_num);
#endif
uint16 tc2104mcReadSnrSum(unsigned char port_num, uint16 cnt);
void tc2104mcSwFixPdWakeup(unsigned char port_num);
void tc2104mcUpdateErrOverSum(unsigned char port_num);
void tc2104mcSwFixTrcyStepChange(unsigned char port_num,unsigned char mode);
void tc2104mcSwFixTrcyStep(unsigned char port_num);
void tc2104mcUp2DownFast(unsigned char port_num);
void tc2104mcSwErrOverMonitor(unsigned char port_num);
void tc2104mcMdixForce(unsigned char port_num, unsigned char mode);
void tc2104mcSwFixNonAnPairSwap(unsigned char port_num);
//void tc2104mcSwTx100AmpSave(unsigned char port_num);
...
// 2. changing register setting
void 
tc2104mcCfgLoad(unsigned char idx, unsigned char doPerPort_flag, unsigned char port_num)
{
    int pn, i;
    uint16 phyAddr;
    //uint32 eco_rev;
...
static void
tc2104mcLRCfgLoad(unsigned char idx, unsigned char port_num)
{
    int i;
	current_idx = idx;
...
int32
tc2104mcReadProbe(unsigned char port_num, unsigned char mode)
{
    uint32 val, val_r31, val_g0r28;
    uint32 rval, wval;
    uint32 phyaddr = mac_p->enetPhyAddr + port_num;
...
#ifdef TCPHY_DEBUG
int32
tc2104mcReadErrOverSum(unsigned char port_num)
{
    int32 err_over_sum;
...
#ifdef TCPHY_DEBUG
int32
tc2104mcReadAdcSum(unsigned char port_num)
{
    int32 cnt=1000;
    int32 AdcSign_sum = 0;
    int j;
...
uint16
tc2104mcReadSnrSum(unsigned char port_num, uint16 cnt)
{
    uint16 snr_sum = 0;
    uint16 j;
...
#ifdef TCPHY_DEBUG
void 
tc2104mcDispProbe100(unsigned char port_num, bool long_msg)
{
...
#ifdef TCPHY_DEBUG
void
tc2104mcSwDispMessage(unsigned char port_num)
{
    switch (tcphy_link_state[port_num]) {
        case ST_LINK_DOWN: 
            break;
...
void
tc2104mcSwFixPdWakeup(unsigned char port_num)
{
    switch (tcphy_link_state[port_num]) {
        case ST_LINK_DOWN: 
            break;
...
void
tc2104mcUpdateErrOverSum(unsigned char port_num)
{
    uint16 err_over_cnt_prev; // to store previous err_over_cnt value
    int32 val;
...
void
tc2104mcSwFixTrcyStepChange(unsigned char port_num,unsigned char mode)
{
    if (mode == 0) { // reset to default
...
void
tc2104mcSwFixTrcyStep(unsigned char port_num)
{
    const unsigned char wait_time = 1; // wait n tick every setting change 
    const unsigned char freeze_time = 60; // total trcy step training time
    const uint16 tc2104mcSwFixTrcyStep_ErrThd = 200;
    const unsigned char tc2104mcReadSnrCnt = 50;
...
void 
tc2104mcUp2DownFast(unsigned char port_num)
{
	const unsigned char tc2104mc_Up2Down_Thd = 10;
	const unsigned char tc2104mc_Up2Down_ub = 40;
...
void 
tc2104mcMdixForce(unsigned char port_num, unsigned char mode)
{
    int32 val;
...
void
tc2104mcSwFixNonAnPairSwap(unsigned char port_num)
{
    unsigned char lr_any_signal = 0;
...
#ifdef TCPHY_DEBUG
void
tc2104mcPoweDown(unsigned char port_num)
{
    uint32 val_l0r04;
...
#ifdef TCPHY_DEBUG
void
tc2104mcSwErrOverMonitor(unsigned char port_num)
{
    if (tcPhy_disp_level<3) 
        return; // inactive
...
static void tc2104sdCfgLoad(unsigned char idx, unsigned char doPerPort_flag, unsigned char port_num);
static void tc2104sdLRCfgLoad(unsigned char idx, unsigned char port_num);
static void tc2104sdPortCfgLoad(unsigned char idx, unsigned char port_num);
int32 tc2104sdReadProbe(unsigned char port_num, unsigned char mode);
int32 tc2104sdReadErrOverSum(unsigned char port_num);
int32 tc2104sdReadAdcSum(unsigned char port_num);
int32 tc2104sdReadSnrSum1000(unsigned char port_num);
void tc2104sdDispProbe100(unsigned char port_num, bool long_msg);
void tc2104sdSwErrOverMonitor(unsigned char port_num);
void tc2104sdSwDispMessage(unsigned char port_num);
...
// 2. changing register setting
static void 
tc2104sdCfgLoad(unsigned char idx, unsigned char doPerPort_flag, unsigned char port_num)
{
    int pn, i;
...
static void 
tc2104sdLRCfgLoad(unsigned char idx, unsigned char port_num)
{
	int i;
	current_idx = idx;
...
int32
tc2104sdReadProbe(unsigned char port_num, unsigned char mode)
{
...
int32
tc2104sdReadErrOverSum(unsigned char port_num)
{
    int32 err_over_sum;
...
int32
tc2104sdReadAdcSum(unsigned char port_num)
{
    int32 cnt=1000;
    int32 AdcSign_sum = 0;
    int j;
...
int32
tc2104sdReadSnrSum(unsigned char port_num)
{
    int32 cnt=1000;
    int32 snr_sum = 0;
    int j;
...
void 
tc2104sdDispProbe100(unsigned char port_num, bool long_msg)
{
#ifdef TCPHY_DEBUG
    TCPHYDISP2("tcphy[%d]:",port_num);
...
#ifdef TCPHY_DEBUG
void
tc2104sdSwErrOverMonitor(unsigned char port_num)
{
    int32 val;
...
void
tc2104sdSwDispMessage(unsigned char port_num)
{
    switch (tcphy_link_state[port_num]) {
        case ST_LINK_DOWN: 
            break;
...
static unsigned char tc2101me_FixNonAnPairSwap_flag = 1;// default: enable

//for sw_FixNonAnPairSwapstatic 
unsigned char  tc2101me_force_crossover = 0;
static unsigned char  tc2101me_force_crossover_next = 0;
static uint16 tc2101me_force_crossover_cnt = 0;
static unsigned char  tc2101me_writeLR26_flag = 0;
static uint16 tc2101me_nosig_cnt = 0;

static int16 tc2101me_Up2Down_cnt_hbt = 0;
static unsigned char tc2101me_Up2DownFastHbt_detect = 0;
static bool tc2101me_link_fail_detect_flag = 0;
//Disable EEE if link fail
static int16 tc2101me_Up2Down_cnt_tbt = 0;
static unsigned char tc2101me_Up2DownFastTbt_detect = 0;
static unsigned char tc2101me_linkup_check_timer = 0;
static bool tc2101meDisEEEHappened = 0;
static unsigned char tc2101me_linkup_check_cnt = 0;

static unsigned char tc2101meDisNextPage_cnt = 0;
static bool tc2101meDisNextPage_flag = 0;


static void tc2101meConfig(int argc, char *argv[], void *p);
static void tc2101meCfgLoad(unsigned char idx);
int32 tc2101meReadProbe(unsigned char port_num, unsigned char mode);
int32 tc2101meReadErrOverSum(unsigned char port_num);
int32 tc2101meReadAdcSum(unsigned char port_num);
int32 tc2101meReadSnrSum(unsigned char port_num);
void tc2101meDispProbe100(unsigned char port_num);

static void tc2101meCfgTx10AmpSave(void);
void tc2101meSwErrOverMonitor(void);
void tc2101meSwDispMessage(unsigned char port_num);
void tc2101meMdixForce(unsigned char mode);
void tc2101meSwFixNonAnPairSwap(void);
//Auto-Disable-NextPage patch 
void tc2101meUp2DownFastHbt(unsigned char port_num);
void tc2101meUp2DownFastTbt(unsigned char port_num);
bool tc2101meLinkFailDetect (unsigned char port_num, unsigned char mode);//0:10BT or 100BT, 1:100BT only, 2:10BT only
void tc2101meDisEEENeeded(unsigned char port_num);
void tc2101meDisNextPageNeeded(unsigned char port_num);
...
// 2. changing register setting
static void 
tc2101meCfgLoad(unsigned char idx)
{
    int i;
    current_idx = idx;
...
int32
tc2101meReadProbe(unsigned char port_num, unsigned char mode)
{
    uint32 val, val_r31, val_g0r28;
    uint32 rval, wval;
    uint32 phyaddr = mac_p->enetPhyAddr + port_num;
...
int32
tc2101meReadErrOverSum(unsigned char port_num)
{
    int32 err_over_sum;
...
int32
tc2101meReadAdcSum(unsigned char port_num)
{
    int32 cnt=1000;
    int32 AdcSign_sum = 0;
    int j;
...
int32
tc2101meReadSnrSum(unsigned char port_num)
{
    int32 cnt=1000;
    int32 snr_sum = 0;
    int j;
...
void 
tc2101meDispProbe100(unsigned char port_num)
{
#ifdef TCPHY_DEBUG
    TCPHYDISP2("tcphy[%d]:",port_num);
...
void
tc2101meSwDispMessage(unsigned char port_num)
{
    switch (tcphy_link_state[port_num]) {
        case ST_LINK_DOWN: 
            break;
...
void 
tc2101meMdixForce(unsigned char mode)
{
    const unsigned char port_num = 0;
	int32 val;
	val = tcPhyReadLReg(port_num, 0, 26);
...
void 
tc2101meUp2DownFastHbt(unsigned char port_num)
{
	const unsigned char tc2101me_Up2Down_Hbt_Thd = 10;
	const unsigned char tc2101me_Up2Down_Hbt_ub = 40;
    if (mr28.lch_linkup_100 & !Nmr1[port_num].link_status) {
...
void 
tc2101meUp2DownFastTbt(unsigned char port_num)
{
	const unsigned char tc2101me_Up2Down_Tbt_Thd = 10;
	const unsigned char tc2101me_Up2Down_Tbt_ub = 40;
	const unsigned char linkup_check_timer_done = 1;
...
bool 
tc2101meLinkFailDetect (unsigned char port_num, unsigned char mode){
	switch(mode){
			case TbtOrHbt:
				    if (sw_FixUp2DownFast_flag 
...
void 
tc2101meDisNextPageNeeded(unsigned char port_num)
{
   const unsigned char tc2101meDisNextPage_cntdone = 5;
...
void 
tc2101meDisEEENeeded(unsigned char port_num)
{
    const unsigned char tc2101me_linkup_check_done = 1;
...
//static unsigned char tc2102me_Up2DownFast_detect = 0;
//static bool tc2102me_link_fail_detect_flag = 0;

static int16 tc2102me_Up2Down_cnt_hbt = 0;
static unsigned char tc2102me_Up2DownFastHbt_detect = 0;
static bool tc2102me_link_fail_detect_flag = 0;
//Disable EEE if link fail
static int16 tc2102me_Up2Down_cnt_tbt = 0;
static unsigned char tc2102me_Up2DownFastTbt_detect = 0;
static unsigned char tc2102me_linkup_check_timer = 0;
static bool tc2102meDisEEEHappened = 0;
static unsigned char tc2102me_linkup_check_cnt = 0;

static unsigned char tc2102meDisNextPage_cnt = 0;
static bool tc2102meDisNextPage_flag = 0;

//static unsigned char tc2102me_reset_needed_flag = 0;
static unsigned char tc2102me_esdphy_init_flag = 1; // default: enable

static void tc2102meConfig(int argc, char *argv[], void *p);
void tc2102meCfgLoad(unsigned char idx);
static void tc2102meLRCfgLoad(unsigned char idx, unsigned char port_num);
int32 tc2102meReadProbe(unsigned char port_num, unsigned char mode);
static void tc2102meCfgTx10AmpSave(void);
static void tc2102meCfgLDPS(void);
#ifdef TCPHY_DEBUG
int32 tc2102meReadErrOverSum(unsigned char port_num);
int32 tc2102meReadAdcSum(unsigned char port_num);
void tc2102meDispProbe100(unsigned char port_num, bool long_msg);
void tc2102mePoweDown(unsigned char port_num);
void tc2102meSwDispMessage(unsigned char port_num);
int32 tc2102meReadSnrSum(unsigned char port_num, uint16 cnt);
void tc2102meUpdateErrOverSum(unsigned char port_num);
void tc2102meSwErrOverMonitor(unsigned char port_num);
bool tc2102meInitialRegFlag(void);
void tc2102meEsdSwPatch(void);
#endif
//void tc2102meUp2DownFast(unsigned char port_num);
//bool tc2102meLinkFailDetect (void);

void tc2102meUp2DownFastHbt(unsigned char port_num);
void tc2102meUp2DownFastTbt(unsigned char port_num);
bool tc2102meLinkFailDetect (unsigned char port_num, unsigned char mode);//0:10BT or 100BT, 1:100BT only, 2:10BT only
void tc2102meDisEEENeeded(unsigned char port_num);
void tc2102meDisNextPageNeeded(unsigned char port_num);
...
// 2. changing register setting
void 
tc2102meCfgLoad(unsigned char idx)
{
    int i;
    uint16 phyAddr=mac_p->enetPhyAddr;
	current_idx = idx;
...
static void
tc2102meLRCfgLoad(unsigned char idx, unsigned char port_num)
{
    int i;
	current_idx = idx;
...
int32
tc2102meReadProbe(unsigned char port_num, unsigned char mode)
{
    uint32 val, val_r31, val_g0r28;
    uint32 rval, wval;
    uint32 phyaddr = mac_p->enetPhyAddr + port_num;
...
#ifdef TCPHY_DEBUG
int32
tc2102meReadErrOverSum(unsigned char port_num)
{
    int32 err_over_sum;
...
#ifdef TCPHY_DEBUG
int32
tc2102meReadAdcSum(unsigned char port_num)
{
    int32 cnt=1000;
    int32 AdcSign_sum = 0;
    int j;
...
#ifdef TCPHY_DEBUG
int32
tc2102meReadSnrSum(unsigned char port_num, uint16 cnt)
{
    uint16 snr_sum = 0;
    uint16 j;
...
#ifdef TCPHY_DEBUG
void 
tc2102meDispProbe100(unsigned char port_num, bool long_msg)
{
...
#ifdef TCPHY_DEBUG
void
tc2102meSwDispMessage(unsigned char port_num)
{
	uint16 phyAddr = mac_p->enetPhyAddr;
    switch (tcphy_link_state[port_num]) {
        case ST_LINK_DOWN: 
...
#ifdef TCPHY_DEBUG
void
tc2102mePoweDown(unsigned char port_num)
{
    uint32 val_l0r04;
	val_l0r04 = tcPhyReadLReg(port_num, 0, 4);
	tcPhyWriteLReg(port_num,0,4,0x0001); // 	Set FLP register to disable all capability
...
#ifdef TCPHY_DEBUG
void
tc2102meUpdateErrOverSum(unsigned char port_num)
{
    uint16 err_over_cnt_prev; // to store previous err_over_cnt value
    int32 val;
...
#ifdef TCPHY_DEBUG
void
tc2102meSwErrOverMonitor(unsigned char port_num)
{
    if (tcPhy_disp_level<3) 
        return; // inactive
...
/*
void 
tc2102meUp2DownFast(unsigned char port_num)
{
	const unsigned char tc2102me_Up2Down_Thd = 10;
	const unsigned char tc2102me_Up2Down_ub = 40;
    if (mr28.lch_linkup_100 & !Nmr1[port_num].link_status) {
...
void 
tc2102meUp2DownFastHbt(unsigned char port_num)
{
	const unsigned char tc2102me_Up2Down_Hbt_Thd = 10;
	const unsigned char tc2102me_Up2Down_Hbt_ub = 40;
    if (mr28.lch_linkup_100 & !Nmr1[port_num].link_status) {
...
void 
tc2102meUp2DownFastTbt(unsigned char port_num)
{
	const unsigned char tc2102me_Up2Down_Tbt_Thd = 10;
	const unsigned char tc2102me_Up2Down_Tbt_ub = 40;
	const unsigned char linkup_check_timer_done = 1;
    if (mr28.lch_linkup_10 & !Nmr1[port_num].link_status) {
...
bool 
tc2102meLinkFailDetect (unsigned char port_num, unsigned char mode){
	switch(mode){
			case TbtOrHbt:
				    if (sw_FixUp2DownFast_flag 
...
void 
tc2102meDisNextPageNeeded(unsigned char port_num)
{
   const unsigned char tc2102meDisNextPage_cntdone = 5;
...
void 
tc2102meDisEEENeeded(unsigned char port_num)
{
    const unsigned char tc2102me_linkup_check_done = 1;
...
static unsigned char tc2104me_Up2DownFastHbt_detect[4] = {0,0,0,0};
static bool tc2104me_link_fail_detect_flag = 0;
static unsigned char tc2104me_reset_needed_perport[4] = {0,0,0,0};
static unsigned char tc2104me_reset_needed_flag = 0;
static unsigned char tc2104me_esdphy_init_flag = 1; // default: enable

//Disable EEE if link fail
static int16 tc2104me_Up2Down_cnt_tbt[4] = {0,0,0,0};
static unsigned char tc2104me_Up2DownFastTbt_detect[4] = {0,0,0,0};
static unsigned char tc2104me_linkup_check_timer[4] = {0,0,0,0};
static bool tc2104meDisEEEHappened[4] = {0,0,0,0};
static unsigned char tc2104me_linkup_check_cnt[4] = {0,0,0,0};

static unsigned char tc2104meDisNextPage_cnt[4] = {0,0,0,0};
...
void tc2104meCfgLoad(unsigned char idx, unsigned char doPerPort_flag, unsigned char port_num);
void tc2104meGRCfgCheck(void);
void tc2104meLRCfgCheck(unsigned char port_num);
void tc2104meCfgCheck(void);
static void tc2104meLRCfgLoad(unsigned char idx, unsigned char port_num);
static void tc2104mePerPortCfgLoad(unsigned char idx, unsigned char port_num);
int32 tc2104meReadProbe(unsigned char port_num, unsigned char mode);
static void tc2104meCfgTx10AmpSave(void);
static void tc2104meCfgLDPS(void);
#ifdef TCPHY_DEBUG
int32 tc2104meReadErrOverSum(unsigned char port_num);
int32 tc2104meReadAdcSum(unsigned char port_num);
void tc2104meDispProbe100(unsigned char port_num, bool long_msg);
void tc2104mePoweDown(unsigned char port_num);
void tc2104meSwDispMessage(unsigned char port_num);
uint16 tc2104meReadSnrSum(unsigned char port_num, uint16 cnt);
void tc2104meUpdateErrOverSum(unsigned char port_num);
void tc2104meSwErrOverMonitor(unsigned char port_num);
bool tc2104meInitialRegFlag(void);
void tc2104meEsdSwPatch(void);
#endif
void tc2104meUp2DownFastHbt(unsigned char port_num);
void tc2104meUp2DownFastTbt(unsigned char port_num);
bool tc2104meLinkFailDetect (unsigned char port_num, unsigned char mode);//0:10BT or 100BT, 1:100BT only, 2:10BT only
void tc2104meDisEEENeeded(unsigned char port_num);
void tc2104meDisNextPageNeeded(unsigned char port_num);
...
// 2. changing register setting
void 
tc2104meCfgLoad(unsigned char idx, unsigned char doPerPort_flag, unsigned char port_num)
{
    int pn, i;
    uint16 phyAddr;
	current_idx = idx;
...
void 
tc2104meLRCfgCheck(unsigned char port_num)
{
	 uint32 phyaddr = mac_p->enetPhyAddr+port_num;
	 uint32 rval;
...
static void
tc2104meLRCfgLoad(unsigned char idx, unsigned char port_num)
{
    int i;
	current_idx = idx;
...
static void 
tc2104mePerPortCfgLoad(unsigned char idx, unsigned char port_num)
{
   int i;
   current_idx = idx;
...
int32
tc2104meReadProbe(unsigned char port_num, unsigned char mode)
{
    uint32 val, val_r31, val_g0r28;
    uint32 rval, wval;
    uint32 phyaddr = mac_p->enetPhyAddr + port_num;
...
#ifdef TCPHY_DEBUG
int32
tc2104meReadErrOverSum(unsigned char port_num)
{
    int32 err_over_sum;
...
#ifdef TCPHY_DEBUG
int32
tc2104meReadAdcSum(unsigned char port_num)
{
    int32 cnt=1000;
    int32 AdcSign_sum = 0;
    int j;
...
#ifdef TCPHY_DEBUG
uint16
tc2104meReadSnrSum(unsigned char port_num, uint16 cnt)
{
    uint16 snr_sum = 0;
    uint16 j;
...
#ifdef TCPHY_DEBUG
void 
tc2104meDispProbe100(unsigned char port_num, bool long_msg)
{
...
#ifdef TCPHY_DEBUG
void
tc2104meSwDispMessage(unsigned char port_num)
{
    switch (tcphy_link_state[port_num]) {
        case ST_LINK_DOWN: 
            break;
...
#ifdef TCPHY_DEBUG
void
tc2104mePoweDown(unsigned char port_num)
{
    uint32 val_l0r04;
	val_l0r04 = tcPhyReadLReg(port_num, 0, 4);
	tcPhyWriteLReg(port_num,0,4,0x0001); // 	Set FLP register to disable all capability
...
#ifdef TCPHY_DEBUG
void
tc2104meUpdateErrOverSum(unsigned char port_num)
{
    uint16 err_over_cnt_prev; // to store previous err_over_cnt value
    int32 val;
...
#ifdef TCPHY_DEBUG
void
tc2104meSwErrOverMonitor(unsigned char port_num)
{
    if (tcPhy_disp_level<3) 
        return; // inactive
...
*/
void 
tc2104meUp2DownFastHbt(unsigned char port_num)
{
	const unsigned char tc2104me_Up2Down_Hbt_Thd = 10;
	const unsigned char tc2104me_Up2Down_Hbt_ub = 40;
    if (mr28.lch_linkup_100 & !Nmr1[port_num].link_status) {
...
void 
tc2104meUp2DownFastTbt(unsigned char port_num)
{
	const unsigned char tc2104me_Up2Down_Tbt_Thd = 10;
	const unsigned char tc2104me_Up2Down_Tbt_ub = 40;
	const unsigned char linkup_check_timer_done = 1;
    if (mr28.lch_linkup_10 & !Nmr1[port_num].link_status) {
...
bool 
tc2104meLinkFailDetect (unsigned char port_num, unsigned char mode){
	switch(mode){
			case TbtOrHbt:
				    if (sw_FixUp2DownFast_flag 
...
void 
tc2104meDisNextPageNeeded(unsigned char port_num)
{
   const unsigned char tc2104meDisNextPage_cntdone = 5;
...
void 
tc2104meDisEEENeeded(unsigned char port_num)
{
    const unsigned char tc2104me_linkup_check_done = 1;
...
void tc2104meSwPatch(void)
{
    ...
    for(pn=0; pn<TCPHY_PORTNUM; pn++) {
		port_state = tcphy_link_state[pn];
		sprintf(dev_name, "eth0.%d", pn + 1);
		dev = dev_get_by_name(&init_net, dev_name);
        tcPhyReadReg(pn,1);
        if( !Nmr1[pn].link_status_prev && !Nmr1[pn].link_status) {
			if(ST_LINK_DOWN != port_state) {
	            tcphy_link_state[pn] = ST_LINK_DOWN;
			...
			if(ST_LINK_DOWN2UP != port_state) {
				tcphy_link_state[pn] = ST_LINK_DOWN2UP;
			}
        }
        else if(Nmr1[pn].link_status_prev && !Nmr1[pn].link_status ) {
			if(ST_LINK_UP2DOWN != port_state) {
				tcphy_link_state[pn]= ST_LINK_UP2DOWN;
			}
		} else {
			if(ST_LINK_UP != port_state) {
				tcphy_link_state[pn] = ST_LINK_UP;
...
static void tc2101mfCfgLoad(unsigned char idx);
int32 tc2101mfReadProbe(unsigned char port_num, unsigned char mode);
int32 tc2101mfReadErrOverSum(unsigned char port_num);
int32 tc2101mfReadAdcSum(unsigned char port_num);
int32 tc2101mfReadSnrSum(unsigned char port_num);
void tc2101mfDispProbe100(unsigned char port_num);


void tc2101mfSwErrOverMonitor(void);
void tc2101mfSwDispMessage(unsigned char port_num);
...
// 2. changing register setting
static void 
tc2101mfCfgLoad(unsigned char idx)
{
    int i;
    current_idx = idx;
...
int32
tc2101mfReadProbe(unsigned char port_num, unsigned char mode)
{
    uint32 val, val_r31, val_g0r28;
    uint32 rval, wval;
    uint32 phyaddr = mac_p->enetPhyAddr + port_num;
...
int32
tc2101mfReadErrOverSum(unsigned char port_num)
{
    int32 err_over_sum;
...
int32
tc2101mfReadAdcSum(unsigned char port_num)
{
    int32 cnt=1000;
    int32 AdcSign_sum = 0;
    int j;
...
int32
tc2101mfReadSnrSum(unsigned char port_num)
{
    int32 cnt=1000;
    int32 snr_sum = 0;
    int j;
...
void 
tc2101mfDispProbe100(unsigned char port_num)
{
#ifdef TCPHY_DEBUG
    TCPHYDISP2("tcphy[%d]:",port_num);
...
void
tc2101mfSwDispMessage(unsigned char port_num)
{
    switch (tcphy_link_state[port_num]) {
        case ST_LINK_DOWN: 
            break;
...
static unsigned char tc2105sg_Up2DownFastHbt_detect[5] = {0,0,0,0,0};
static bool tc2105sg_link_fail_detect_flag = 0;
//static unsigned char tc2105sg_reset_needed_perport[5] = {0,0,0,0,0};
//static unsigned char tc2105sg_reset_needed_flag = 0;
//static unsigned char tc2105sg_esdphy_init_flag = 1; // default: enable

//Disable EEE if link fail
static int16 tc2105sg_Up2Down_cnt_tbt[5] = {0,0,0,0,0};
static unsigned char tc2105sg_Up2DownFastTbt_detect[5] = {0,0,0,0,0};
static unsigned char tc2105sg_linkup_check_timer[5] = {0,0,0,0,0};
static bool tc2105sgDisEEEHappened[5] = {0,0,0,0,0};
static unsigned char tc2105sg_linkup_check_cnt[5] = {0,0,0,0,0};

static unsigned char tc2105sgDisNextPage_cnt[5] = {0,0,0,0,0};
static bool tc2105sgDisNextPage_flag[5] = {0,0,0,0,0};

static void tc2105sgConfig(int argc, char *argv[], void *p);
void tc2105sgCfgLoad(unsigned char idx, unsigned char doPerPort_flag, unsigned char port_num);
void tc2105sgGRCfgCheck(void);
void tc2105sgLRCfgCheck(unsigned char port_num);
void tc2105sgCfgCheck(void);
static void tc2105sgLRCfgLoad(unsigned char idx, unsigned char port_num);
static void tc2105sgPerPortCfgLoad(unsigned char idx, unsigned char port_num);
int32 tc2105sgReadProbe(unsigned char port_num, unsigned char mode);
static void tc2105sgCfgTx10AmpSave(void);
//static void tc2105sgCfgLDPS(void);
#ifdef TCPHY_DEBUG
int32 tc2105sgReadErrOverSum(unsigned char port_num);
int32 tc2105sgReadAdcSum(unsigned char port_num);
void tc2105sgDispProbe100(unsigned char port_num, bool long_msg);
void tc2105sgPoweDown(unsigned char port_num);
void tc2105sgSwDispMessage(unsigned char port_num);
uint16 tc2105sgReadSnrSum(unsigned char port_num, uint16 cnt);
void tc2105sgUpdateErrOverSum(unsigned char port_num);
void tc2105sgSwErrOverMonitor(unsigned char port_num);
//bool tc2105sgInitialRegFlag(void);
//void tc2105sgEsdSwPatch(void);
#endif
void tc2105sgUp2DownFastHbt(unsigned char port_num);
void tc2105sgUp2DownFastTbt(unsigned char port_num);
bool tc2105sgLinkFailDetect (unsigned char port_num, unsigned char mode);//0:10BT or 100BT, 1:100BT only, 2:10BT only
void tc2105sgDisEEENeeded(unsigned char port_num);
void tc2105sgDisNextPageNeeded(unsigned char port_num);
...
// 2. changing register setting
void 
tc2105sgCfgLoad(unsigned char idx, unsigned char doPerPort_flag, unsigned char port_num)
{
    int pn, i;
    uint16 phyAddr;
	current_idx = idx;
...
void 
tc2105sgLRCfgCheck(unsigned char port_num)
{
	 uint32 phyaddr = mac_p->enetPhyAddr+port_num;
	 uint32 rval;
/*
...
static void
tc2105sgLRCfgLoad(unsigned char idx, unsigned char port_num)
{
    int i;
	current_idx = idx;
...
static void 
tc2105sgPerPortCfgLoad(unsigned char idx, unsigned char port_num)
{
   int i;
   current_idx = idx;
...
int32
tc2105sgReadProbe(unsigned char port_num, unsigned char mode)
{
    uint32 val, val_r31, val_g0r28;
    uint32 rval, wval;
    uint32 phyaddr = mac_p->enetPhyAddr + port_num;
...
#ifdef TCPHY_DEBUG
int32
tc2105sgReadErrOverSum(unsigned char port_num)
{
    int32 err_over_sum;
...
#ifdef TCPHY_DEBUG
int32
tc2105sgReadAdcSum(unsigned char port_num)
{
    int32 cnt=1000;
    int32 AdcSign_sum = 0;
    int j;
...
#ifdef TCPHY_DEBUG
uint16
tc2105sgReadSnrSum(unsigned char port_num, uint16 cnt)
{
    uint16 snr_sum = 0;
    uint16 j;
...
#ifdef TCPHY_DEBUG
void 
tc2105sgDispProbe100(unsigned char port_num, bool long_msg)
{
...
#ifdef TCPHY_DEBUG
void
tc2105sgSwDispMessage(unsigned char port_num)
{
    switch (tcphy_link_state[port_num]) {
        case ST_LINK_DOWN: 
            break;
...
#ifdef TCPHY_DEBUG
void
tc2105sgPoweDown(unsigned char port_num)
{
    uint32 val_l0r04;
	val_l0r04 = tcPhyReadLReg(port_num, 0, 4);
	tcPhyWriteLReg(port_num,0,4,0x0001); // 	Set FLP register to disable all capability
...
#ifdef TCPHY_DEBUG
void
tc2105sgUpdateErrOverSum(unsigned char port_num)
{
    uint16 err_over_cnt_prev; // to store previous err_over_cnt value
    int32 val;
...
#ifdef TCPHY_DEBUG
void
tc2105sgSwErrOverMonitor(unsigned char port_num)
{
    if (tcPhy_disp_level<3) 
        return; // inactive
...
*/
void 
tc2105sgUp2DownFastHbt(unsigned char port_num)
{
	const unsigned char tc2105sg_Up2Down_Hbt_Thd = 10;
	const unsigned char tc2105sg_Up2Down_Hbt_ub = 40;
    if (mr28.lch_linkup_100 & !Nmr1[port_num].link_status) {
...
void 
tc2105sgUp2DownFastTbt(unsigned char port_num)
{
	const unsigned char tc2105sg_Up2Down_Tbt_Thd = 10;
	const unsigned char tc2105sg_Up2Down_Tbt_ub = 40;
	const unsigned char linkup_check_timer_done = 1;
    if (mr28.lch_linkup_10 & !Nmr1[port_num].link_status) {
...
bool 
tc2105sgLinkFailDetect (unsigned char port_num, unsigned char mode){
	switch(mode){
			case TbtOrHbt:
				    if (sw_FixUp2DownFast_flag 
...
void 
tc2105sgDisNextPageNeeded(unsigned char port_num)
{
   const unsigned char tc2105sgDisNextPage_cntdone = 5;
...
void 
tc2105sgDisEEENeeded(unsigned char port_num)
{
    const unsigned char tc2105sg_linkup_check_done = 1;
...
void tc2101miCfgLoad(unsigned char idx);
void tc2101miGRCfgCheck(void);
void tc2101miLRCfgCheck(unsigned char port_num);
void tc2101miCfgCheck(void);
static void tc2101miLRCfgLoad(unsigned char idx, unsigned char port_num);
static void tc2101miPerPortCfgLoad(unsigned char idx, unsigned char port_num);
int32 tc2101miReadProbe(unsigned char port_num, unsigned char mode);
static void tc2101miCfgTx10AmpSave(void);
//static void tc2105sgCfgLDPS(void);
#ifdef TCPHY_DEBUG
int32 tc2101miReadErrOverSum(unsigned char port_num);
int32 tc2101miReadAdcSum(unsigned char port_num);
void tc2101miDispProbe100(unsigned char port_num, bool long_msg);
void tc2101miPoweDown(unsigned char port_num);
void tc2101miSwDispMessage(unsigned char port_num);
uint16 tc2101miReadSnrSum(unsigned char port_num, uint16 cnt);
void tc2101miUpdateErrOverSum(unsigned char port_num);
void tc2101miSwErrOverMonitor(unsigned char port_num);
//bool tc2105sgInitialRegFlag(void);
//void tc2105sgEsdSwPatch(void);
#endif
...
// 2. changing register setting
void 
tc2101miCfgLoad(unsigned char idx)
{
    int i;
    uint16 phyAddr;
	current_idx = idx;
...
void 
tc2101miLRCfgCheck(unsigned char port_num)
{
	 uint32 phyaddr = mac_p->enetPhyAddr+port_num;
	 uint32 rval;
/*
...
static void
tc2101miLRCfgLoad(unsigned char idx, unsigned char port_num)
{
    int i;
	current_idx = idx;
...
static void 
tc2101miPerPortCfgLoad(unsigned char idx, unsigned char port_num)
{
   int i;
   current_idx = idx;
...
int32
tc2101miReadProbe(unsigned char port_num, unsigned char mode)
{
    uint32 val, val_r31, val_g0r28;
    uint32 rval, wval;
    uint32 phyaddr = mac_p->enetPhyAddr + port_num;
...
#ifdef TCPHY_DEBUG
int32
tc2101miReadErrOverSum(unsigned char port_num)
{
    int32 err_over_sum;
...
#ifdef TCPHY_DEBUG
int32
tc2101miReadAdcSum(unsigned char port_num)
{
    int32 cnt=1000;
    int32 AdcSign_sum = 0;
    int j;
...
#ifdef TCPHY_DEBUG
uint16
tc2101miReadSnrSum(unsigned char port_num, uint16 cnt)
{
    uint16 snr_sum = 0;
    uint16 j;
...
#ifdef TCPHY_DEBUG
void 
tc2101miDispProbe100(unsigned char port_num, bool long_msg)
{
...
#ifdef TCPHY_DEBUG
void
tc2101miSwDispMessage(unsigned char port_num)
{
    switch (tcphy_link_state[port_num]) {
        case ST_LINK_DOWN: 
            break;
...
#ifdef TCPHY_DEBUG
void
tc2101miPoweDown(unsigned char port_num)
{
    uint32 val_l0r04;
	val_l0r04 = tcPhyReadLReg(port_num, 0, 4);
	tcPhyWriteLReg(port_num,0,4,0x0001); // 	Set FLP register to disable all capability
...
#ifdef TCPHY_DEBUG
void
tc2101miUpdateErrOverSum(unsigned char port_num)
{
    uint16 err_over_cnt_prev; // to store previous err_over_cnt value
    int32 val;
...
#ifdef TCPHY_DEBUG
void
tc2101miSwErrOverMonitor(unsigned char port_num)
{
    if (tcPhy_disp_level<3) 
        return; // inactive
...
void tc2105mjCfgLoad(unsigned char idx, unsigned char doPerPort_flag, unsigned char port_num);
static void tc2105mjLRCfgLoad(unsigned char idx, unsigned char port_num);
static void tc2105mjCfgTx10AmpSave(void);
void tc2105mjGRCfgCheck(void);
void tc2105mjLRCfgCheck(unsigned char port_num);
void tc2105mjCfgCheck(void);
int32 tc2105mjReadProbe(unsigned char port_num, unsigned char mode);
#ifdef TCPHY_DEBUG
int32 tc2105mjReadErrOverSum(unsigned char port_num);
int32 tc2105mjReadAdcSum(unsigned char port_num);
void tc2105mjDispProbe100(unsigned char port_num, bool long_msg);
void tc2105mjSwDispMessage(unsigned char port_num);
uint16 tc2105mjReadSnrSum(unsigned char port_num, uint16 cnt);
void tc2105mjUpdateErrOverSum(unsigned char port_num);
void tc2105mjSwErrOverMonitor(unsigned char port_num);
#endif //TCPHY_DEBUG
...
// 2. changing register setting
void 
tc2105mjCfgLoad(unsigned char idx, unsigned char doPerPort_flag, unsigned char port_num)
{
    int pn,i;
    uint16 phyAddr;
	current_idx = idx;
...
static void
tc2105mjLRCfgLoad(unsigned char idx, unsigned char port_num)
{
    int i;
	current_idx = idx;
...
void 
tc2105mjLRCfgCheck(unsigned char port_num)
{
	 uint32 phyaddr = mac_p->enetPhyAddr+port_num;
	 uint32 rval;
/*
...
int32
tc2105mjReadProbe(unsigned char port_num, unsigned char mode)
{
    uint32 val, val_r31, val_g0r28;
    uint32 rval, wval;
    uint32 phyaddr = mac_p->enetPhyAddr + port_num;
...
#ifdef TCPHY_DEBUG
int32
tc2105mjReadErrOverSum(unsigned char port_num)
{
    int32 err_over_sum;
...
#ifdef TCPHY_DEBUG
int32
tc2105mjReadAdcSum(unsigned char port_num)
{
    int32 cnt=1000;
    int32 AdcSign_sum = 0;
    int j;
...
#ifdef TCPHY_DEBUG
uint16
tc2105mjReadSnrSum(unsigned char port_num, uint16 cnt)
{
    uint16 snr_sum = 0;
    uint16 j;
...
#ifdef TCPHY_DEBUG
void 
tc2105mjDispProbe100(unsigned char port_num, bool long_msg)
{
...
#ifdef TCPHY_DEBUG
void
tc2105mjSwDispMessage(unsigned char port_num)
{
    switch (tcphy_link_state[port_num]) {
        case ST_LINK_DOWN: 
            break;
...
#ifdef TCPHY_DEBUG
void
tc2105mjUpdateErrOverSum(unsigned char port_num)
{
    uint16 err_over_cnt_prev; // to store previous err_over_cnt value
    int32 val;
...
static unsigned char tc2105sk_Up2DownFastHbt_detect[5] = {0,0,0,0,0};
...
static unsigned char tc2105sk_Up2DownFastTbt_detect[5] = {0,0,0,0,0};
static unsigned char tc2105sk_linkup_check_timer[5] = {0,0,0,0,0};
static bool tc2105skDisEEEHappened[5] = {0,0,0,0,0};
static unsigned char tc2105sk_linkup_check_cnt[5] = {0,0,0,0,0};

static unsigned char tc2105skDisNextPage_cnt[5] = {0,0,0,0,0};
static bool tc2105skDisNextPage_flag[5] = {0,0,0,0,0};

static void tc2105skConfig(int argc, char *argv[], void *p);
void tc2105skCfgLoad(unsigned char idx, unsigned char doPerPort_flag, unsigned char port_num);
void tc2105skGRCfgCheck(void);
void tc2105skLRCfgCheck(unsigned char port_num);
void tc2105skCfgCheck(void);
static void tc2105skLRCfgLoad(unsigned char idx, unsigned char port_num);
static void tc2105skPerPortCfgLoad(unsigned char idx, unsigned char port_num);
int32 tc2105skReadProbe(unsigned char port_num, unsigned char mode);
static void tc2105skCfgTx10AmpSave(void);
//static void tc2105skCfgLDPS(void);
#ifdef TCPHY_DEBUG
int32 tc2105skReadErrOverSum(unsigned char port_num);
int32 tc2105skReadAdcSum(unsigned char port_num);
void tc2105skDispProbe100(unsigned char port_num, bool long_msg);
void tc2105skPoweDown(unsigned char port_num);
void tc2105skSwDispMessage(unsigned char port_num);
uint16 tc2105skReadSnrSum(unsigned char port_num, uint16 cnt);
void tc2105skUpdateErrOverSum(unsigned char port_num);
void tc2105skSwErrOverMonitor(unsigned char port_num);
//bool tc2105skInitialRegFlag(void);
//void tc2105skEsdSwPatch(void);
#endif
void tc2105skUp2DownFastHbt(unsigned char port_num);
void tc2105skUp2DownFastTbt(unsigned char port_num);
bool tc2105skLinkFailDetect (unsigned char port_num, unsigned char mode);//0:10BT or 100BT, 1:100BT only, 2:10BT only
void tc2105skDisEEENeeded(unsigned char port_num);
void tc2105skDisNextPageNeeded(unsigned char port_num);
...
// 2. changing register setting
void 
tc2105skCfgLoad(unsigned char idx, unsigned char doPerPort_flag, unsigned char port_num)
{
    int pn, i;
    uint16 phyAddr;
	current_idx = idx;
...
void 
tc2105skLRCfgCheck(unsigned char port_num)
{
	 uint32 phyaddr = mac_p->enetPhyAddr+port_num;
	 uint32 rval;
/*
...
static void
tc2105skLRCfgLoad(unsigned char idx, unsigned char port_num)
{
    int i;
	current_idx = idx;
	for( i=0; i<TC2105SG_PHY_INIT_LDATA_LEN; i++ ){         
...
static void 
tc2105skPerPortCfgLoad(unsigned char idx, unsigned char port_num)
{
   int i;
   current_idx = idx;
   for( i=0; i<TC2105SG_PHY_INIT_PERPDATA_LEN; i++ ){         
...
int32
tc2105skReadProbe(unsigned char port_num, unsigned char mode)
{
    uint32 val, val_r31, val_g0r28;
    uint32 rval, wval;
    uint32 phyaddr = mac_p->enetPhyAddr + port_num;
...
#ifdef TCPHY_DEBUG
int32
tc2105skReadErrOverSum(unsigned char port_num)
{
    int32 err_over_sum;
...
#ifdef TCPHY_DEBUG
int32
tc2105skReadAdcSum(unsigned char port_num)
{
    int32 cnt=1000;
    int32 AdcSign_sum = 0;
    int j;
...
#ifdef TCPHY_DEBUG
uint16
tc2105skReadSnrSum(unsigned char port_num, uint16 cnt)
{
    uint16 snr_sum = 0;
    uint16 j;
...
#ifdef TCPHY_DEBUG
void 
tc2105skDispProbe100(unsigned char port_num, bool long_msg)
{
...
#ifdef TCPHY_DEBUG
void
tc2105skSwDispMessage(unsigned char port_num)
{
    switch (tcphy_link_state[port_num]) {
        case ST_LINK_DOWN: 
            break;
...
#ifdef TCPHY_DEBUG
void
tc2105skPoweDown(unsigned char port_num)
{
    uint32 val_l0r04;
	val_l0r04 = tcPhyReadLReg(port_num, 0, 4);
	tcPhyWriteLReg(port_num,0,4,0x0001); // 	Set FLP register to disable all capability
...
#ifdef TCPHY_DEBUG
void
tc2105skUpdateErrOverSum(unsigned char port_num)
{
    uint16 err_over_cnt_prev; // to store previous err_over_cnt value
    int32 val;
...
#ifdef TCPHY_DEBUG
void
tc2105skSwErrOverMonitor(unsigned char port_num)
{
    if (tcPhy_disp_level<3) 
        return; // inactive
...
*/
void 
tc2105skUp2DownFastHbt(unsigned char port_num)
{
	const unsigned char tc2105sk_Up2Down_Hbt_Thd = 10;
	const unsigned char tc2105sk_Up2Down_Hbt_ub = 40;
    if (mr28.lch_linkup_100 & !Nmr1[port_num].link_status) {
...
void 
tc2105skUp2DownFastTbt(unsigned char port_num)
{
	const unsigned char tc2105sk_Up2Down_Tbt_Thd = 10;
	const unsigned char tc2105sk_Up2Down_Tbt_ub = 40;
	const unsigned char linkup_check_timer_done = 1;
    if (mr28.lch_linkup_10 & !Nmr1[port_num].link_status) {
...
bool 
tc2105skLinkFailDetect (unsigned char port_num, unsigned char mode){
	switch(mode){
			case TbtOrHbt:
				    if (sw_FixUp2DownFast_flag 
...
void 
tc2105skDisNextPageNeeded(unsigned char port_num)
{
   const unsigned char tc2105skDisNextPage_cntdone = 5;
...
void 
tc2105skDisEEENeeded(unsigned char port_num)
{
    const unsigned char tc2105sk_linkup_check_done = 1;
...
void phy100BT_dump(uint32 wait_time)
{
  ...
  // local device
  unsigned char  mr_anen;
  unsigned char  lr_dplx;
  // link-partner
...
int tcPhyInit(macAdapter_t * mac_p){
    ...
    for( i=start_addr; i<start_addr+tcPhyPortNum; i++ ){            
        tcMiiStationWrite(i, MII_BMCR, BMCR_RESET );
    }
    ...
    for( i=start_addr; i<start_addr+tcPhyPortNum; i++ ){            
        tcMiiStationWrite(i, MII_BMCR, (BMCR_ANRESTART | BMCR_ANENABLE) );
    }
...
// tcPhy initial: reset, load default register setting, restat AN
int tcPhyPortInit(unsigned char port_num){
    //miiStationWrite( mac_p, MII_BMCR, BMCR_RESET);
    #ifndef LINUX_OS
    tcMiiStationWrite(port_num, MII_BMCR, BMCR_RESET);
	#endif
    ...
    // always boot-up with AN-enable
    //miiStationWrite( mac_p, MII_BMCR, (BMCR_ANRESTART | BMCR_ANENABLE) );           
    tcMiiStationWrite(port_num, MII_BMCR, (BMCR_ANRESTART | BMCR_ANENABLE) );
...
unsigned char getTcPhyFlag(void){
    return tcPhyFlag;
}
...
uint32 getTcPhyStatusReg(macAdapter_t * mac_p){
    ...
    if (!sw_patch_flag)//if sw patch off, mac get PHY status via MDIO directly.
		return ( miiStationRead(mac_p, MII_BMSR) );
...
uint32 getTcPhyRMCAPReg(macAdapter_t * mac_p){
	...
	#endif
		return ( miiStationRead(mac_p, MII_LPA));	
}

uint16 getTcPhyNStatusReg(unsigned char port_num){
    uint16 phyAddr;

    if (!sw_patch_flag) {
        phyAddr = mac_p->enetPhyAddr + port_num;
        return ( tcMiiStationRead(phyAddr, MII_BMSR) ); 
    }
    else {
...
#ifdef TCPHY_DEBUG
unsigned char getTcPhyLookbackFlag(void){
    return phy_loopback_flag;
}
unsigned char getTcPhyForceLinkFlag(void){
...
doMacRegDump (int argc, char *argv[], void *p)
{
    ...
    const unsigned char phyAddr=22; // tc2206 mac
    unsigned char pn;
    uint32 wval;
    uint32 tx_pkt_cnt;
    uint32 rx_pkt_cnt;
...
#ifdef LINUX_OS
unsigned char phyMbpChk(struct sk_buff* oldMbp){
#else
unsigned char phyMbpChk(mbuf_t*oldMbp){
#endif
...
// called by macRxRingProc() for doPhyLoopback()
#ifdef LINUX_OS
unsigned char phy_recv_err_check(struct sk_buff** oldMbp, volatile macRxDescr_t *rxDescrp, uint32 *frameSize){
#ifdef TC2031_DEBUG
 #if  !(defined(CONFIG_MIPS_TC3182) ||  defined(CONFIG_MIPS_TC3262))
	int i = 0;
	if( phy_loopback_flag)
...
#else
unsigned char phy_recv_err_check(mbuf_t **oldMbp, macRxDescr_t *rxDescrp, uint32 *frameSize){
    #ifdef TC2031_DEBUG
    int i = 0;
    if( phy_loopback_flag)
    {
...
#endif
#ifndef LINUX_OS
static unsigned char mac_head[14]={0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0, 0, 0, 0, 0, 1, 0xff, 0xff};
...
static int doPhyLoopback (int argc, char *argv[], void *p)
{
    ...
    int len=60;
    int *seq;
    unsigned char random_num, send_fail_flag;
    ...
    while (j < loop) {
        ...
        mbp->data[14] = (unsigned char)((j>>8)&0x00ff);     
        mbp->data[15] = (unsigned char)(j&0x00ff);      
        for(i = 16; i < len; i++){  /* fill pattern */
            mbp->data[i] = (unsigned char)((random_num+i-16)&0xff);
        }
...
int doMacSend (int argc, char *argv[], void *p)
{
    ...
    pattern = (unsigned char) pattern32;
...
int doMacSendRandom (int argc, char *argv[], void *p)
{
    ...
    while (j < loop) {
        ...
        for(i = 14; i < len; i++){  /* fill pattern */
            mbp->data[i] = (unsigned char) (rand() + rand());
        }
        mbp->cnt = len;
...
)
{
	...
	z[3] = *((unsigned char *)&ul + 0);
	z[2] = *((unsigned char *)&ul + 1);
	z[1] = *((unsigned char *)&ul + 2);
	z[0] = *((unsigned char *)&ul + 3);
