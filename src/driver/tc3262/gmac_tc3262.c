/*
** $Id: //BBN_Linux/Branch/Branch_for_Kernel_Upgrade_20120517/tclinux_phoenix/modules/private/tc3262/gmac_tc3262.c#2 $
*/
/************************************************************************
 *
                 ...
                 int *eof, void *data)
{
	...
	for (port = 0; port <= 6; port++) {
		...
		index += sprintf(buf+index, "Bx 64 pkts            = 0x%08lx, ", gswPbusRead(MIB_BX_PKT_64_CNT + mib_offset));
		CHK_BUF();
		index += sprintf(buf+index, "Bx 65-127 pkts        = 0x%08lx\n", gswPbusRead(MIB_BX_PKT_65TO127_CNT + mib_offset));
		CHK_BUF();
		index += sprintf(buf+index, "Bx 128-255 pkts       = 0x%08lx, ", gswPbusRead(MIB_BX_PKT_128TO255_CNT + mib_offset));
		CHK_BUF();
		index += sprintf(buf+index, "Bx 256-511 pkts       = 0x%08lx\n", gswPbusRead(MIB_BX_PKT_256TO511_CNT + mib_offset));
		CHK_BUF();
		index += sprintf(buf+index, "Bx 512-1023 pkts      = 0x%08lx, ", gswPbusRead(MIB_BX_PKT_512TO1023_CNT + mib_offset));
		CHK_BUF();
		index += sprintf(buf+index, "Bx 1024-MAX pkts      = 0x%08lx\n", gswPbusRead(MIB_BX_PKT_1024TOMAX_CNT + mib_offset));
		CHK_BUF();
...
static int __init tc3262_gmac_init(void)
{
    ...
    /*
     * TBS_TAG:add by pengyao 20130411
