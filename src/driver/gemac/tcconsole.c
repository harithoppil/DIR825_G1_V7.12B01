/************************************************************************
 *
 *	Copyright (C) 2006 Trendchip Technologies, Corp.
 *	All Rights Reserved.
 *
 * Trendchip Confidential; Need to Know only.
 * Protected as an unpublished work.
 *
 * The computer program listings, specifications and documentation
 * herein are the property of Trendchip Technologies, Co. and shall
 * not be reproduced, copied, disclosed, or used in whole or in part
 * for any reason without the prior express written permission of
 * Trendchip Technologeis, Co.
 *
 *************************************************************************/
#include <linux/module.h>
#include <linux/kernel.h>
#include <linux/netdevice.h>
#include <linux/etherdevice.h>
#include <linux/spinlock.h>
#include <linux/interrupt.h>
#include <linux/signal.h>
#include <linux/sched.h>
#include <linux/mm.h>
#include <linux/init.h>
#include <linux/timer.h>
#include <linux/delay.h>
#include <linux/proc_fs.h>
#include <linux/dma-mapping.h>
#include <linux/mii.h>
#include <asm/io.h>
#include <asm/tc3162/tc3162.h>
#include <asm/tc3162/ledcetrl.h>
#include <asm/tc3162/TCIfSetQuery_os.h>
#include <linux/if_vlan.h>
#include "gemac.h"
#include "smi.h"

#define MAX_VLAN_LAYER          2
#define MAC_RTS_START           0x01
#define MAC_RTS_RESPONSE        0x02
#define MAC_RTS_STOP            0x03
#define MAC_RTS_ON              0x04
#define MAC_RTS_CONSOLE_ON      0x05
#define MAC_RTS_CONSOLE_CMD     0x06
#define MAC_RTS_CONSOLE_DATA    0x07


#ifdef TC_CONSOLE_ENABLE
static unsigned char dmt_NewVersion = 0;
static struct net_device *tcc_dev = NULL;
static unsigned char tcc_mac_addr[] = {0x00, 0x00, 0xaa, 0xbb, 0xcc, 0xff};
static char tcc_cmd[1024] = {0};
static atomic_t tcconsole_timeout;
static struct timer_list tcc_timer;
extern adsldev_ops *adsl_dev_ops; /* defined in arch/mips/ralink/setup.c */
extern void tcconsole_cmd(char* cmd, int len); /* defined in drivers/serial/tc3162_uart.c */
extern struct net_device *switch_dev[SWITCH_PORT_MAX]; /* defined in gemac.c */

/* ADSL RTS dump function */
void getTcconsoleFile(void)
{
	#if defined(CONFIG_MIPS_TC3182) ||  defined(CONFIG_MIPS_TC3262)
	dmt_NewVersion = 1; /*For tcconsole, function prototype of MAC and DMT have to be matched*/
	#else
	struct file *filp;
	
	filp = filp_open("/proc/tcconsole.tmp", O_RDONLY, 0);
	if(!IS_ERR(filp)){  /* New dmt version will create tcconsole.tmp */
		filp_close(filp, NULL);
	 	dmt_NewVersion = 1; /*For tcconsole, function prototype of MAC and DMT have to be matched*/
	}
	else
	 	dmt_NewVersion = 0;
	#endif
}

/*_____________________________________________________________________________
**      function descriptions:
**		Find out the tc console proto index of socket buffer that we must to 
**		skip the vlan header.
**
**      parameters:
**         skb: socket buffer to send
**         vlan_level: VLan level
**             
**      global:
**         None
**             
**      return:
**		   The index of tc console protocl.     
**	     
**      call:
**         None
**      		
**      revision:
**      1. Here 2010/03/18
**____________________________________________________________________________
*/
int tcconsole_pro_idx(struct sk_buff *skb, int* vlan_level)
{
	int i=0;
	int proto_idx=0;
	int idx = (VLAN_ETH_ALEN<<1); /*VLAN_ETH_ALEN * 2=12*/
	*vlan_level=0;
	proto_idx=idx;
	for(i=1; i<=MAX_VLAN_LAYER; i++){
		if(skb->data[idx]==0x81 && skb->data[idx+1]==0x00) {
			*vlan_level=i;
			idx+=VLAN_HLEN;
		} else {
			break;
		}
	}
	if(*vlan_level!=0) {
		proto_idx += (VLAN_HLEN<<(*vlan_level-1));
	}
	
	return proto_idx; 
}


/*_____________________________________________________________________________
**      function descriptions:
**      	The periodic check the tcconsole packet is timeout or not, if there
**      is no tcconsole packet to send and timeout occurt, we will be disable the
**		uart message to send ethernet.
**   
**      parameters:
**         None
**             
**      global:
**         tcc_uart_timeout
**             
**      return:
**         None
**	     
**      call:
**   	   None
** 
**      revision:
**      1. Here 2010/09/23
**____________________________________________________________________________
*/
void tcconsole_chk(unsigned long data)
{	
	if(atomic_dec_and_test(&tcconsole_timeout)) {
		return;
	} else {/* Send message enabled.*/	
		mod_timer(&tcc_timer, jiffies + HZ); /* Counting down */
	}
}

int tcconsole_proc(struct sk_buff *skb)
{
	int tc_pro_idx=0;
	int tcc_cmd_idx=0;
	int rm_len=0;
	int vlan_level=0;
	
	tc_pro_idx=tcconsole_pro_idx(skb, &vlan_level);
	if((skb->data[tc_pro_idx] == 0xaa) && (skb->data[tc_pro_idx+1] == 0xaa)) {		
		if(adsl_dev_ops) {
			tcc_cmd_idx = tc_pro_idx + 2;
			switch(skb->data[tcc_cmd_idx]) {
				case MAC_RTS_START:
					getTcconsoleFile();
					memcpy(tcc_mac_addr,skb->data+ETH_ALEN, ETH_ALEN);/*Store the tcconsole client mac address*/
					tcc_dev = skb->dev; /* Store the tcconsole client port */
					if(atomic_read(&tcconsole_timeout) > 0) {
						atomic_set(&tcconsole_timeout, 720);
					}
					break;
				case MAC_RTS_STOP:
					del_timer_sync(&tcc_timer);
					atomic_set(&tcconsole_timeout, 0);
					break;
				case MAC_RTS_CONSOLE_ON:
					atomic_set(&tcconsole_timeout, 720);/* Default is 3 mins = 3x60x4 */
					mod_timer(&tcc_timer, jiffies + HZ);
					break;
				case MAC_RTS_CONSOLE_CMD:
					memcpy(tcc_cmd, skb->data + tcc_cmd_idx + 1, skb->len - tcc_cmd_idx);
					tcconsole_cmd(tcc_cmd, strlen(tcc_cmd));
					if(atomic_read(&tcconsole_timeout) > 0) {
						atomic_set(&tcconsole_timeout, 720);
					}
					break;
				default:
					if(atomic_read(&tcconsole_timeout) > 0) {
						atomic_set(&tcconsole_timeout, 720);
					}
					break;
			}
			/*if there is inclued vlan header, we must be strip the vlan header before to send upper layer*/
			if((vlan_level==1) || (vlan_level==2)){
				rm_len= (VLAN_HLEN<<(vlan_level-1));
				memmove(skb->data+rm_len, skb->data, (VLAN_ETH_ALEN<<1));
				skb_pull(skb, rm_len);
			}	
			adsl_dev_ops->rts_rcv((struct sk_buff *)skb->data);
		}
		dev_kfree_skb_any(skb);
		return 1;
	} else {
		return 0;
	}
	
}

/*_____________________________________________________________________________
**      function descriptions:
**			Encapsulate the console message to tcconsole packet then send to 
**		tcconsole utilty.
**   
**      parameters:
**         msg: console message
**         len: length of console mesage
**             
**      global:
**         tcc_uart_timeout
**             
**      return:
**         None
**	     
**      call:
**   	   skbmgr_dev_alloc_skb2k
**   	   tc3162_mac_tx
**   	   tc3262_gmac_tx
** 
**      revision:
**      1. Here 2010/09/26
**____________________________________________________________________________
*/
void uart_msg_to_tcconsole(char *msg, int len)
{
	char console_data_head[15] = {0};
	int tcc_pro_len = sizeof(console_data_head);
	int pktLen = 0;
	struct sk_buff *skb_tmp = NULL;
	
	if(atomic_read(&tcconsole_timeout) > 0) {
		skb_tmp= skbmgr_dev_alloc_skb2k();
		if((skb_tmp != NULL) && (NULL != tcc_dev)) {
			memcpy(console_data_head, tcc_mac_addr, ETH_ALEN);
			memcpy(console_data_head+ETH_ALEN, tcc_dev->dev_addr, ETH_ALEN);
			console_data_head[12]=0xaa;
			console_data_head[13]=0xaa;
			console_data_head[14]=MAC_RTS_CONSOLE_DATA;
			memcpy(skb_tmp->data, console_data_head, tcc_pro_len);
			memcpy(skb_tmp->data + tcc_pro_len, msg, len);
			pktLen=tcc_pro_len +len;
			if(pktLen < ETH_ZLEN){
				memset(skb_tmp->data+ pktLen, 0, ETH_ZLEN - pktLen);
				pktLen=ETH_ZLEN;
			}
			skb_put(skb_tmp, pktLen);
			tc3262_gmac_tx(skb_tmp, tcc_dev);
		}
	}
}
EXPORT_SYMBOL_GPL(uart_msg_to_tcconsole);

void macSend(unsigned int chanId, struct sk_buff *skb)
{
	struct sk_buff *skb_tmp;
	dmtConsoleData_t *dmt_data;

	if(NULL == tcc_dev) {
		tcc_dev = switch_dev[SWITCH_PORT0];
	}
    if((dmt_NewVersion) && (chanId != WAN2LAN_CH_ID)) {
		dmt_data = (dmtConsoleData_t *)skb;
		skb_tmp = dev_alloc_skb(0x700);
		if(skb_tmp != NULL){
			memcpy(skb_put(skb_tmp, dmt_data->len), dmt_data->data, dmt_data->len);
			#if  defined(CONFIG_MIPS_TC3182) ||  defined(CONFIG_MIPS_TC3262)
			tc3262_gmac_tx(skb_tmp, tcc_dev);
			#else
			tc3162_mac_tx(skb_tmp, tc3162_mac_dev);
			#endif
		}
	} else {
		#if  defined(CONFIG_MIPS_TC3182) ||  defined(CONFIG_MIPS_TC3262)
		tc3262_gmac_tx(skb, tcc_dev);
		#else
		tc3162_mac_tx(skb, tc3162_mac_dev);
		#endif
	}
}

static int eth_tcc_timeout_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len;

	len = sprintf(page, "%04d\n", atomic_read(&tcconsole_timeout));
	len -= off;
	*start = page + off;

	if(len > count) {
		len = count;
	} else {
		*eof = 1;
	}
	if(len < 0) {
		len = 0;
	}
	return len;
}

static int eth_tcc_timeout_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char val_string[32];
	int t = 0;

	if(count > sizeof(val_string) - 1) {
		return -EINVAL;
	}
	if(copy_from_user(val_string, buffer, count)) {
		return -EFAULT;
	}
	val_string[count] = '\0';
	t = simple_strtoul(val_string, NULL, 10);
	if(t < 1) {/* stop the counting down timer */
		del_timer_sync(&tcc_timer);
	}
	atomic_set(&tcconsole_timeout, t);
	
	return count;
}

void create_tcconsole_proc(void)
{
	struct proc_dir_entry *eth_proc;
	
	rcu_assign_pointer(send_uart_msg, uart_msg_to_tcconsole);
	setup_timer(&tcc_timer, tcconsole_chk, 0);
	eth_proc = create_proc_entry("tc3162/tcconsole_timeout", 0, NULL);
	if(NULL != eth_proc) {
		eth_proc->read_proc = eth_tcc_timeout_read_proc;
		eth_proc->write_proc = eth_tcc_timeout_write_proc;
	}
}

void delete_tcconsole_proc(void)
{
	rcu_assign_pointer(send_uart_msg, NULL);
	del_timer_sync(&tcc_timer);
	remove_proc_entry("tc3162/tcconsole_timeout", 0);
}

#else

void macSend(unsigned int chanId, struct sk_buff *skb)
{
	#if defined(CONFIG_MIPS_TC3182) ||  defined(CONFIG_MIPS_TC3262)
	tc3262_gmac_tx(skb, switch_dev[SWITCH_PORT0]);
	#else
	tc3162_mac_tx(skb, tc3162_mac_dev);
	#endif
}
#endif
EXPORT_SYMBOL(macSend);
