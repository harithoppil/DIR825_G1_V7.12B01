...
typedef struct
{
    unsigned int value[RTK_DOT_1AS_TIMESTAMP_UNIT_IN_WORD_LENGTH];
...
typedef struct
{
	...
	/* CVLAN acton */
	unsigned int 		filterIngressCvlanIdx;
	unsigned int     	filterIngressCvlanVid;
	unsigned int 		filterCvlanIdx;
	/* SVLAN action */
	unsigned int     	filterEgressSvlanIdx;
    unsigned int      filterSvlanVid;
	unsigned int 		filterSvlanIdx;

	/* Policing action */
	unsigned int     	filterPolicingIdx[FILTER_POLICING_MAX];

	/* Forwarding action */
    unsigned int      filterRedirectPortmask;
    unsigned int      filterAddDstPortmask;
	unsigned int 		filterPortmask;

	/* QOS action */
    unsigned int      filterPriority;
...
typedef unsigned int rtk_filter_op_t;
...
typedef struct
{
    unsigned int addr[RTK_IPV6_ADDR_WORD_LENGTH];
...
typedef unsigned int rtk_filter_number_t;

typedef struct rtk_filter_pattern_s
{
    unsigned int value[FILTER_PATTERN_MAX];
    unsigned int mask[FILTER_PATTERN_MAX];
...
typedef struct rtk_filter_field {
    ...
    unsigned int fieldTemplateNo;
    unsigned int fieldTemplateIdx[RTK_FILTER_FIELD_USED_MAX];
...
typedef struct
{
	struct rtk_filter_field *fieldHead;
...
typedef struct
{
    unsigned int      dataFieldRaw[RTK_FILTER_RAW_FIELD_NUMBER];
    unsigned int      careFieldRaw[RTK_FILTER_RAW_FIELD_NUMBER];
...
{
    unsigned char value[RTK_MAX_NUM_OF_INTERRUPT_TYPE];
} rtk_int_status_t;
...
typedef struct rtk_l2_addr_table_s
{
    ...
    unsigned int 	cvid;
	unsigned int 	fid;
	unsigned int 	is_ipmul;
	unsigned int 	is_static;
}rtk_l2_addr_table_t;


typedef unsigned int  rtk_mac_cnt_t;     /* meter id type  */
...
typedef unsigned int  rtk_meter_id_t;     /* meter id type  */
...
{
    unsigned int    AutoNegotiation;  /*PHY register 0.12 setting for auto-negotiation process*/
    unsigned int    Half_10;          /*PHY register 4.5 setting for 10BASE-TX half duplex capable*/
    unsigned int    Full_10;          /*PHY register 4.6 setting for 10BASE-TX full duplex capable*/
    unsigned int    Half_100;         /*PHY register 4.7 setting for 100BASE-TX half duplex capable*/
    unsigned int    Full_100;         /*PHY register 4.8 setting for 100BASE-TX full duplex capable*/
    unsigned int    Full_1000;        /*PHY register 9.9 setting for 1000BASE-T full duplex capable*/
    unsigned int    FC;               /*PHY register 4.10 setting for flow control capability*/
    unsigned int    AsyFC;            /*PHY register 4.11 setting for  asymmetric flow control capability*/
} rtk_port_phy_ability_t;

typedef unsigned int  rtk_port_phy_data_t;     /* phy page  */
...
typedef unsigned int  rtk_port_phy_page_t;     /* phy page  */
...
typedef struct rtk_rtctResult_s
{
    ...
    union
    {
        struct fe_result_s
        {
            unsigned int      isRxShort;
            unsigned int      isTxShort;
            unsigned int      isRxOpen;
            unsigned int      isTxOpen;
            unsigned int      isRxMismatch;
            unsigned int      isTxMismatch;
            unsigned int      isRxLinedriver;
            unsigned int      isTxLinedriver;
            unsigned int      rxLen;
            unsigned int      txLen;
        } fe_result;
...
typedef unsigned int  rtk_pri_t;         /* priority vlaue */

typedef struct rtk_priority_select_s
{
    unsigned int port_pri;
    unsigned int dot1q_pri;
    unsigned int acl_pri;
    unsigned int dscp_pri;
    unsigned int cvlan_pri;
    unsigned int svlan_pri;
    unsigned int dmac_pri;
    unsigned int smac_pri;
} rtk_priority_select_t;


typedef unsigned int  rtk_qid_t;        /* queue id type */

typedef struct rtk_qos_pri2queue_s
{
    unsigned int pri2queue[RTK_MAX_NUM_OF_PRIORITY];
} rtk_qos_pri2queue_t;

typedef struct rtk_qos_queue_weights_s
{
    unsigned int weights[RTK_MAX_NUM_OF_QUEUE];
...
typedef unsigned int  rtk_queue_num_t;    /* queue number*/
...
typedef unsigned int  rtk_rate_t;     /* rate type  */

typedef unsigned long long rtk_stat_counter_t;
...
typedef struct rtk_stat_port_cntr_s
{
    ...
    unsigned int inMldChecksumError;
    unsigned int inIgmpChecksumError;
    unsigned int inMldSpecificQuery;
    unsigned int inMldGeneralQuery;
    unsigned int inIgmpSpecificQuery;
    unsigned int inIgmpGeneralQuery;
    unsigned int inMldLeaves;
    unsigned int inIgmpLeaves;
    unsigned int inIgmpJoinsSuccess;
    unsigned int inIgmpJoinsFail;
    unsigned int inMldJoinsSuccess;
    unsigned int inMldJoinsFail;
    unsigned int inReportSuppressionDrop;
    unsigned int inLeaveSuppressionDrop;
    unsigned int outIgmpReports;
    unsigned int outIgmpLeaves;
    unsigned int outIgmpGeneralQuery;
    unsigned int outIgmpSpecificQuery;
    unsigned int outMldReports;
    unsigned int outMldLeaves;
    unsigned int outMldGeneralQuery;
    unsigned int outMldSpecificQuery;
    unsigned int inKnownMulticastPkts;
...
typedef unsigned int  rtk_stg_t;        /* spanning tree instance id type */
...
typedef unsigned int  rtk_stp_msti_id_t;     /* MSTI ID  */
...
typedef unsigned int rtk_svlan_index_t;

typedef struct rtk_svlan_memberCfg_s{
    unsigned int svid;
    unsigned int memberport;
    unsigned int untagport;
    unsigned int fiden;
    unsigned int fid;
...
typedef unsigned int rtk_svlan_tpid_t;
...
{
    unsigned char value[RTK_MAX_NUM_OF_TRUNK_HASH_VAL];
} rtk_trunk_hashVal2Port_t;

typedef unsigned int  rtk_vlan_proto_type_t;     /* protocol and port based VLAN protocol type  */
